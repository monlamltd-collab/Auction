...
> **⚠️ SUPERSEDED 2026-05-08.** This plan describes a `lib/extractors/details/` symmetric two-tier DOM-extractor architecture that was abandoned. The 2026-05-08 refactor retired the entire DOM-extractor system (`lib/extractors/` and all test snapshots) in favour of a unified Firecrawl JSON-extract pipeline (`lib/pipeline/firecrawl-extract.js`) plus per-house markdown recognisers in `HOUSE_OVERRIDES`. Detail-page hydration now flows through `lib/scraper/lot-detail.js` with a 30-day `lot_details` cache. For current architecture see `CLAUDE.md` and `.claude/skills/auction-conventions/SKILL.md`. This file is retained only as historical reference.
