...
| `circuit_open_deadlock` | A house FIXED in code (recogniser/URL shipped, verified against the live page) still returns 0/stale lots; `house_skills` shows `circuit_state` flapping back to `open` + `health_score` 0 + a stale `last_success_at`, and the deploy logs read `AUTO: Skipping {house} — circuit breaker open`. A manual `UPDATE house_skills SET circuit_state='closed'` does NOT stick: `isCircuitOpen()` reads the in-memory `_healthMap` (`lib/harness/house-health.js`), loaded ONLY at server startup (`initHouseHealth`, server.js), and the harness manager re-opens the circuit from in-memory `health=0`. Deadlock — the house can't scrape to raise its health, so the circuit never closes, so it never scrapes. cliveemson (recogniser #136 never ran), charlesdarrow, sdlauctions 2026-06-27. | **`POST /api/admin/rescrape {house}` now bypasses the circuit** (`ignoreCircuit` opt on `autoAnalyseOne`; the cron path still respects it). The one operator scrape runs and `updateHealth()` closes the circuit on success. Pre-bypass workaround (old code): set DB `circuit_state='closed', health_score=50, consecutive_failures=0` THEN restart the `Auction` service so `_healthMap` reloads (health≥40 stops the manager re-opening it). **AUTO-RECOVERY (2026-06-28):** `isCircuitOpen()` now grants one half-open trial scrape when a circuit has been open ≥24h (resetting the clock so a failed trial backs off another 24h), so the cron self-drains open circuits instead of skipping them forever — this is what cleared the systemic 117-house deadlock that froze the long tail after the 2026-06-13 Crawlee cutover (only ~22 of 168 houses were being scraped/day). Manual rescrape is still the fast path; auto-recovery is the safety net. Genuinely-dead houses re-trip and should be taken out of rotation by the dormancy sweep so they stop consuming trials. |
| `dead_sentinel_silent_blackout` | A house is at **0 live** but has NO `recall_diagnostic` / `recall_below_100` history at all — the harness never complained because its **recall sentinel matches nothing**, so recall computes 0/0 instead of 0/N. Tell-tale: the sentinel regex ends in a lookahead like `(?=$|[/?#])` while real lot URLs sit inside markdown's `](…)` or HTML's `href="…"`. The extractor failure (usually an AI-quota death with no recogniser) is then invisible: the house looks quiet, not broken. savills 2026-07-21 — 287-lot catalogue, sentinel counted 0, dark for months. | **ALWAYS test the sentinel against the LIVE page before trusting any recall number.** `sentinelIdsFromText(md, RECALL_SENTINELS[slug])` on both the turndown markdown AND the raw HTML — a correct sentinel returns the same count for both. Fix the lookahead to accept the delimiters a URL actually sits behind (`(?=$|[/?#)\s"'\]])`), mirror it onto the `HOUSE_RECOGNISERS[slug].recallSentinelPattern`, and assert both forms in the house's test. Then fix the real extraction gap (normally a deterministic recogniser). Ref: savills PR — sentinel + calendar drill + static recogniser, 0 → 306 lots. |
| `concurrent_sales_partial` | A house publishes SEVERAL upcoming catalogues at once and the drill scopes to the soonest, so the later sales' live lots never ship. Looks healthy (recall 100% against the one page fetched) but the site total is higher. savills 2026-07-21 — 287 in the soonest sale, 19 more spread across three later sales = 94% shipped. | Have `resolveCatalogueUrl` return an **ordered LIST** of page targets (supported by the `staticCatalogue` block in `lib/analysis.js`; merge is first-wins by lot id, `auctionDateIso` stamped per target). Do NOT copy edwardmellor's "soonest only" rule without checking: probe each upcoming catalogue and count cards. Also note the page-1-hash short-circuit is skipped for multi-target houses — an unchanged first page says nothing about the later sales. |
| `ai_only_freshness_rot` | House shows **0 live** but `lots` still holds a full, CORRECT set of rows — they're just stale. Detection: `SELECT status, count(*), max(last_seen_at) FROM lots WHERE house=$1 GROUP BY 1;` → an `available` cohort whose `max(last_seen_at)` is **> 7 days old** (`get_active_lots` freshness window). The house has **no recogniser** and depends on the AI extractor, so when the AI quota dies the scrape "succeeds" with 0–1 lots and stops re-stamping `last_seen_at`; the good rows silently age out. Degraded AI runs also FABRICATE: williamhbrownnorwich 2026-07-21 emitted one junk lot (page LOGO as `image_url`, the catalogue URL as `url`) and had earlier invented a lot from the **office address in the page footer**. `house_skills` looks healthy throughout (health 100, circuit closed, probe `same`) — the usual dark-house signals all read green. | This is NOT a URL/circuit/captcha problem — the catalogue is fine. **Write a deterministic recogniser** so the house stops depending on the AI (same fix as suttonkersh, futureauctions). Hand-built static pages are the cheapest wins: `staticCatalogue: true, maxPages: 1` + one plain-HTTP fetch. Verify recall by counting lots that survive `normaliseScrapedLot`, not regex hits. If the source has no ended badges, take the live boundary from the **auction date embedded in the lot URL** and drop past-dated lots. Ref: `recogniseWilliamHBrownNorwichLotsFromMarkdown` + `docs/houses/williamhbrownnorwich.md`. |
