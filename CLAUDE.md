...
> **A page is not a lot (`lib/types/lot.js::isNonLotUrl`).** `normaliseScrapedLot` rejects any lot whose detail URL is the site root, an `index`/`default` page, or the catalogue URL itself — that's the page the extractor was standing on, not a lot, and it's what a degraded run emits (usually with the site logo as the image). 47 such rows had reached `lots`, 17 served as `available`; because `lots.url` is globally UNIQUE each one also squats that page's identity slot. An **empty** `detail_url` is deliberately still allowed (separate path, 0 rows in prod). Covered by `tests/test-normalise-lot.js` Test 9, which pins the boundary — `/linked-cottage` is not `/link`, and a sibling sale page is kept.
...
- **Prune overwrites `enrichment_manifest`** — `persist-lots.js` prunes with `.update({ status: 'withdrawn', enrichment_manifest: stamp })`, which REPLACES the manifest, destroying the enrichment provenance (OS Places, EPC, scoring) of every pruned lot. The ghost sweep hit the same design and now merges via the `retire_lots` RPC (2026-07-22); the prune should adopt it. Not changed in that PR to keep the fix surgical — flagged, not fixed silently.
