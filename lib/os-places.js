...
// Transient-failure cooldown. A lookup that fails for a TRANSIENT reason
// (circuit_open / 429 rate-limit / timeout) writes a short-lived negative cache
// entry tagged {transient:true}, so the SAME address isn't re-queried on every
// enrichment run while the API is unhappy. Without this the cache only ever
// stored SUCCESSES, so the ~8.4k lots that 429'd just got hammered again every
// scrape/wave/first-contact pass — and every deploy reset the in-memory breaker
// to closed, firing a fresh burst. That churn (not legitimate per-lot usage) is
// what kept OS Places 429'd and the cache frozen since 2026-04-27. 3 days is
// long enough to stop the storm, short enough to retry once the API recovers.
const TRANSIENT_FAIL_TTL_DAYS = 3;
 ...
 */
// Write a short-TTL transient-failure marker so a 429/circuit_open/timeout
// address isn't re-queried every run (see TRANSIENT_FAIL_TTL_DAYS). Best-effort.
async function cacheTransientFail(cacheKey, postcode) {
  ...
  try {
    await supabase.from('os_places_cache').upsert({
      ...
      lng: null,
      match_score: null,
      raw_response: { transient: true },
      fetched_at: new Date().toISOString(),
    }, { onConflict: 'address_key' });
  } catch { /* non-fatal — worst case we re-query sooner */ }
}
...
export async function lookupAddress(lot) {
  ...
  if (supabase) {
    try {
      ...
      if (cached) {
        ...
        const isTransient = !cached.uprn && cached.raw_response && cached.raw_response.transient === true;
        const ttlDays = cached.uprn ? CACHE_TTL_DAYS
          : (isTransient ? TRANSIENT_FAIL_TTL_DAYS : NEGATIVE_CACHE_TTL_DAYS);
        if (ageDays < ttlDays) {
          if (cached.uprn) {
            const r = {
              status: 'cache_hit',
  ...
  if (osBreaker.isOpen()) {
    // Cool this address down so it isn't re-selected every run while the
    // breaker rides out the rate-limit — this is what stops the churn.
    await cacheTransientFail(cacheKey, postcode);
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      // Cool down on transient codes (429 rate-limit / 5xx) so we don't re-hammer
      // the same address next run. NOT on 401/403 — that's an auth/key problem we
      // must keep surfacing, not mask for 3 days.
      if (resp.status === 429 || resp.status >= 500) await cacheTransientFail(cacheKey, postcode);
    ...
    // Timeout / network error — both transient; cool the address down.
    await cacheTransientFail(cacheKey, postcode);
