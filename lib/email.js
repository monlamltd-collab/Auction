...
// Generic transactional sender — used by saved-search alerts and any
// other one-off notification flow. Returns { ok: boolean }, never throws.
// Caller is responsible for idempotency (we don't dedupe at this layer).
export async function sendTransactionalEmail({ to, subject, html, from }) {
  const resendKey = process.env.RESEND_API_KEY;
  if (!resendKey) {
    log.warn('sendTransactionalEmail: RESEND_API_KEY not set — skipping', { to, subject });
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: from || 'Auction Brain <hello@auctionbrain.co.uk>',
        to,
        subject,
        html,
      }),
    ...
    if (!r.ok) {
      ...
      log.warn('sendTransactionalEmail: non-2xx', { to, subject, status: r.status, body: txt.slice(0, 200) });
      return { ok: false, reason: `http-${r.status}` };
    }
    return { ok: true };
  } catch (err) {
    log.error('sendTransactionalEmail: threw', { to, subject, err: err.message });
