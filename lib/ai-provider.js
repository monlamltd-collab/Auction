...
async function callOpenRouterProvider(prompt, model, maxTokens, systemPrompt) {
  ...
  const response = await rateLimited('openrouter', () =>
    fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages,
        max_tokens: maxTokens,
        // Let OpenRouter transparently retry the request on an alternative
        // upstream if the chosen model's provider is rate-limited.
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => '');
    throw new Error(`OpenRouter API error (${model}): ${response.status} ${body.substring(0, 200)}`);
  }
