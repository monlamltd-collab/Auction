...
async function callOpenRouterProvider(prompt, model, maxTokens, systemPrompt, { skipFallbacks = false } = {}) {
  ...
  // Backup model(s): OPENROUTER_FALLBACK_MODELS (comma-separated slugs, e.g.
  // "deepseek/deepseek-chat-v4-flash"). OpenRouter's `models` array tries each
  // in order within ONE request, so if the primary model is down/rate-limited
  // it transparently rolls over to the next — model-level resilience on top of
  // the provider-level cascade in callAI. skipFallbacks pins the call to exactly
  // one model — used by callSpecificModel so a benchmark measures the model it
  // names, not a silent backup.
  const backups = skipFallbacks ? [] : (process.env.OPENROUTER_FALLBACK_MODELS || '')
...
export async function callAI(prompt, { tier = 'fast', maxTokens = 8000, systemPrompt = null, pdfBase64 = null, taskType = 'extraction', budgetExempt = false, userId = null } = {}) {
  ...
  for (let idx = 0; idx < chain.length; idx++) {
    ...
    try {
      ...
      logAICost({ provider: providerName, model, tokensIn, tokensOut, estCost, taskType, durationMs, userId });
