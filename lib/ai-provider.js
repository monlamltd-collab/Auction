...
const OPENROUTER_MAX_ATTEMPTS = Math.max(1, parseInt(process.env.OPENROUTER_MAX_ATTEMPTS || '3'));
...
export function openRouterBackoffMs(attempt, retryAfterMs) {
  if (retryAfterMs != null && Number.isFinite(retryAfterMs)) return Math.min(Math.max(retryAfterMs, 0), 8000);
  const base = 400 * Math.pow(2, attempt - 1); // 400ms, 800ms, 1600ms, …
  return Math.min(base + base * 0.25 * Math.random(), 8000); // +≤25% jitter, cap 8s
}
