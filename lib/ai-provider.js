...
const PROVIDERS = {
  ...
  openrouter: {
    ...
    pricing: {
      // Per-1M-token cost of the models we actually route to; OpenRouter bills
      // the underlying model. estimateCost returns 0 for any UNLISTED model, so
      // an unlisted OPENROUTER_*_MODEL logs $0 (cost-blind) rather than crashing
      // — keep this table in step with the configured chains. Verified against
      // the live OpenRouter models API 2026-07-07.
      'google/gemini-2.5-flash-lite': { input: 0.10, output: 0.40 },
      'google/gemini-2.5-flash':      { input: 0.30, output: 2.50 },
      'google/gemini-2.5-pro':        { input: 1.25, output: 10.00 },
      // Gemini 2.5 slugs are being superseded by 3.x on OpenRouter — the
      // current-gen cheap Google models for the fallback slot:
      'google/gemini-3.1-flash-lite': { input: 0.25, output: 1.50 },
      'google/gemini-3.5-flash':      { input: 1.50, output: 9.00 },
      // DeepSeek V4 family. deepseek-v4-pro is the current fast-tier model but
      // was UNLISTED → logging $0 (invisible cost); deepseek-v4-flash is the
      // proposed capable-tier replacement for Gemini 2.5 Pro (~34x cheaper/call).
...
// ── Fallback-frequency telemetry (reset daily with the budget) ──
// providerFallbackCount = callAI rolled past the PRIMARY provider (e.g. a Gemini
// 429 rolled to OpenRouter). modelFallbackCount = OpenRouter served a request via
// a BACKUP model (e.g. the free non-Gemini rung) rather than the one asked for.
// Surfaced via getAICostSummary so the operator can see how often the chain is
// rescuing extraction from a Gemini-wide quota outage.
...
// Default in-request backup model when OPENROUTER_FALLBACK_MODELS is unset: a
// free, NON-Gemini text model. OpenRouter's default fast/capable models are BOTH
// Gemini (google/gemini-2.5-*), so without a non-Gemini rung the "fallback chain"
// just rolls Gemini→Gemini and dies with the free-tier 429 (2026-06-17 extraction
// cascade: flash-lite + pro both quota-exhausted → ai_extraction_failure across
// 6+ houses). meta-llama/llama-3.3-70b-instruct:free is the cheapest workable
// escape hatch; reorder/override via OPENROUTER_FALLBACK_MODELS (set it to ''
// to disable). Verified live slug 2026-06-17 (openrouter.ai/models).
const DEFAULT_OPENROUTER_FALLBACK_MODELS = 'meta-llama/llama-3.3-70b-instruct:free';
export function openRouterGlobalBackups() {
  // `??` (not `||`) so an explicit OPENROUTER_FALLBACK_MODELS='' disables the
  // default rather than silently re-enabling it.
  const raw = process.env.OPENROUTER_FALLBACK_MODELS ?? DEFAULT_OPENROUTER_FALLBACK_MODELS;
...
function estimateCost(provider, model, tokensIn, tokensOut) {
  ...
  // OpenRouter often reports a DATE-SUFFIXED served slug (e.g.
  // "deepseek/deepseek-v4-pro-20260423") while the pricing table is keyed on
  // the clean slug — strip a trailing -YYYYMMDD so the cost still resolves
  // instead of silently logging $0 (2026-07-07: deepseek-v4-pro logged $0).
  const pricing = providerConfig.pricing[model]
    || providerConfig.pricing[String(model).replace(/-\d{8}$/, '')];
  if (!pricing) return 0;
...
// ── OpenRouter transient-failure retry ──
// OpenRouter is the primary provider (AI_PROVIDER=openrouter). When its request
// throws, callAI falls through to the DIRECT Gemini provider — whose free-tier
// key is quota-dead, so the fallback is a guaranteed 429 that kills extraction
// (the mid-June/July dark-house cascade). Most OpenRouter failures are transient
// (429 rate-limit, 5xx, a dropped connection); retrying with backoff keeps the
// call on OpenRouter's paid billing instead of collapsing onto dead Gemini.
// 4xx that won't recover (400 bad request / context-length, 401/403 auth, 402
// no credits, 404 unknown model) are NOT retried — they need config, not time.
// parseInt of a non-numeric env value is NaN, and Math.max(1, NaN) is NaN — a
// NaN attempt-cap makes the retry loop never run (attempt <= NaN is false), so
// the call throws WITHOUT ever hitting OpenRouter and cascades to dead Gemini:
// the exact failure this retry logic exists to prevent. Coerce with a finite
// fallback so a misconfigured value degrades to the default, not to zero calls.
export function positiveIntEnv(raw, def) {
  const n = parseInt(raw, 10);
  return Number.isFinite(n) && n > 0 ? n : def;
}
const OPENROUTER_BACKOFF_CAP_MS = 8000;
const OPENROUTER_MAX_ATTEMPTS = positiveIntEnv(process.env.OPENROUTER_MAX_ATTEMPTS, 3);
const OPENROUTER_RETRYABLE_STATUS = new Set([408, 409, 425, 429, 500, 502, 503, 504]);
// Strip OpenRouter's snapshot suffix (`-20260423`) and `:free` tag so an undated
// request served as its dated snapshot (deepseek-v4-flash → …-20260423) is NOT
// misread as a backup-model fallover — it's the same model, normal resolution.
export function baseModelSlug(s) {
  return String(s || '').replace(/-\d{8}$/, '').replace(/:free$/i, '');
}
export function openRouterBackoffMs(attempt, retryAfterMs) {
  if (retryAfterMs != null && Number.isFinite(retryAfterMs)) return Math.min(Math.max(retryAfterMs, 0), OPENROUTER_BACKOFF_CAP_MS);
  const base = 400 * Math.pow(2, attempt - 1); // 400ms, 800ms, 1600ms, …
  return Math.min(base + base * 0.25 * Math.random(), OPENROUTER_BACKOFF_CAP_MS); // +≤25% jitter
}
...
async function callOpenRouterProvider(prompt, model, maxTokens, systemPrompt, { skipFallbacks = false } = {}) {
  ...
  // Retry transient failures before letting callAI fall through to dead Gemini.
  let lastErr;
  for (let attempt = 1; attempt <= OPENROUTER_MAX_ATTEMPTS; attempt++) {
    let response;
    try {
      response = await rateLimited('openrouter', () =>
      ...
      // Dropped connection / DNS / timeout — always transient, always retry.
      lastErr = new Error(`OpenRouter network error (${model}): ${netErr.message}`);
      if (attempt < OPENROUTER_MAX_ATTEMPTS) {
        await new Promise(r => setTimeout(r, openRouterBackoffMs(attempt, null)));
        continue;
      }
      throw lastErr;
    }
    ...
    if (!response.ok) {
      ...
      const err = new Error(`OpenRouter API error (${model}${backups.length ? ` +${backups.length} backup` : ''}): ${response.status} ${errBody.substring(0, 200)}`);
      err.status = response.status;
      if (OPENROUTER_RETRYABLE_STATUS.has(response.status) && attempt < OPENROUTER_MAX_ATTEMPTS) {
        const retryAfterHdr = parseFloat(response.headers.get('retry-after'));
        const retryAfterMs = Number.isFinite(retryAfterHdr) ? retryAfterHdr * 1000 : null;
        // If the server demands a wait longer than our backoff cap, sleeping the
        // cap just guarantees a second 429 (the window isn't clear yet) — a futile
        // ~2×cap dead wait before the inevitable fail-over. Fail over NOW so the
        // house retries on the next scrape cycle instead of blocking extraction.
        if (retryAfterMs != null && retryAfterMs > OPENROUTER_BACKOFF_CAP_MS) {
          err.message += ` [Retry-After ${retryAfterMs}ms > ${OPENROUTER_BACKOFF_CAP_MS}ms cap — failing over now]`;
          throw err;
        }
        console.warn(`AI: OpenRouter ${response.status} (${model}) — retry ${attempt}/${OPENROUTER_MAX_ATTEMPTS - 1}`);
        await new Promise(r => setTimeout(r, openRouterBackoffMs(attempt, retryAfterMs)));
        lastErr = err;
        continue;
      }
      throw err; // non-retryable (4xx config error) or retries exhausted
    }

    const data = await response.json();
    const choice = data.choices?.[0];
    if (!choice) throw new Error(`OpenRouter returned empty response (${model}): ${JSON.stringify(data).substring(0, 200)}`);
    // data.model reports which model actually served the request. Only count/log
    // a genuine backup fallover — compare BASE slugs so an undated primary served
    // as its dated snapshot (normal resolution) is not mistaken for a backup.
    if (data.model && baseModelSlug(data.model) !== baseModelSlug(model)) {
      modelFallbackCount++;
  ...
  throw lastErr || new Error(`OpenRouter: exhausted ${OPENROUTER_MAX_ATTEMPTS} attempts (${model})`);
}
...
export async function callAI(prompt, { tier = 'fast', maxTokens = 8000, systemPrompt = null, pdfBase64 = null, taskType = 'extraction', budgetExempt = false, userId = null } = {}) {
  ...
  let lastErr;
  const providerErrors = []; // one "{provider}: {message}" per failed rung
  for (let idx = 0; idx < chain.length; idx++) {
    ...
    try {
      ...
      if (idx > 0) {
        ...
        console.log(`AI: ${providerName} (model ${servedModel}) succeeded after ${idx} provider failure(s) [${taskType}]`);
      }
      return result.text;
    } catch (err) {
      lastErr = err;
      providerErrors.push(`${providerName}: ${err.message}`);
  ...
  // Surface the WHOLE chain, primary first. Throwing only lastErr (the last
  // rung) misdirects triage — an OpenRouter-caused outage would be reported as
  // a direct-Gemini quota 429 (the dead free-tier fallback), sending the
  // operator to the wrong provider. Lead with the primary so the alert names
  // the true root cause; keep the Error type from the last rung.
  if (chain.length > 1 && providerErrors.length > 1) {
    const agg = new Error(`All AI providers failed [${chain.join(' → ')}] — ${providerErrors.join(' | ')}`);
    if (lastErr && lastErr.status != null) agg.status = lastErr.status;
    throw agg;
  }
...
export function getAICostSummary() {
  return {
    ...
    // Fallback frequency (today): how often the chain rolled past the primary
    // provider / OpenRouter rolled to a backup model. Non-zero = the primary
    // (usually Gemini) is degraded and the non-Gemini rung is earning its keep.
    providerFallbacks: providerFallbackCount,
    modelFallbacks: modelFallbackCount,
  };
}
