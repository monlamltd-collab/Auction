...
// behaviour change.
//
// Live DB columns intentionally OMITTED from this set:
//   id, raw_text, enriched_at, first_seen_at, extracted_with, scraped_with,
//   search_vector (generated), field_sources, uprn, property_key,
//   os_classification, auction_id, floor_plan_url
//
// They exist on the table but aren't part of the standard API-rendering
// contract. Callers that need them must expand their `.select()` explicitly.
// (Note: `dbRowToLot` below still emits `enrichedAt` and `rawText` keys for
// back-compat with the legacy createDbRowToLot consumers — they will be
// undefined unless the caller's select includes those columns. This matches
...
export function dbRowToLot(row) {
  ...
  return {
    ...
    epcDate: row.epc_date,
    epcFloorAreaSqm: row.epc_floor_area_sqm ?? null,
    epcFloorAreaSqft: row.epc_floor_area_sqft ?? null,
    epcWorksCostMid: row.epc_works_cost_mid ?? null,
    epcWorksSummary: Array.isArray(row.epc_works_summary) ? row.epc_works_summary : [],
    ...
    // ── Legacy internal-mapper-only fields (undefined under canonical LOTS_SELECT) ──
...
export function lotToDbRow(lot) {
  ...
  set('price_status', lot.priceStatus);
  ...
  set('floor_plan_url', lot.floorPlanUrl);
  ...
  set('sold_price', lot.soldPrice);

  // ── EPC ──
  set('epc_rating', lot.epcRating);
  set('epc_score', lot.epcScore);
  set('epc_date', lot.epcDate);
  set('epc_floor_area_sqm', lot.epcFloorAreaSqm);
  set('epc_floor_area_sqft', lot.epcFloorAreaSqft);
  set('epc_works_cost_mid', lot.epcWorksCostMid);
  set('epc_works_summary', lot.epcWorksSummary);
  ...
  set('raw_text', lot.rawText);
  set('search_text', lot._searchText);

  // ── Observability ──
  set('enrichment_manifest', lot._enrichment);
  set('quality_score', lot.qualityScore);
  set('quality_issues', lot.qualityIssues);
