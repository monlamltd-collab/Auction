...
//   enriched_at, first_seen_at, created_at, sources, uprn, property_key,
...
export const LOT_COLUMNS = Object.freeze([
  'id',  // DB UUID -> lot._dbId; shareable /lot/:id + ?lot= drawer state
  'house',   // logical key. Live DB has BOTH house (canonical, NOT NULL) and house_slug (nullable shadow, kept = house by trigger lots_sync_house_slug). LOTS_SELECT reads house_slug; see note below.
  'auctioneer',
  'lot_number',
  'url',
  'catalogue_url',
...
// Physical-column aliases for the PostgREST select. LOT_COLUMNS holds LOGICAL
// field names (the keys consumers read); where the physical column differs, map
// it here as 'logicalKey:physical_column'. Phase 2a (expand phase, not a
// completed rename): lots.house is still the canonical NOT NULL column and
// house_slug is a nullable shadow kept identical to it by the
// lots_sync_house_slug trigger (see migrations/2026-06-27-lots-house-slug-shim.sql).
// This alias reads house_slug and returns it under the JSON key `house`, so no
// consumer — nor the LOT_COLUMNS contract test (tests/test-lot-columns.js) — has
// to change. NB house_slug is nullable; it is only guaranteed populated because
// the trigger mirrors house on every write. If the contract phase ever drops the
// trigger, switch this alias to read the canonical `house` first.
const SELECT_ALIASES = Object.freeze({ house: 'house:house_slug' });
export const LOTS_SELECT = LOT_COLUMNS.map(c => SELECT_ALIASES[c] || c).join(', ');
...
export function dbRowToLot(row) {
  ...
  return {
    ...
    statedIncomePa: row.stated_income_pa ?? null,
    incomeKind: row.income_kind || null,
...
export function lotToDbRow(lot) {
  ...
  // Integer columns: coerce so a fractional value (e.g. AI extraction returns
  // bedrooms 2.5 — lot-schema declares `bedrooms` a number, not an integer)
  // never reaches an INTEGER column. Postgres rejects "2.5" with "invalid input
  // syntax for type integer", which fails the ENTIRE 50-row batch upsert and
  // silently drops those lots. Round numeric-ish values; null anything else.
  const coercions = [];
  const setInt = (col, val) => {
    if (val === undefined) return;
    if (val === null || val === '') { out[col] = null; return; }
    const n = typeof val === 'number' ? val : parseFloat(val);
    const rounded = Number.isFinite(n) ? Math.round(n) : null;
    out[col] = rounded;
    // Audit only a GENUINE alteration of a real source value — a fractional or
    // unparseable value that got rounded or nulled. A clean "3" -> 3 is not noise.
    if (rounded === null || rounded !== n) coercions.push({ field: col, from: val, to: rounded });
  };
  ...
  // ── Pricing ──
  setInt('price', lot.price);
  set('price_text', lot.priceText);

  // ── Property attributes ──
  set('prop_type', lot.propType);
  setInt('beds', lot.beds);
  set('tenure', lot.tenure);
  setInt('lease_length', lot.leaseLength);
  setInt('sqft', lot.sqft);
  ...
  // ── Flood ──
  setInt('flood_zone', lot.floodZone);
  ...
  setInt('street_sales_count', lot.streetSalesCount);
  setInt('below_market', lot.belowMarket);
  setInt('est_monthly_rent', lot.estMonthlyRent);
  ...
  set('deal_signals', lot.dealSignals);
  set('stated_income_pa', lot.statedIncomePa);
  set('income_kind', lot.incomeKind);
  ...
  // Per-lot audit trail for any value the integer boundary had to alter, so a
  // silently-coerced field (e.g. beds 2.5 → 3) is visible in enrichment_manifest,
  // not just the console. Inlined rather than via recordFieldCoercion() to keep
  // this leaf module free of lib/ imports; mirrors that recorder's shape.
  if (coercions.length && out.enrichment_manifest && typeof out.enrichment_manifest === 'object') {
    const m = out.enrichment_manifest;
    if (!Array.isArray(m.data_hygiene)) m.data_hygiene = [];
    for (const c of coercions) m.data_hygiene.push({ kind: 'int_coercion', ...c });
  }
 ...
 */
/**
 * Is this "detail URL" actually a page rather than a lot?
 *
 * A degraded extraction run emits the page it was standing on as if it were a
 * lot: the site root, an index page, or the catalogue URL itself — typically
 * with the site logo as the image (williamhbrownnorwich 2026-07-21 shipped
 * exactly one such lot; bagshaws had two). 47 of these rows reached `lots`,
 * 17 of them served as `available`, and because `lots.url` is UNIQUE each one
 * also squats the identity slot for that page.
 *
 * Deterministic and cheap, so it runs at the single funnel every scraped lot
 * passes through. An EMPTY url is deliberately NOT rejected here — that path
 * has its own handling and no rows in prod.
 */
export function isNonLotUrl(url, catalogueUrl = '') {
  ...
  const norm = (u) => {
    try {
      ...
      return { path, key: `${host}${path}${p.search}` };
    } catch { return null; }
  };
  const u = norm(raw);
  if (!u) return false;                                    // unparseable — leave to other validation
  if (u.path === '') return true;                          // bare origin
  if (/^\/(index|default)\.html?$/.test(u.path)) return true;
  const c = catalogueUrl ? norm(catalogueUrl) : null;
  return Boolean(c && u.key === c.key);                    // the catalogue page itself
}
...
export function normaliseScrapedLot(raw, { house, catalogueUrl, extractionSource = 'firecrawl-json' } = {}) {
  ...
  // A "lot" whose detail URL is the site root, an index page, or the catalogue
  // page itself is not a lot — it's the page the extractor was standing on.
  const detailUrl = stripEigCatalogueParams(raw.detail_url || '');
  if (isNonLotUrl(detailUrl, catalogueUrl)) return null;
  ...
  return {
    ...
    url: detailUrl,
    bullets,
    // Narrative passthrough: the source description survives as its own field
    // (persisted to lots.description) — the bullets fold above is a display/
    // signal convenience, not the narrative's storage home. The narrative
    // sweep later replaces short card-level text with the full detail-page
    // narrative (lib/pipeline/narrative-sweep.js).
    description: (typeof raw.description === 'string' && raw.description.trim().length >= 20)
