...
// Placeholder/template-address detector. These are the shapes an LLM emits when it
// "extracts" a contentless/SPA page (the SDL Auctions 2026-06 incident: "456 Elm
// Street", "4 Willow Close, Anytown, AT1 1AD"). Keys on ADDRESS content only —
// never on URL shape, because real catalogues use /property/{id}/ URLs.
// High precision — a real UK address never matches these patterns.
export const PLACEHOLDER_ADDRESS_RE = /\b(?:anytown|sometown|exampletown|demo(?:ville)?|sampletown)\b|\bAT1\s*1A[A-Z]\b|^\d{1,4}\s+(?:elm|oak|maple|pine|poplar|willow|cedar|birch)\s+(?:street|road|close|drive|avenue|lane)\b/i;

/**
 * Reject a batch of extracted catalogue lots that is clearly fabricated/hallucinated
 * (synthetic template addresses). Keys on ADDRESS content only — never on URL shape,
 * because real catalogues use /property/{id}/ URLs. Conservative: needs >= 5 lots and
 * a high placeholder ratio so a genuine small catalogue is never killed.
