...
export async function enrichLotsFromLotPages(lots, opts = {}) {
  ...
  const addrIsTownOnly = a => /^[^,\d]+\s+-\s+[^,\d]+$/.test((a || '').trim()); // "Sittingbourne - Kent" placeholder shape
  const isGapFillTarget = (l) => (
    !l.address || l.address.trim().length < 5
    || addrIsDescription(l.address || '')
    || addrIsTownOnly(l.address)
    || !l.postcode
    || !l.imageUrl
    || !l.tenure
    || !l.condition
  ...
  for (const l of lots) {
    ...
    // 'never-deep' — the catalogue (a JSON API on both current houses) already
    // carries everything a detail page could add, so a re-scrape fetch is pure
    // waste. Until 2026-07-22 this branch gap-filled exactly like the default
    // below, so the policy did nothing; worse, isGapFillTarget tests fields
    // these APIs never publish (`condition`, `vacant`), so EVERY lot requalified
    // on EVERY cycle forever — on underthehammer up to DETAIL_FETCH_CAP_PER_RUN
    // Crawlee browser renders per cycle against Next.js SPA shells that yield
    // nothing. First-contact lots still bypass this via the branch above: the
    // one-off kitchen-sink pass is a separate deliberate invariant
    // (enrich-stage.js) and is bounded to once per lot URL, ever.
  ...
  // Skipped, not silent — a house whose profile is wrong should be visible in
  // the logs rather than quietly never deep-fetching.
  if (neverDeepSkipped > 0) {
    console.log(`Lot-page enrichment: ${neverDeepSkipped} already-known lots skipped (never-deep profile — catalogue is canonical)`);
  }
  ...
  for (let i = 0; i < targets.length; i += concurrency) {
    ...
    await Promise.allSettled(batch.map(async (lot) => {
      try {
        ...
        // "Town - County" placeholder addresses (cliveemson's catalogue cards
        // carry only e.g. "Sittingbourne - Kent", so 14 distinct lots rendered
        // as identical cards — 2026-07-03 duplicate-lots incident). Upgrade
        // from the detail page, but ONLY to a candidate that is genuinely more
        // specific (has a comma or a postcode-shaped token) — a page headline
        // must never replace a real, if coarse, location.
        const addrIsTownOnly = lot.address && /^[^,\d]+\s+-\s+[^,\d]+$/.test(lot.address.trim());
        if (!lot.address || lot.address.trim().length < 5 || addrLooksLikeDescription || addrIsTownOnly) {
          let address = '';
          ...
          const specificEnough = !addrIsTownOnly
            || address.includes(',')
            || /[A-Z]{1,2}\d\w?\s*\d[A-Z]{2}/i.test(address);
          if (address && address.length >= 5 && specificEnough) { setField(lot, 'address', address, 'detail-page'); stats.address++; }
        }
