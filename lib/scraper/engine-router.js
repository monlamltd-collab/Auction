...
export function chooseEngine(ctx = {}) {
  ...
  if (manualEngine) {
    ...
    if (manualEngine === ENGINES.CRAWLEE && !crawleeInstalled) {
      return firecrawlAvailable
        ? { engine: ENGINES.FIRECRAWL, reason: 'manual-lock+crawlee-unavailable' }
        : { engine: 'puppeteer', reason: 'manual-lock+crawlee-and-firecrawl-unavailable' };
    }
  ...
  // 4. Bot-protected — Firecrawl is the best anti-bot engine we run today
  //    (managed-unblocker tier is future work). Crawlee never *proactively*
  //    takes these houses; it only reaches them via the zero-credit failover
  //    below, where its fingerprint-suite hardening still beats bare Puppeteer
  //    and degraded-but-present beats a stale catalogue.
  if (botProtected) {
  ...
  // 6. Config default — CRAWLEE_DEFAULT=true makes Crawlee the main engine
  //    (Firecrawl remains the in-run fallback when Crawlee yields 0 lots).
...
//
// Failover policy (Phase 3): when Firecrawl is unavailable, Crawlee takes over
// for ANY house that has it installed (crawleeInstalled) — degraded-but-present
// beats a stale catalogue — not just allowlisted ones. Proactive selection of
// Crawlee (desired === CRAWLEE) still respects the allowlist via crawleeAvailable.
function resolveAvailability(desired, reason, { crawleeAvailable, crawleeInstalled, firecrawlAvailable }) {
  if (desired === ENGINES.CRAWLEE && !crawleeAvailable) {
    ...
    if (crawleeInstalled) return { engine: ENGINES.CRAWLEE, reason: `${reason}+firecrawl-exhausted` };
