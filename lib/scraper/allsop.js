...
import { assessLotAddress } from '../quality/address-quality.js';
...
export async function scrapeAllsopApi(baseUrl) {
  ...
  for (let pg = 1; pg <= MAX_PAGES; pg++) {
    ...
    try {
      ...
      // Stop on an empty page. The API's empty shape is {"data":{"results":[]}}
      // (NOT "data":[]), so strip whitespace and match "results":[] too —
      // otherwise pagination never detects the end and runs to MAX_PAGES.
      if (html.length < 100 || html.includes('"data":[]') ||
          html.replace(/\s/g, '').includes('"results":[]') || html.includes('"template":"404"')) {
  ...
  for (const p of pages) {
    try {
      ...
      for (const item of results) {
        ...
        // Reject city/region/portfolio shells ("Bristol", "Teesside", "The Ace
        // Portfolio") — these are marketing titles, not browseable lot cards.
        const addrGate = assessLotAddress(address, {
          postcode: item.postcode || null,
          reference: ref || null,
        });
        if (!addrGate.ok) continue;
