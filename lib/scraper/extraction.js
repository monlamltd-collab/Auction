...
export const HOUSE_EXTRACTION_HINTS = {
  ...
  paulfosh:      'EIG online auction platform (auction.paulfosh.com). Lot panels with lot number, address, guide price, images, and detail links.',
  ...
  underthehammer:          'Under The Hammer. Next.js React SPA (underthehammer.com). Served by the bespoke JSON-API scraper (lib/scraper/underthehammer.js) — this hint is a dead fallback. Property cards at /for-auction/properties with title, address, guide price, bedrooms, property type, images on blob.core.windows.net, and detail links to /property/{id}.',
  ...
  purplebricksgoto:        'Purplebricks via GOTO Properties. EIG platform (purplebricks.gotoproperties.co.uk). Standard EIG lot-panel cards with h3.list-address, guide price in list-guideprice, img.list-image, and /lot/details/ links. Full static archive requested with pagesize=5000; the deterministic recogniser retains only affirmative-live cards.',
...
// Max catalogue-content chars fed to the model in ONE extraction call. A dense
// single-page catalogue (bondwolfe: 174 lots, 128k chars) produces more JSON
// than the 16k output-token cap can hold, so the response truncates mid-array
// and ~half the lots are lost — a direct hit to THE 100% COMMANDMENT that no
// model choice can fix. Splitting the content so each call emits < 16k output
// tokens keeps every lot. ~55k chars ≈ ~75 lots ≈ ~13k output tokens < cap.
// Env-tunable; overlap ensures a lot straddling a split appears whole in one
// chunk (the grounding+dedup pass below removes the duplicate).
const MAX_EXTRACTION_CHARS_PER_CALL = parseInt(process.env.MAX_EXTRACTION_CHARS_PER_CALL || '55000');
const EXTRACTION_CHUNK_OVERLAP = parseInt(process.env.EXTRACTION_CHUNK_OVERLAP || '2500');

/**
 * Split content into overlapping chunks no larger than maxChars, snapping the
 * cut to a newline in the back half to avoid slicing mid-lot. Exported for tests.
 */
export function chunkContent(text, maxChars = MAX_EXTRACTION_CHARS_PER_CALL, overlap = EXTRACTION_CHUNK_OVERLAP) {
  const s = String(text || '');
  ...
  while (start < s.length) {
    ...
    if (end < s.length) {
      ...
      if (nl > start + maxChars * 0.5) end = nl; // snap to a newline in the back half
    }
    chunks.push(s.slice(start, end));
    if (end >= s.length) break;
    start = Math.max(start + 1, end - overlap); // overlap so boundary lots survive
  }
  return chunks;
}
...
export async function extractLotsWithAI(pages, house, onProgress, catalogueUrl, deps = {}) {
  ...
  for (let i = 0; i < pages.length; i += batchSize) {
    ...
    const contentBlock = strippedBatch.map(p => `=== PAGE ${p.page} ===\n${p.content}`).join('\n\n');
    const buildPrompt = (content) => `You are extracting property auction lot data from a UK auction house catalogue (${house}).
${hint ? `\nStructure hint: ${hint}\n` : ''}
...
Return ONLY the JSON array:`;
    try {
      // Chunk oversized content so no single call's JSON output exceeds the
      // 16k-token cap and truncates (THE 100% COMMANDMENT — a truncated dense
      // page silently drops ~half its lots). Small pages → one chunk = one call,
      // unchanged. Overlap + the dedup below make boundary lots safe.
      const contentChunks = chunkContent(contentBlock);
      if (contentChunks.length > 1) {
        console.log(`  ${house}: dense content (${contentBlock.length} chars) split into ${contentChunks.length} chunks to prevent output-cap truncation`);
      }
      const lots = [];
      let repaired = false;
      for (const chunk of contentChunks) {
        incApiCallCount();
        const text = await getCallAI()(buildPrompt(chunk), { tier: extractionTier, maxTokens: 16000, taskType: 'extraction' });
        // Truncation-tolerant parse: salvage every complete lot from a response
        // cut off at the maxTokens ceiling instead of losing the whole chunk.
        const parsed = parseLotArray(text);
        if (parsed.repaired) repaired = true;
        if (parsed.lots.length) lots.push(...parsed.lots);
      }
      log.info('ai_extraction', { house, tier: extractionTier, batch: Math.floor(i/batchSize)+1, chunks: contentChunks.length });
      if (repaired) {
        console.warn(`AI extraction: repaired truncated JSON for ${house} — salvaged ${lots.length} lot(s) across ${contentChunks.length} chunk(s)`);
      }
      if (lots.length > 0) {
