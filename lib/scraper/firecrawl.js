...
// Same as batchExtractCatalogues but submits the per-lot DETAIL_SCHEMA. Used
// to backfill lots the listing-page JSON missed. Bypasses changeTracking by
// design — detail pages are one-shot per lot.
export async function batchExtractDetails(urls, options = {}) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  if (!getBudget().canUseFirecrawl()) throw new Error('Firecrawl unavailable');

  const formats = [{ type: 'json', schema: DETAIL_SCHEMA, prompt: DETAIL_PROMPT }];
  ...
  if (!resp.ok) throw new Error(`Firecrawl batch (details) ${resp.status}: ${await resp.text().catch(() => '')}`);
  const data = await resp.json();
  if (!data.success) throw new Error(`Firecrawl batch (details) failed: ${data.error || 'unknown'}`);
...
export async function pollBatchJob(jobId) {
  ...
  return {
    ...
    results: (data.data || []).map(d => ({
      ...
      // Catalogue-shape fields (CATALOGUE_SCHEMA)
      lots: d.json?.lots || [],
      auctionDate: d.json?.auction_date || null,
      totalLots: d.json?.total_lots || null,
      changeStatus: d.changeTracking?.changeStatus || null,
      markdown: d.markdown || '',
      // Detail-shape fields (DETAIL_SCHEMA) — populated when batch was submitted via batchExtractDetails
      json: d.json || null,
      error: d.error || null,
    })),
  };
