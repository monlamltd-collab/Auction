...
// ═══════════════════════════════════════════════════════════════
// Cloudflare-bypass-only mode (Simon, 2026-06-15)
// ═══════════════════════════════════════════════════════════════
// Firecrawl exists to get PAST Cloudflare — not to scrape pages we can render
// for free with Crawlee/Puppeteer. On 2026-06-01..03 the FIRE-1 agent
// (/v2/extract, 23 credits/call) plus the steady-state JSON-extract and
// homepage-watch crons burned 6,209 credits probing NON-CF house homepages —
// exactly the per-page scraping this guard forbids. Every Firecrawl entry point
// is now restricted to the CF-bypass stealth scrape (`proxy:'stealth'`); the
// generic render/extract/agent paths throw FC_CF_BYPASS_ONLY and their callers
// fall back (Crawlee → Puppeteer → HTTP / Gemini — all already handle the throw).
// Set FIRECRAWL_CF_BYPASS_ONLY=false to restore the legacy behaviour instantly.
export function isFirecrawlCfBypassOnly() {
  return process.env.FIRECRAWL_CF_BYPASS_ONLY !== 'false';
}
export function assertFirecrawlAllowed(caller, options = {}) {
  if (isFirecrawlCfBypassOnly() && options.proxy !== 'stealth') {
    const err = new Error(
      `Firecrawl restricted to Cloudflare-bypass only — ${caller} blocked. ` +
      `Use Crawlee/Puppeteer for per-page scraping, or set FIRECRAWL_CF_BYPASS_ONLY=false.`
    );
    err.code = 'FC_CF_BYPASS_ONLY';
    throw err;
  }
}
...
export async function scrapeWithFirecrawl(url, options = {}) {
  ...
  assertFirecrawlAllowed('scrapeWithFirecrawl', options);
  ...
  // CF-bypass: `proxy:'stealth'` routes via Firecrawl's residential proxy + CF
  // solver — the only thing that passes Cloudflare from our datacenter IP (verified
  // 2026-06-14 on edwardmellor + symondsandsampson). Costs ~5 credits vs 1, so
  // callers set it ONLY for known CF-blocked houses (1,000 credits/mo budget).
  ...
  const doFetch = async () => {
    ...
    // scrapeWithFirecrawl is the basic /v2/scrape path (no JSON extract).
    // Credit weight accounts for caller-supplied actions/waits, which
    // Firecrawl bills on top of the base page fetch.
    const actionCount = Array.isArray(options.actions) ? options.actions.length : 0;
    const weight = getBudget().computeScrapeWeight({ jsonExtract: false, actionCount });
    getBudget().recordFcRequest(currentTier(), weight, {
      endpoint: '/v2/scrape',
      caller: 'firecrawl.scrapeWithFirecrawl',
      outcome: 'success',
      url,
...
export async function extractCatalogue(url, options = {}) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  assertFirecrawlAllowed('extractCatalogue', options);
  ...
  const doFetch = async () => {
    ...
    // changeTracking 'same' short-circuits server-side — Firecrawl bills it
    // as a basic scrape even though we asked for JSON extract. Weight the
    // request accordingly so the internal counter tracks the dashboard.
    const sameShortCircuit = data.data?.changeTracking?.changeStatus === 'same';
    const weight = getBudget().computeScrapeWeight({ jsonExtract: !sameShortCircuit, actionCount: 0 });
    getBudget().recordFcRequest(currentTier(), weight, {
      endpoint: '/v2/scrape',
      caller: 'firecrawl.extractCatalogue',
      outcome: 'success',
      url,
      elapsedMs: Date.now() - t0,
    });
...
export async function extractHomepage(url, options = {}) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  assertFirecrawlAllowed('extractHomepage', options);
  if (!getBudget().canUseFirecrawl()) throw new Error('Firecrawl unavailable');
  ...
  const doFetch = async () => {
    ...
    getBudget().recordFcRequest(currentTier(), weight, {
      endpoint: '/v2/scrape',
      caller: 'firecrawl.extractHomepage',
      outcome: 'success',
      url,
      elapsedMs: Date.now() - t0,
    });
...
export async function extractDetail(url, options = {}) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  assertFirecrawlAllowed('extractDetail', options);
  if (!getBudget().canUseFirecrawl()) throw new Error('Firecrawl unavailable');
  ...
  const doFetch = async () => {
    ...
    getBudget().recordFcSuccess();
    // extractDetail always requests JSON schema extraction; no changeTracking
    // here, so the JSON-extract weight always applies.
    const weight = getBudget().computeScrapeWeight({ jsonExtract: true, actionCount: 0 });
    getBudget().recordFcRequest(currentTier(), weight, {
      endpoint: '/v2/scrape',
      caller: 'firecrawl.extractDetail',
      outcome: 'success',
      url,
      elapsedMs: Date.now() - t0,
    });
...
export async function batchExtractCatalogues(urls, options = {}) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  assertFirecrawlAllowed('batchExtractCatalogues', options);
  if (!getBudget().canUseFirecrawl()) throw new Error('Firecrawl unavailable');
...
export async function mapSiteUrls(url, search) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  assertFirecrawlAllowed('mapSiteUrls');
  ...
  // Firecrawl bills /v2/map at 1 credit per call regardless of result count.
  // Previously this call was unbudgeted — see audit 5.1.
  getBudget().recordFcSuccess();
  getBudget().recordFcMapRequest(currentTier(), {
    endpoint: '/v2/map',
    caller: 'firecrawl.mapSiteUrls',
    outcome: 'success',
    url,
    elapsedMs: Date.now() - t0,
  });
...
export async function agentExtract(urls, prompt, schema, options = {}) {
  if (!getBudget().fcKey) throw new Error('FIRECRAWL_API_KEY not set');
  // FIRE-1 agent is the single most expensive FC path (23 credits/call) and is
  // never a Cloudflare-bypass — always blocked in CF-bypass-only mode.
  assertFirecrawlAllowed('agentExtract', options);
  if (!getBudget().canUseFirecrawl()) throw new Error('Firecrawl unavailable');

  const urlList = Array.isArray(urls) ? urls : [urls];
  const eventUrl = urlList[0] || null;
  const t0 = Date.now();
  ...
  while (Date.now() < deadline) {
    ...
    if (pollData.status === 'completed') {
      getBudget().recordFcSuccess();
      // Agent calls cost more than basic scrapes — use the FIRE-1 multiplier.
      getBudget().recordFcAgentRequest(currentTier(), {
        endpoint: '/v2/extract',
        caller: 'firecrawl.agentExtract',
        outcome: 'success',
        url: eventUrl,
        elapsedMs: Date.now() - t0,
      });
    ...
    if (pollData.status === 'failed' || pollData.status === 'cancelled') {
      // Firecrawl charges for started /extract jobs regardless of outcome,
      // so credit the call before throwing — otherwise BUDGET-FC will drift
      // below the dashboard total (audit 5.4).
      getBudget().recordFcAgentRequest(currentTier(), {
        endpoint: '/v2/extract',
        caller: 'firecrawl.agentExtract',
        outcome: pollData.status,
        url: eventUrl,
        elapsedMs: Date.now() - t0,
      });
  ...
  // Client-side timeout — the job is still likely running on Firecrawl's
  // side and will bill regardless. Credit it before bailing.
  getBudget().recordFcAgentRequest(currentTier(), {
    endpoint: '/v2/extract',
    caller: 'firecrawl.agentExtract',
    outcome: 'timeout',
