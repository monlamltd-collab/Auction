...
export const HOUSE_RECOGNISERS = {
  ...
  purplebricksgoto: { maxPages: 1, staticCatalogue: true, recallSentinelPattern: new RegExp(EIG_SENTINEL_SRC, 'g'), recogniseFromMarkdown: recognisePurplebricksGotoLotsFromMarkdown },
...
// Landing pages whose deterministic resolver discovers actual dated catalogues.
// Queueing these roots is safe: a resolver returns no target before publication
// and supplies the real auction date when it finds a live catalogue.
export const RESOLVER_ROOT_SLUGS = new Set(
  Object.entries(HOUSE_RECOGNISERS)
    .filter(([, cfg]) => typeof cfg?.resolveCatalogueUrl === 'function')
    .map(([slug]) => slug),
);
