...
import {
  ...
  recogniseBagshawsLotsFromMarkdown,
  recogniseWilliamHBrownNorwichLotsFromMarkdown,
...
export const HOUSE_RECOGNISERS = {
  // Pattinson moved to a BESPOKE scraper 2026-07-22 (paginateAs 'pattinson_api',
  // lib/scraper/pattinson.js): the catalogue is behind Cloudflare AND paginated
  // across 90 fixed 20-lot pages, so the render path could only ever reach ~17%.
  // The recogniser below no longer runs for this house — it stays registered as
  // the documented markdown fallback (and to resolve the recall sentinel); its
  // behaviour is still pinned by tests/test-crawlee-recognition.js Test 4.
  ...
  // Bagshaws Residential — hand-maintained static HTML page (no CMS, no JS) that
  // lists the house's slice of the Sequence/Connells national sale; every lot
  // links out to barnardmarcusauctions.co.uk/auctions/{DD-month-YYYY}/{id}/.
  // staticCatalogue: one plain-HTTP fetch carries all 23 cards with photo, guide
  // and SOLD-PRIOR badge, so no render and no AI. The sentinel matches the
  // existing RECALL_SENTINELS.bagshaws entry (lot ids on the Barnard Marcus host).
  bagshaws: { maxPages: 1, staticCatalogue: true, recallSentinelPattern: /barnardmarcusauctions\.co\.uk\/auctions\/[a-z0-9-]+\/(\d{6,})/gi, recogniseFromMarkdown: recogniseBagshawsLotsFromMarkdown },
  // William H Brown (Norwich) — hand-built static XHTML table (no JS) listing the
  // branch's own lots out of the shared Sequence / Barnard Marcus sale. No
  // recogniser before, so it depended on the quota-dead AI extractor and went
  // dark. staticCatalogue: one plain-HTTP fetch carries the whole catalogue.
  // Sentinel mirrors RECALL_SENTINELS.williamhbrownnorwich exactly.
  williamhbrownnorwich: { maxPages: 1, staticCatalogue: true, recallSentinelPattern: /barnardmarcusauctions\.co\.uk\/auctions\/[a-z0-9-]+\/(\d{6,})/gi, recogniseFromMarkdown: recogniseWilliamHBrownNorwichLotsFromMarkdown },
