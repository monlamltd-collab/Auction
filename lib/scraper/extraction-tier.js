...
// After this many runs on the 'capable' crutch, the house has earned a
// DETERMINISTIC fix — flag it once for a per-house markdown recogniser (the
// Pattinson/John Pye pattern) so even the cheap model hits full recall.
const RECOGNISER_FLAG_RUNS = parseInt(process.env.EXTRACTION_RECOGNISER_FLAG_RUNS || '5');
...
export function recordExtractionRecall(ext, recall, { at = null } = {}) {
  ...
  return {
    ...
    // Runs spent on the 'capable' crutch — feeds the needs-recogniser flag.
    capableRuns: tier === 'capable' ? (prev.capableRuns || 0) + 1 : 0,
    recogniserFlaggedAt: prev.recogniserFlaggedAt || null,
  };
}

/**
 * True when this house has leaned on the 'capable' model tier long enough to
 * deserve a deterministic per-house recogniser instead — and hasn't been
 * flagged yet. The caller fires the needs_recogniser alert and stamps
 * recogniserFlaggedAt so it fires exactly once per promotion episode.
 */
export function shouldFlagForRecogniser(ext) {
  return !!ext && ext.tier === 'capable'
    && (ext.capableRuns || 0) >= RECOGNISER_FLAG_RUNS
    && !ext.recogniserFlaggedAt;
}
...
export const _thresholds = { WEAK_RECALL, STRONG_RECALL, MIN_RUNS, EWMA_ALPHA, ALLOW_DEMOTE, RECOGNISER_FLAG_RUNS };
