    ...
    lots: {
      ...
      items: {
        ...
        properties: {
          ...
          address: { type: 'string', description: 'Full UK postal address — must include a street name (and ideally a postcode). Do NOT return property-type descriptors (e.g. "A three bedroom semi-detached house") — those are descriptions, not addresses; return null instead. Do NOT return banners, viewing buttons, or status labels (e.g. "Virtual Viewing", "Sold prior to auction", "National online auction bidding now open!", "Click to view lots") — return null instead. If the card does not expose a real address, return null rather than guessing or substituting a description.' },
          ...
          image_url: { type: 'string', description: 'PRIORITY field. Main property image URL — full URL starting with https://. Search the card for an `<img>` tag and ALSO inspect data-src, data-lazy-src, data-original, and srcset attributes (lazy-loaded sites store the real URL on those rather than `src`). If srcset is present, prefer the largest variant. PROXY UNWRAP: if the URL you find is a Next.js image-proxy wrapper of the form `<host>/_next/image?url=<inner>` (commonly with host `cdn.example.com`), do NOT return the wrapper — URL-decode and return the inner `<inner>` value instead (e.g. for `https://cdn.example.com/_next/image?url=https%3A%2F%2Freal.cdn%2Fimg.jpg&w=3840&q=75` return `https://real.cdn/img.jpg`). Reject: data:image base64 placeholders, 1x1 tracking pixels, logo/icon thumbnails (avatars, "no image available" stock icons). Return null only if the card genuinely has no property photo at all — do NOT return null just because the image is hidden behind lazy-load attributes; check those attributes first.' },
...
//
// Address discipline added 2026-05-05 after diagnosing 1,231 stranded lots
// across 425 dupe-groups whose `address` field was placeholder/banner text
// (e.g. "A three bedroom semi-detached house", "Virtual Viewing"). The LLM
// was confidently returning generic descriptors when the listing card didn't
// expose a real address. We now explicitly tell it to return null instead.
export const CATALOGUE_PROMPT =
  ...
  'ADDRESS DISCIPLINE: the `address` field must be a real UK postal address — a street name (and ideally a postcode). ' +
  'It must NOT be a property-type descriptor like "A three bedroom semi-detached house" or "Two bedroom flat" — those describe the property, they are not addresses. ' +
  'It must NOT be a banner, button label, or status text such as "Virtual Viewing", "Sold prior to auction", "National online auction bidding now open!", "Click to view lots", "Lot 47", or similar. ' +
  'If the card genuinely does not expose a real address (e.g. the address is hidden behind a "Click to view" button), return null for `address` rather than substituting a description, the property type, or any UI label.\n\n' +
  'IMAGE DISCIPLINE: the `image_url` field is a PRIORITY field — every visible property card has a thumbnail and you must extract it. ' +
  'Look in this order: the `<img>` tag\'s `src`, then its `data-src`, `data-lazy-src`, `data-original`, then `srcset` (use the largest variant if present). ' +
  'Lazy-loaded sites (Next.js, common CMSes) store the real URL on `data-src` while `src` holds a placeholder — DO NOT return the placeholder. ' +
  'PROXY UNWRAP: if the URL you find is a Next.js image-proxy wrapper of the form `<host>/_next/image?url=<inner>&w=…&q=…` (commonly with host `cdn.example.com`, but the same pattern applies to any host), do NOT return the wrapper. URL-decode the `url=` query parameter and return that inner value as image_url. ' +
  'Example: `https://cdn.example.com/_next/image?url=https%3A%2F%2Freal.cdn%2Fimg.jpg&w=3840&q=75` → return `https://real.cdn/img.jpg`. ' +
  'Reject and return null ONLY for: `data:image/...;base64,...` placeholders, tracking pixels (e.g. 1x1 gifs), logo/icon thumbnails, and "no image available" stock icons. ' +
  'Do NOT return null just because the card uses lazy-loading — check the lazy attributes first. ' +
  'If you cannot extract image_url for a card whose siblings DO have image_url, the listing\'s thumbnail is hidden in a non-standard attribute — search wider before giving up.';
