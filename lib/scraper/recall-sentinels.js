...
// EIG OAS lot ids are EITHER numeric (paulfosh /lot/details/186986) OR a UUID
// (tcpa /lot/details/742e9488-b032-…). The old numeric-only sentinel matched
// NOTHING on the UUID houses, so their recall silently read 0 (tcpa: ~2,714 live
// lots, sentinel 0). This form matches both id shapes. `.source` is spread into
// per-house literals below (each its own instance — a shared /g regex's lastIndex
// would leak across houses).
export const EIG_SENTINEL_SRC = '\\/lot\\/(?:details|redirect)\\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|\\d+)';
...
export const RECALL_SENTINELS = {
  ...
  paulfosh: new RegExp(EIG_SENTINEL_SRC, 'g'),
  harmanhealy: new RegExp(EIG_SENTINEL_SRC, 'g'),
  tcpa: new RegExp(EIG_SENTINEL_SRC, 'g'),
  firstforauctions: new RegExp(EIG_SENTINEL_SRC, 'g'),
  purplebricksgoto: new RegExp(EIG_SENTINEL_SRC, 'g'),
  ...
  seelauctions:             new RegExp(EIG_SENTINEL_SRC, 'g'), // online.seelauctions.co.uk
  sheldonbosley:            new RegExp(EIG_SENTINEL_SRC, 'g'), // online.sbkauctions.co.uk
  benjaminstevens:          new RegExp(EIG_SENTINEL_SRC, 'g'), // online.benjaminstevensauctions.co.uk
  hmox:                     new RegExp(EIG_SENTINEL_SRC, 'g'), // auctions.hmox.co.uk
  henrysykes:               new RegExp(EIG_SENTINEL_SRC, 'g'), // onlineauctions.henrysykes.co.uk
  sarahmains:               new RegExp(EIG_SENTINEL_SRC, 'g'), // www.auctionworks.co.uk
  cotswoldpropertyauctions: new RegExp(EIG_SENTINEL_SRC, 'g'), // cotswoldpropertyauctions.co.uk — pattern inferred (no lots in DB yet)
  ...
  // Savills — /auctions/{date-id}/{address-slug}-{lot-id}
  // The lookahead must accept the DELIMITERS a lot URL actually sits behind:
  // markdown's `](…)`, HTML's `href="…"` / `'…'`, and whitespace. The old
  // `(?=$|[/?#])` form matched NOTHING in either markdown or HTML, so savills
  // recall silently read 0 against a 287-lot catalogue and the house's total
  // blackout was never flagged. Verified 287/287 on the live 28–29 July 2026
  // catalogue, in both markdown and raw HTML (2026-07-21).
  savills: /\/auctions\/[\w-]+\/[\w-]+-(\d{4,6})(?=$|[/?#)\s"'\]])/gi,
  ...
  // BTG Eddisons (formerly 'sdl' slug, formerly included 'network') —
  // /properties/{lot-id-with-suffix}/for-auction-{location}
  // 'network' slug folded into btgeddisons 2026-06-21 (network is retired).
  btgeddisons: /btgeddisonspropertyauctions\.com\/properties\/([\w-]+?)(?:-\d+)?\/for-auction/gi,

  // Charles Darrow — independent Devon/Cornwall house, de-conflated from the
  // overloaded 'sdl' slug 2026-06-21. Own ASP.NET site; lots are
  // /propertyInfo/{numericId}/for-sale/{type-slug}/{location} (also mirrored on
  // the HOUSE_RECOGNISERS entry's recallSentinelPattern).
  charlesdarrow: /charlesdarrow\.co\.uk\/propertyInfo\/(\d+)/gi,

  // SDL Property Auctions — major UK auctioneer carved out of the overloaded
  // 'sdl' slug (de-conflation plan 4, onboarded 2026-06-22). Own catalogue at
  // sdlauctions.co.uk; lots are /property/{numericId}/{type}-for-auction-{town}/
  // (also mirrored on the HOUSE_RECOGNISERS.sdlauctions recallSentinelPattern).
  sdlauctions: /sdlauctions\.co\.uk\/property\/(\d+)/gi,
  ...
  // Lot pages are /property/{18-char Salesforce id} — verified 2026-07-21 by
  // clicking a card in a live browser render (the cards are router-push
  // handlers, not anchors, so this can't be read off the static HTML). The old
  // /for-auction/{slug} pattern matched the CATALOGUE url itself
  // (/for-auction/properties) and nothing else, so recall always read ~1.
  underthehammer:            /underthehammer\.com\/property\/([A-Za-z0-9]{15,})(?:[/?#]|$)/gi,
  // CF-stealth two-tier scraper; lot URLs are /property/{id}/{postcode}/{town}/{slug}.
  // Capture the id (first segment) — it is unique per lot; the postcode is not.
  symondsandsampson:         /auctions\.symondsandsampson\.co\.uk\/property\/([a-z0-9]+)\/[a-z0-9]+\/[a-z0-9-]+/gi,
  ...
  // wrightmarshall removed 2026-07-04 — retired (estate agent; lots via iamsold)
  davidjames:                 /iamsold\.co\.uk\/property\/([a-f0-9]{32})/gi,

  // (sdl and scargillmann entries removed 2026-06-21: sdl renamed to btgeddisons above;
  //  scargillmann retired — was mis-wired to sdlauctions.co.uk)
  ...
  mchughandco:               new RegExp(EIG_SENTINEL_SRC, 'g'),
  ...
  hammertime:                new RegExp(EIG_SENTINEL_SRC, 'g'),
  ...
  // webbers removed 2026-07-10: its lots live on webbers.bambooauctions.com, so the
  // .co.uk pattern matched NOTHING and recall silently read 0. It now resolves via
  // detectPlatformSentinel() → the Bamboo /property/{slug-id} pattern.
  ...
  // Slug-only paths:
  // Loveitts (Auction House Coventry & Warwickshire on loveitts.co.uk) —
  // lot pages are /auctions/{address-slug} (sometimes trailing -N). Exclude the
  // catalogue root and /auctions/upcoming-auctions calendar page.
  // Verified against live lots 2026-07-28 (e.g. …/auctions/52-abbey-court-…).
  loveitts:             /loveitts\.co\.uk\/auctions\/(?!upcoming-auctions(?:\/|$))([a-z0-9-]{12,})(?=$|[/?#)\s"'\]])/gi,
  ...
  // hawkesford / howkinsandharrison / fisherGerman removed 2026-07-10: all three run
  // their auctions on Bamboo subdomains (HOUSE_ROOTS retargeted), so they now resolve
  // via detectPlatformSentinel() → the shared Bamboo pattern. The old .co.uk patterns
  // matched nothing on the real catalogue and made recall read 0.
...
export function detectPlatformSentinel(slug) {
  ...
  // EIG platform: /lot/details/{id} or /lot/redirect/{id} — id numeric OR uuid.
  if (rootUrl.includes('eigonlineauctions.com') ||
      rootUrl.includes('eigpropertyauctions.co.uk')) {
    return new RegExp(EIG_SENTINEL_SRC, 'g');
  }
...
// is neither resolvable nor listed here. Add `slug: 'reason'` when a house
// genuinely has no lot URL to count (e.g. retired, JSON-API, or PDF-only).
export const KNOWN_SENTINEL_GAPS = {
  // Retired 2026-06-21: was mis-wired to sdlauctions.co.uk (SDL Auctions is its own
  // house, onboarded in plan 4). 0 real lots ever persisted. HOUSE_ROOTS entry
  // kept for historical display name; active scraping skipped via RETIRED_HOUSES.
  scargillmann: 'retired 2026-06-21 — mis-wired to sdlauctions.co.uk; no real lots',
};
