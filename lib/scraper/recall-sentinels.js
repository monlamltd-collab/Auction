...
// Houses with NO recall sentinel, each with a reason. This list is the
// pressure valve for the "every house has a sentinel" rule — the coverage
// test fails if a house is neither resolvable nor documented here, so a new
// house can't silently ship as a recall blind spot.
//   structured-api : lots come from a JSON API, not a rendered page — recall
//                    is the API's pagination, not a markup regex.
//   federated      : lots scatter across many third-party domains; no single
//                    URL shape exists. Needs per-domain entries when sampled.
//   pdf            : catalogue is a PDF; no lot URLs to count.
export const KNOWN_SENTINEL_GAPS = {
  // (allsop has an explicit pattern above for its rendered pages; its API
  // path doesn't use it, which is fine — the entry costs nothing.)
};
