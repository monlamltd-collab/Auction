...
import { scrapeWithFirecrawl, isFirecrawlCfBypassOnly } from './firecrawl.js';
...
export async function scrapeRenderedPage(url, house, options = {}) {
  // Tier 1: Firecrawl (if available and not skipped/exhausted). Skipped entirely
  // in CF-bypass-only mode — this render path never sets proxy:'stealth', so FC
  // would only throw FC_CF_BYPASS_ONLY and fall through. Going straight to
  // Puppeteer avoids the wasted call + a misleading "Firecrawl failed" log line.
  if (getBudget().canUseFirecrawl() && !getBudget().isSkipped(house) && !isFirecrawlCfBypassOnly()) {
      try {
