...
async function _doAutoAnalyseAll() {
  ...
  console.log(`AUTO: Queued ${houses.length} houses (${skippedByManager.length} skipped by manager, ${neverScrapedCount} never-scraped boosted)`);
...
async function autoAnalyseOne(url) {
  ...
  const RECALL_SENTINELS = {
    ...
    mchughandco:               /mchughandco\.com\/auction-(?:property|lot)\/([a-z0-9-]+)/gi,
