...
import { isFirecrawlExtractEnabled, extractCatalogueNative, extractPaginatedCatalogue, extractCatalogueWithBackfill } from './pipeline/firecrawl-extract.js';
import {
...
async function autoAnalyseOne(url) {
  ...
  // ── Detail-page backfill for SPAs with low JSON recall ──
  // Listing-page JSON typically misses ~50% of lots on dense SPAs (e.g. Pattinson).
  // For these houses we parse markdown for detail URLs as ground truth, then call
  // /v2/scrape with DETAIL_SCHEMA on each missed ID. No bespoke markdown parsers.
  const BACKFILL_HOUSES = {
    pattinson: {
      maxPages: 84,
      ...
      // returns "server closed abruptly" after ~60s. Without changeTracking it
      // returns full content in <1s. We accept the cost of always re-scraping.
  ...
  if (isFirecrawlExtractEnabled() && BACKFILL_HOUSES[house]) {
    try {
      const tpResult = await extractCatalogueWithBackfill(scrapeUrl, house, BACKFILL_HOUSES[house]);
      if (tpResult.lots.length > 0) {
        console.log(`AUTO: ${house} — Firecrawl backfill: ${tpResult.lots.length} lots (${tpResult.backfillCount || 0} from detail pages)`);
        const enrichDeps = {
          analyseLot,
      ...
      console.log(`AUTO: ${house} — backfill returned 0 lots, falling back to legacy`);
    } catch (tpErr) {
      console.log(`AUTO: ${house} — backfill error (${tpErr.message}), falling back to legacy`);
    }
  }
  ...
  if (isFirecrawlExtractEnabled() && rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const fcResult = isPaginated
        ? await extractPaginatedCatalogue(scrapeUrl, house, { paginateAs: rewritten.paginateAs, forceExtract })
        : await extractCatalogueNative(scrapeUrl, house, { forceExtract });
      if (fcResult.skipped) {
