...
import { HOUSE_RECOGNISERS } from './scraper/house-recognisers.js';
...
async function autoAnalyseOne(url, opts = {}) {
  ...
  // Free page-1 change gate: render page 1 with Crawlee, hash it, and if it
  // matches the stored hash skip the (token-costing) extract entirely.
  const crawleePage1Unchanged = async () => {
    if (forceFresh) return false;
    try {
      const probePage = await scrapeWithCrawlee(scrapeUrl);
      if (probePage?.html) {
        const h = createHash('md5').update(probePage.html).digest('hex');
        const prior = await getCataloguePage1Hash(house);
        if (prior && prior === h) return true;
      }
    } catch (e) { console.log(`AUTO: ${house} crawlee page-1 probe failed (${e.message})`); }
    return false;
  };
  ...
  const foldCrawleeStats = (outcomeFields) => recordEngineOutcome(
    enginePatch?.engine_stats || engineSkill?.engine_stats || {},
    'crawlee',
    outcomeFields,
  );

  // Promoted / locked / zero-credit-failover: render + extract with Crawlee
  // (recogniser-aware), persist on success. Returns { done, outcome, enginePatch }:
  // done=true → run complete (caller returns); done=false → 0 lots, caller falls
  // through to the Firecrawl safety net.
  const runCrawleePrimary = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, reason = '' }) => {
    crawleeTried = true;
    if (await crawleePage1Unchanged()) {
      console.log(`AUTO: ${house} unchanged (crawlee page-1 hash) — skipping extract`);
      hashHitCount++;
      return { done: true, outcome: 'same', enginePatch: null };
    }
    const cr = await renderAndExtractWithCrawlee(scrapeUrl, house, { maxPages, recallSentinelPattern, recogniseFromMarkdown });
    ...
    if (cr.lots.length > 0) {
      ...
      return { done: true, outcome: 'changed', enginePatch: { engine_stats: foldCrawleeStats({ success: true, recall: cr.recall, credits: 0, at: now }) } };
    }
    console.log(`AUTO: ${house} — Crawlee returned 0 lots, falling back to Firecrawl`);
    return { done: false, outcome: null, enginePatch: { engine_stats: foldCrawleeStats({ success: false, recall: cr.recall, credits: 0, at: now }) } };
  };

  // Shadow: after Firecrawl persisted, run Crawlee as a challenger and let the
  // parity gate decide promotion. Returns an enginePatch (with preferred_engine
  // on promote) or null.
  const runCrawleeShadow = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, fcResult }) => {
    try {
      const cr = await renderAndExtractWithCrawlee(scrapeUrl, house, { maxPages, recallSentinelPattern, recogniseFromMarkdown });
      ...
      const stats = foldCrawleeStats({ success: cr.lots.length > 0, recall: cr.recall, credits: 0, at: new Date().toISOString() });
      console.log(`AUTO: ${house} — engine parity ${verdict.promote ? 'PASS → promoting to crawlee' : 'hold'} (${verdict.reason})`);
      fireAlert({
        type: 'engine_parity',
        severity: 'info',
        house,
        message: `Crawlee vs Firecrawl on ${house}: ${verdict.promote ? 'PROMOTE' : 'hold'} — fc ${fcResult.lots.length} lots/${pct(fcResult.recall)} q${verdict.incBatchQuality}, cr ${cr.lots.length} lots/${pct(cr.recall)} q${verdict.chBatchQuality}`,
        meta: { verdict },
      }).catch(() => {});
      return verdict.promote ? { engine_stats: stats, preferred_engine: 'crawlee' } : { engine_stats: stats };
  ...
  const isCrawleeCandidate = () => hasCrawlee() && isCrawleeEnabled(house)
    && !isPdfUrl(scrapeUrl) && rewritten.paginateAs !== 'allsop_api'
  ...
  if (rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      // Recogniser houses keep their registry sentinel even on this path
      // (RECALL_SENTINELS has no entries for pattinson/johnpye/markjenkinson).
      const recallSentinelPattern = HOUSE_OVERRIDES[house]?.recallSentinelPattern
        || RECALL_SENTINELS[house] || detectPlatformSentinel(house) || null;

      // ── Best-engine-first router (docs/ENGINE-ROUTER.md) ──
      const recogniser = HOUSE_OVERRIDES[house]?.recogniseFromMarkdown || null;
      const { engine: chosenEngine, reason: engineReason } = resolveEngine(recogniser);
      const crawleeMaxPages = HOUSE_OVERRIDES[house]?.maxPages || (isPaginated ? 25 : 1);
      ...
      if (chosenEngine === ENGINES.CRAWLEE && !crawleeTried) {
        ...
        const res = await runCrawleePrimary({ recogniseFromMarkdown: recogniser, recallSentinelPattern, maxPages: crawleeMaxPages, reason: engineReason });
        if (res.enginePatch) enginePatch = res.enginePatch;
        if (res.done) { outcome = res.outcome; return; }
      ...
      if (fcResult.lots.length > 0) {
        ...
        if (isCrawleeCandidate() && isShadowMode()) {
          const patch = await runCrawleeShadow({ recogniseFromMarkdown: recogniser, recallSentinelPattern, maxPages: crawleeMaxPages, fcResult });
          if (patch) enginePatch = patch;
        }
