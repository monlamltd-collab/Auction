...
async function autoAnalyseOne(url, opts = {}) {
  ...
  const runCrawleePrimary = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, paginateAs = null, reason = '' }) => {
    ...
    const withFp = (stats) => (fingerprint ? { ...stats, _fingerprint: fingerprint } : stats);
    ...
    if (cr.lots.length === 0 || (fcAvailable && lossy)) {
      ...
      if (cr.lots.length === 0 || fcAvailable) {
        return { done: false, outcome: null, enginePatch: { engine_stats: withFp(foldCrawleeStats({ success: cr.lots.length > 0, recall: cr.recall, credits: 0, at: now }, cr.recall)) } };
      }
    }
    ...
    return { done: true, outcome: 'changed', enginePatch: { engine_stats: withFp(foldCrawleeStats({ success: true, recall: cr.recall, credits: 0, at: now }, cr.recall)) } };
  };
  ...
  if (rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const recallSentinelPattern = resolveRecallSentinel(house, HOUSE_OVERRIDES[house]?.recallSentinelPattern);
