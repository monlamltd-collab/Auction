...
// ── Bespoke-scraper houses ──
// Houses served by a self-contained scraper in scrape-stage.js rather than the
// shared render→recognise→extract path (`rewritten.paginateAs` is the switch).
// They must skip Crawlee, the Firecrawl JSON extract, and the paginated-URL
// machinery — their scraper owns pagination and emits normalised lots directly.
// ONE list so a new bespoke house can't be added to some guards and missed by
// others (three separate string comparisons used to be kept in sync by hand).
const BESPOKE_SCRAPER_PAGINATE_AS = new Set([
  'allsop_api',                 // lib/scraper/allsop.js
  'symondsandsampson_stealth',  // lib/scraper/symondsandsampson.js
  'underthehammer_api',         // lib/scraper/underthehammer.js
  'pattinson_api',              // lib/scraper/pattinson.js
]);
const usesBespokeScraper = (paginateAs) => BESPOKE_SCRAPER_PAGINATE_AS.has(paginateAs);
...
/**
 * Pick which catalogue-ready auctions to scrape per house, within the
 * MAX_AUCTIONS_PER_HOUSE lookahead budget.
 *
 * `always_on` rows are the house's EVERGREEN catalogue — the URL that always
 * holds the current lots — not a dated sale. Sorting purely by date parks their
 * 2099-12-31 sentinel LAST, so two stale dated rows can starve the only
 * lot-bearing URL out of the queue entirely.
 *
 * suttonkersh 2026-07-22: a dated 404 (`/auctions/current-auction-lots`) and a
 * dated 0-lot hub page (`/auctions-property`) outranked the always_on gallery
 * that carries all 16 live lots. The house went dark — health 0, circuit open,
 * last_success NULL — while its extractor was perfectly healthy. The auction
 * watcher re-dates those junk rows forward each sale, so the starvation was
 * self-renewing. Also hit countrywide (and retired sdl).
 *
 * Budget is unchanged: always_on rows are taken FIRST, then the nearest dated
 * rows fill any remaining slots. No extra scrapes.
 *
 * @param {Record<string, Array<{date?: string, status?: string, url?: string}>>} byHouse
 * @param {number} max
 * @returns {{ready: Array<object>, skipped: Array<{house: string, auction: object}>}}
 */
export function selectAuctionsPerHouse(byHouse, max) {
  const ready = [];
  const skipped = [];
  for (const [house, auctions] of Object.entries(byHouse)) {
    const alwaysOn = auctions.filter(a => a.status === 'always_on');
    const dated = auctions
      .filter(a => a.status !== 'always_on')
      .sort((a, b) => new Date(a.date || 0) - new Date(b.date || 0));
    const keep = [...alwaysOn, ...dated.slice(0, Math.max(0, max - alwaysOn.length))];
    const keptSet = new Set(keep);
    ready.push(...keep);
    for (const a of auctions) if (!keptSet.has(a)) skipped.push({ house, auction: a });
  }
...
async function _doAutoAnalyseAll() {
  ...
  // ── Step 0.5: Reconcile existing calendar rows conservatively ──
  try {
    await syncCalendar({ supabase });
  } catch (e) {
    console.warn('AUTO-CALENDAR: reconciliation failed (non-fatal) —', e.message);
  }
  ...
  // (selection helper below — exported + unit-tested in tests/test-calendar-selection.js)
  ...
  const { ready, skipped: skippedLookahead } = selectAuctionsPerHouse(byHouse, MAX_AUCTIONS_PER_HOUSE);
  for (const { house: h, auction: skippedA } of skippedLookahead) {
  ...
  // AI-OUTAGE GATE: skip the sweep entirely while the AI stack is quota-dead.
  // During an outage EVERY AI-dependent house reports a 0-lot regression, so the
  // sweep would try to "heal" dozens of houses whose catalogue URLs are perfectly
  // fine — burning cycles and, before the #196 verification guard, corrupting
  // good URLs with junk. The URL is not the problem when the extractor is down.
  if (_deps.FIRECRAWL_API_KEY && !_deps.isFcCreditExhausted() && !getCreditExhausted()) {
    try {
...
// ══════════════════════════════════════════════════════════════════
async function healBrokenHouse(slug, oldUrl, opts = {}) {
  // opts.force=true (admin heal) bypasses AUTO_HEAL_ENABLED=false kill switch.
...
// ══════════════════════════════════════════════════════════════════
async function discoverAndUpdateCalendar(opts = {}) {
  return _discoverAndUpdateCalendarImpl({
    ...
    force: !!(opts && opts.force),
    watcherHandledSlugs: opts?.watcherHandledSlugs,
...
async function autoAnalyseOne(url, opts = {}) {
  ...
  // Dormant houses (between-auctions / retired) legitimately return only ended
  // lots, so their per-scrape breakage alerts (recall gap, zero-lots) are false
  // alarms. Read the operator/retired-house flag once and downgrade those alerts
  // to info below. Best-effort: a read failure defaults to not-dormant (i.e. the
  // house keeps full alerting), never suppressing a real breakage.
  let isDormant = false;
  try {
    const { data: _dormantRow } = await supabase.from('house_skills').select('dormant').eq('slug', house).maybeSingle();
    isDormant = !!_dormantRow?.dormant;
  } catch { /* non-fatal — default to not-dormant so alerting is never wrongly silenced */ }
  ...
  const runCrawleePrimary = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, paginateAs = null, reason = '' }) => {
    ...
    // ── THE 100% COMMANDMENT — recall gate ──
    // Every available lot must be captured. recall < sentinel parity means lots
    // were LEFT BEHIND — a coverage BUG surfaced loudly + queryably in
    // pipeline_alerts (type='recall_below_100'), not tolerated as "good enough".
    // Was: a soft 'warning' below an 85% floor, so 85–99% passed silently — the
    // exact drift from the commandment. (review F13; 100%-gate 2026-07-07.)
    if (cr.recall != null) {
      fireAlert(recallGateAlert({
        house, recall: cr.recall, lots: cr.lots.length,
        sentinelLots: cr.sentinelLots, reason, engine: 'crawlee', recognised: cr.recognised,
        dormant: isDormant,
      })).catch(() => {});
    }
  ...
  const isCrawleeCandidate = () => hasCrawlee() && isCrawleeEnabled(house)
    && !isPdfUrl(scrapeUrl) && !usesBespokeScraper(rewritten.paginateAs)
  ...
  // A bespoke-scraper house must never enter this block: its scraper owns
  // pagination and emits normalised lots directly, so a Crawlee render+extract
  // here would be a duplicate scrape with strictly worse recall. (Pattinson was
  // the case that made this concrete — it held both an override entry AND a
  // bespoke scraper; the guard is house-agnostic so the next one is safe too.)
  if (HOUSE_OVERRIDES[house] && !usesBespokeScraper(rewritten.paginateAs)) {
    try {
  ...
  if (!usesBespokeScraper(rewritten.paginateAs) && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const isPaginated = rewritten.paginateAs && !usesBespokeScraper(rewritten.paginateAs);
      ...
      // ── EIG OAS paginated static catalogue (current-auction scoping) ──
      // The EIG /search page is the FULL ARCHIVE behind a signed token (raw
      // ?Order=/?auctionId= params are ignored), so we cannot ask the server for
      // "current only". Instead: paginate ?pagesize&page in the native (end-date
      // descending) order, run the recogniser — which returns ONLY live lots and
      // drops every ended/past-dated card — and STOP at the first page that has
      // cards but zero live lots (Type B: the live head is exhausted, the ended
      // archive begins) or an empty page (Type A: the pre-scoped current set
      // ends). This bounds the fetch to the live head (never the 47MB archive)
      // and the recogniser guarantees no ended lot is ever persisted as live.
      // CRITICAL: on zero live lots we return a GENUINE ZERO — we must NOT fall
      // through to the Gemini engine, which would re-extract the archive's ended
      // lots as status='available' (the exact leak this whole path prevents).
      if (rec?.staticCatalogue && recogniser && rec.paginateStatic) {
        try {
          const pagesize = rec.staticPageSize || 200;
          const maxPages = rec.maxPages || 40;
          const merged = new Map();
          let page1Html = null;
          for (let page = 1; page <= maxPages; page++) {
            // Force LIST view: the EIG OAS GRID view renders compact cards with NO
            // status/date discriminator, so the recogniser can't tell live from
            // ended and keeps everything (firstforauctions leaked 520 mostly-ended
            // lots as live in ?view=Grid; List view scopes it to 48 with a clean
            // boundary). Also drop ?showall, which forces the full archive.
            let pageUrl;
            try {
              const u = new URL(scrapeUrl);
              u.searchParams.delete('showall');
              u.searchParams.set('view', 'List');
              u.searchParams.set('pagesize', String(pagesize));
              u.searchParams.set('page', String(page));
              pageUrl = u.toString();
            } catch {
              pageUrl = `${scrapeUrl}${scrapeUrl.includes('?') ? '&' : '?'}view=List&pagesize=${pagesize}&page=${page}`;
            }
            let html;
            try { html = await (_deps.fetchPage || fetchPage)(pageUrl); }
            catch (e) { console.log(`AUTO: ${house} — EIG page ${page} fetch failed (${e.message})`); break; }
            if (page === 1) page1Html = html;
            const md = htmlToRecognitionMarkdown(html, pageUrl);
            const advertised = new Set([...md.matchAll(/\/lot\/details\/([A-Za-z0-9][A-Za-z0-9-]{2,})/g)].map(m => m[1])).size;
            const map = recogniser(md) || new Map();
            let added = 0;
            for (const [id, lot] of map) { if (!merged.has(id)) { merged.set(id, lot); added++; } }
            console.log(`AUTO: ${house} — EIG page ${page}: ${advertised} advertised, ${map.size} live, +${added} new (total ${merged.size})`);
            if (advertised === 0) break;   // past the last catalogue page
            if (map.size === 0) break;      // cards present but 0 live → into the ended archive
            if (added === 0) break;         // no new live ids → dedup saturation / loop guard
          }
          if (!forceFresh && page1Html) {
            const prior = await getCataloguePage1Hash(house).catch(() => null);
            if (prior && prior === createHash('md5').update(page1Html).digest('hex')) {
              console.log(`AUTO: ${house} unchanged (static page-1 hash) — skipping`);
              hashHitCount++; outcome = 'same'; return;
            }
          }
          const rawLots = [...merged.values()]
            .map(l => normaliseScrapedLot(l, { house, catalogueUrl: scrapeUrl, extractionSource: 'eig-static-recognition' }))
            .filter(Boolean);
          if (rawLots.length > 0) {
            console.log(`AUTO: ${house} → EIG static catalogue: ${rawLots.length} live lots`);
            setLastScrapeEngine('http');
            setLastExtractorUsed('recogniser');
            await enrichAndPersistLots(rawLots);
            try { if (page1Html) await setCataloguePage1Hash(house, createHash('md5').update(page1Html).digest('hex')); } catch { /* best-effort */ }
            outcome = 'changed';
            return;
          }
          // Zero live lots = house is between auctions (paulfosh-style) OR the
          // recogniser found only ended cards. Genuine zero — return, do NOT fall
          // through to Gemini (which would ship the ended archive as live).
          console.log(`AUTO: ${house} — EIG static recognition 0 live lots (dormant/between auctions) — genuine zero`);
          outcome = 'same';
          return;
        } catch (e) {
          console.log(`AUTO: ${house} — EIG static pagination error (${e.message}) — genuine-zero guard, not falling through`);
          // Even on error we do NOT fall through to Gemini for EIG houses: a
          // partial/failed fetch must never become an ended-lot leak. Degrade to
          // "no change" and let the next cycle retry.
          outcome = 'error';
          return;
        }
      }

      // ── Generic offset/page paginated static catalogue ──────────────────────
      // For SSR houses whose catalogue is split across fixed-size pages addressed by
      // a single query param (futureauctions: `?offset=N` in steps of 21 across 749
      // entries). Deliberately SEPARATE from the EIG block above: EIG needs the
      // live↔ended boundary stop and a genuine-zero guard, this one just walks the
      // catalogue to exhaustion. Stops on an empty page or when a page yields no NEW
      // lot ids (loop guard), and falls through to the engine router on 0 lots so a
      // house degrades rather than breaking.
      //   rec.staticPaginate = { param: 'offset', step: 21 }   // or { param:'page', step:1, from:1 }
      if (rec?.staticCatalogue && recogniser && rec.staticPaginate?.param) {
        try {
          const { param, step = 1, from = 0 } = rec.staticPaginate;
          const maxPages = rec.maxPages || 40;
          const merged = new Map();
          let page1Html = null;
          for (let p = 0; p < maxPages; p++) {
            const value = from + p * step;
            let pageUrl;
            try { const u = new URL(scrapeUrl); u.searchParams.set(param, String(value)); pageUrl = u.toString(); }
            catch { pageUrl = `${scrapeUrl}${scrapeUrl.includes('?') ? '&' : '?'}${param}=${value}`; }
            let html;
            try { html = await (_deps.fetchPage || fetchPage)(pageUrl); }
            catch (e) { console.log(`AUTO: ${house} — static page ${value} fetch failed (${e.message})`); break; }
            if (p === 0) page1Html = html;
            const map = recogniser(htmlToRecognitionMarkdown(html, pageUrl)) || new Map();
            let added = 0;
            for (const [id, lot] of map) { if (!merged.has(id)) { merged.set(id, lot); added++; } }
            console.log(`AUTO: ${house} — static ${param}=${value}: ${map.size} lots, +${added} new (total ${merged.size})`);
            if (map.size === 0) break;   // ran off the end of the catalogue
            if (added === 0) break;      // no new ids → dedup saturation / loop guard
          }
          if (!forceFresh && page1Html) {
            const prior = await getCataloguePage1Hash(house).catch(() => null);
            if (prior && prior === createHash('md5').update(page1Html).digest('hex')) {
          ...
          const rawLots = [...merged.values()]
            ...
            .filter(Boolean);
          if (rawLots.length > 0) {
            console.log(`AUTO: ${house} → paginated static catalogue: ${rawLots.length} lots`);
            setLastScrapeEngine('http');
            setLastExtractorUsed('recogniser');
            await enrichAndPersistLots(rawLots);
            try { if (page1Html) await setCataloguePage1Hash(house, createHash('md5').update(page1Html).digest('hex')); } catch { /* best-effort */ }
          ...
          console.log(`AUTO: ${house} — paginated static recognition 0 lots, falling through to engine router`);
        } catch (e) {
          console.log(`AUTO: ${house} — paginated static catalogue failed (${e.message}), falling through`);
        }
      }

      if (rec?.staticCatalogue && recogniser) {
        try {
          // Two-tier houses (edwardmellor) resolve the real catalogue URL from a
          // stable landing page before fetching; single-tier houses fetch scrapeUrl
          // directly. A null resolve = no upcoming auction → genuine zero, not a bug.
          //
          // resolveCatalogueUrl may return ONE target or an ORDERED LIST of them.
          // A list is how a house whose sales run in parallel (savills lists four
          // upcoming catalogues, all carrying live lots) or whose catalogue
          // paginates by PATH segment reaches 100% — the pages are fetched in
          // order and merged first-wins by lot id.
          let targets = [{ url: scrapeUrl, auctionDateIso: '' }];
          if (typeof rec.resolveCatalogueUrl === 'function') {
            const resolved = await rec.resolveCatalogueUrl(scrapeUrl, (_deps.fetchPage || fetchPage));
            const list = (Array.isArray(resolved) ? resolved : [resolved]).filter(Boolean);
            if (!list.length) {
              console.log(`AUTO: ${house} — no upcoming auction on landing page (genuine zero)`);
              outcome = 'same'; return;
            }
            targets = list.map(r => (typeof r === 'string'
              ? { url: r, auctionDateIso: '' }
              : { url: r.url, auctionDateIso: r.auctionDateIso || '' }));
          }
          const targetUrl = targets[0].url;
          const html = await (_deps.fetchPage || fetchPage)(targetUrl);
          // Change detection stays on page 1 — but only short-circuits a
          // SINGLE-target house. On a multi-target house an unchanged first page
          // says nothing about the later sales/pages, and skipping there would
          // freeze them out of the UI.
          if (!forceFresh && targets.length === 1) {
            const prior = await getCataloguePage1Hash(house).catch(() => null);
          ...
          for (let ti = 0; ti < targets.length; ti++) {
            ...
            let pageHtml = html;
            if (ti > 0) {
              try { pageHtml = await (_deps.fetchPage || fetchPage)(t.url); }
              catch (e) { console.log(`AUTO: ${house} — static target ${t.url} fetch failed (${e.message})`); continue; }
            }
            const pageMap = recogniser(htmlToRecognitionMarkdown(pageHtml, t.url)) || new Map();
            let added = 0;
            for (const [id, l] of pageMap) {
              if (merged.has(id)) continue;
              merged.set(id, t.auctionDateIso ? { ...l, auction_date: l.auction_date || t.auctionDateIso } : l);
              added++;
            }
            if (targets.length > 1) console.log(`AUTO: ${house} — static ${t.url}: ${pageMap.size} lots, +${added} new (total ${merged.size})`);
          }
          const rawLots = [...merged.values()]
            .map(l => normaliseScrapedLot(l, { house, catalogueUrl: targetUrl, extractionSource: 'static-recognition' }))
            .filter(Boolean);
          if (rawLots.length > 0) {
  ...
  const scrapeDeps = {
    ...
    scrapeUnderTheHammer: _deps.scrapeUnderTheHammer,
    scrapePattinson: _deps.scrapePattinson,
    ...
    if (!healResult) {
      ...
      try {
        await supabase.from('pipeline_alerts').insert({
          ...
          // Dormant houses return 0 lots by definition (between auctions) — keep
          // the record for observability but as info, not a breakage warning.
          severity: isDormant ? 'info' : 'warning',
          house,
          message: `${HOUSE_DISPLAY_NAMES[house] || house} returned 0 lots and no replacement URL was found. The catalogue URL may be stale, the extractor broken, or the page may genuinely be empty.${isDormant ? ' [dormant]' : ''}`,
        });
      } catch { /* non-fatal */ }
    }
    outcome = 'error';
    ...
    if (outcome === 'error' && house && !healedSuccessfully) {
      try {
        if (_deps.harnessUpdateHealth) {
          // aiUnavailable: when the AI stack is quota-dead an AI-dependent house
          // CANNOT extract, so its zero indicts our extractor, not the house.
          // house-health.js treats that as a non-event instead of a consecutive
          // failure — otherwise one outage trips the whole fleet's circuits.
          _deps.harnessUpdateHealth(house, {
            lots: { lots: [], batchQuality: 0 },
            extractionMethod: 'none',
            aiUnavailable: getCreditExhausted(),
          });
        }
