      ...
      if (rec?.staticCatalogue && recogniser && rec.paginateStatic) {
        try {
          ...
          for (let page = 1; page <= maxPages; page++) {
            let pageUrl;
            try { const u = new URL(scrapeUrl); u.searchParams.set('pagesize', String(pagesize)); u.searchParams.set('page', String(page)); pageUrl = u.toString(); }
            catch { pageUrl = `${scrapeUrl}${scrapeUrl.includes('?') ? '&' : '?'}pagesize=${pagesize}&page=${page}`; }
            let html;
