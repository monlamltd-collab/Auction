...
async function _doAutoAnalyseAll() {
  ...
  if (houseSlugs.length > 0) {
    const { data: skillRows } = await supabase
      .from('house_skills')
      .select('slug, next_scrape_at, preferred_engine, engine_locked, engine_stats, dormant')
      .in('slug', houseSlugs);
...
async function autoAnalyseOne(url, opts = {}) {
  ...
  // Dormant houses (known between-auctions/defunct, catalogue all-terminal) get
  // their breakage alerts downgraded to info and skip the liveness health-degrade —
  // a 0/all-terminal result is expected, not a failure. Flag defaults false, so
  // only explicitly-flagged houses (e.g. defunct EIG accounts) are affected; every
  // other house behaves exactly as before. Auto-clears on the next live lot (persist-stage).
  const isDormant = engineSkill?.dormant === true;
  ...
  const runCrawleePrimary = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, paginateAs = null, reason = '' }) => {
    ...
    if (cr.recall != null) {
      // Dormant houses under-extract their all-ended catalogues harmlessly, so a
      // low-recall reading there isn't actionable — keep it as info, not a warning.
      const lowRecall = cr.recall < CRAWLEE_RECALL_FLOOR;
      fireAlert({
        type: 'recall_diagnostic',
        severity: (lowRecall && !isDormant) ? 'warning' : 'info',
        house,
        message: `Crawlee recall ${pct(cr.recall)}: ${cr.lots.length}/${cr.sentinelLots} (${reason})${isDormant ? ' [dormant]' : ''}`,
        meta: { engine: 'crawlee', recall: cr.recall, lots: cr.lots.length, sentinelLots: cr.sentinelLots, recognised: cr.recognised, reason, dormant: isDormant },
      }).catch(() => {});
    }
    ...
    if (!healResult) {
      ...
      try {
        await supabase.from('pipeline_alerts').insert({
          ...
          // Dormant houses (known between-auctions/defunct) legitimately return
          // 0 lots — record it as info, not a warning, so it stops paging.
          severity: isDormant ? 'info' : 'warning',
          house,
          message: isDormant
            ? `${HOUSE_DISPLAY_NAMES[house] || house} returned 0 lots — house flagged dormant (between auctions / no live catalogue). Expected; auto-clears when a live lot reappears.`
            : `${HOUSE_DISPLAY_NAMES[house] || house} returned 0 lots and no replacement URL was found. The catalogue URL may be stale, the extractor broken, or the page may genuinely be empty.`,
        });
      } catch { /* non-fatal */ }
    }
    outcome = 'error';
  ...
  outcome = 'changed';
    ...
    if (outcome === 'error' && house && !healedSuccessfully) {
      try {
        // Dormant houses legitimately extract nothing (no live auction) — don't
        // degrade their health for it, or they'd churn toward 'broken' forever.
        // Still stamp last_extracted_count=0 below so the per-run signal is honest.
        if (_deps.harnessUpdateHealth && !isDormant) {
