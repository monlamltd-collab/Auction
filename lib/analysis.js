...
async function autoAnalyseOne(url, opts = {}) {
  ...
  if (rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const hasMarkdownRecogniser = !!HOUSE_OVERRIDES[house]?.recogniseFromMarkdown;
      const { engine: chosenEngine, reason: engineReason } = resolveEngineForHouse({
        house, rewritten, catalogueUrl: scrapeUrl, engineSkill, hasMarkdownRecogniser,
      });
      const crawleeMaxPages = HOUSE_OVERRIDES[house]?.maxPages || (isPaginated ? 25 : 1);
      // A house is a Crawlee migration candidate when config + structure allow it
      // but its learned policy hasn't promoted it yet — these get shadow-evaluated.
      const crawleeCandidate = hasCrawlee() && isCrawleeEnabled(house)
        && !hasMarkdownRecogniser && !isPdfUrl(scrapeUrl) && rewritten.paginateAs !== 'allsop_api';
      const pct = (r) => (r == null ? 'n/a' : `${(r * 100).toFixed(0)}%`);

      // ── Promoted/locked Crawlee: render + Gemini only, no Firecrawl spend ──
      if (chosenEngine === ENGINES.CRAWLEE) {
        console.log(`AUTO: ${house} → engine crawlee (${engineReason})`);
        // Free page-1 change gate — skip the Gemini extract when nothing changed.
        if (!forceFresh) {
          try {
          ...
          } catch (probeErr) { console.log(`AUTO: ${house} crawlee page-1 probe failed (${probeErr.message})`); }
        }
        const cr = await renderAndExtractWithCrawlee(scrapeUrl, house, { maxPages: crawleeMaxPages, recallSentinelPattern });
        const now = new Date().toISOString();
        if (cr.lots.length > 0) {
          console.log(`AUTO: ${house} — Crawlee+Gemini: ${cr.lots.length} lots (recall ${pct(cr.recall)})`);
          ...
          enginePatch = { engine_stats: recordEngineOutcome(engineSkill?.engine_stats || {}, 'crawlee', { success: true, recall: cr.recall, credits: 0, at: now }) };
          outcome = 'changed';
          return;
        }
        // 0 lots from Crawlee — record the miss and fall through to Firecrawl safety net.
        console.log(`AUTO: ${house} — Crawlee returned 0 lots, falling back to Firecrawl`);
        enginePatch = { engine_stats: recordEngineOutcome(engineSkill?.engine_stats || {}, 'crawlee', { success: false, recall: cr.recall, credits: 0, at: now }) };
      }
      ...
      if (fcResult.lots.length > 0) {
        ...
        // ── Shadow comparison: is Crawlee ready to take over this house? ──
        // Investors already have the Firecrawl result (persisted above). We now
        // run Crawlee+Gemini as a challenger and let the product-integrity gate
        // decide whether to promote — strict recall parity AND no field-quality
        // loss. Only candidates not yet promoted, and only in shadow mode.
        if (crawleeCandidate && isShadowMode() && engineSkill?.preferred_engine !== 'crawlee') {
          try {
            const cr = await renderAndExtractWithCrawlee(scrapeUrl, house, { maxPages: crawleeMaxPages, recallSentinelPattern });
            ...
            enginePatch = verdict.promote ? { engine_stats: stats, preferred_engine: 'crawlee' } : { engine_stats: stats };
            ...
            console.log(`AUTO: ${house} — shadow compare failed: ${shadowErr.message}`);
          }
        }
