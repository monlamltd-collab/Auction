...
import { getExtractionTier, recordExtractionRecall } from './scraper/extraction-tier.js';
...
async function autoAnalyseOne(url, opts = {}) {
  ...
  const foldCrawleeStats = (outcomeFields, extractionRecall = undefined) => {
  ...
  const runCrawleePrimary = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, paginateAs = null, reason = '' }) => {
    ...
    if (cr.lots.length === 0 || (fcAvailable && lossy)) {
      ...
      if (cr.lots.length === 0 || fcAvailable) {
        return { done: false, outcome: null, enginePatch: { engine_stats: foldCrawleeStats({ success: cr.lots.length > 0, recall: cr.recall, credits: 0, at: now }, cr.recall) } };
      }
    }
    ...
    return { done: true, outcome: 'changed', enginePatch: { engine_stats: foldCrawleeStats({ success: true, recall: cr.recall, credits: 0, at: now }, cr.recall) } };
  };
