...
async function autoAnalyseOne(url, opts = {}) {
  ...
  // in Firecrawl markdown vs how many made it into JSON — exposes extraction
  // quality regressions. Houses without a sentinel still scrape; they just
  // don't emit the inline recall metric (snapshot-based recall covers all
  // houses structurally via /api/admin/recall instead).
  //
  // Resolution order:
  //   1. Per-house override in HOUSE_OVERRIDES (pattinson, johnpye)
  //   2. Auto-detection from HOUSE_ROOTS URL via detectPlatformSentinel() below
  //   3. null (no sentinel)
  //
  // Bespoke per-house RECALL_SENTINELS retired 2026-05-22 — superseded by
  // catalogue_snapshots-based recall (lib/pipeline/recall.js + /api/admin/recall).
  ...
  if (rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const recallSentinelPattern = detectPlatformSentinel(house) || null;
