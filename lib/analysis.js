...
import { isFirecrawlExtractEnabled, extractCatalogueNative, extractPaginatedCatalogue } from './pipeline/firecrawl-extract.js';
import {
...
async function autoAnalyseOne(url) {
  ...
  const FC_EXTRACT_SKIP = ['pattinson'];
  if (isFirecrawlExtractEnabled() && rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const fcResult = isPaginated
        ? await extractPaginatedCatalogue(scrapeUrl, house, { paginateAs: rewritten.paginateAs })
        : await extractCatalogueNative(scrapeUrl, house);
      if (fcResult.skipped) {
      ...
      // 0 lots from standard extraction — try agent mode before legacy fallback
      console.log(`AUTO: ${house} — Firecrawl extract returned 0 lots, trying agent mode`);
      try {
      ...
      console.log(`AUTO: ${house} — falling back to legacy pipeline`);
    } catch (fcErr) {
