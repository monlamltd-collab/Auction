...
async function autoAnalyseOne(url, opts = {}) {
  ...
  const crawleePage1Unchanged = async () => {
    ...
    try {
      const probe = await scrapeWithCrawlee(scrapeUrl);
      if (probe?.html) {
        const h = createHash('md5').update(probe.html).digest('hex');
  ...
  const runCrawleePrimary = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, reason = '' }) => {
    if (await crawleePage1Unchanged()) { hashHitCount++; return { done: true, outcome: 'same', enginePatch: null }; }
    ...
    if (cr.lots.length > 0) {
      ...
      return { done: true, outcome: 'changed', enginePatch: { engine_stats: recordEngineOutcome(engineSkill?.engine_stats || {}, 'crawlee', { success: true, recall: cr.recall, credits: 0, at: now }) } };
    }
    console.log(`AUTO: ${house} — Crawlee returned 0 lots, falling back to Firecrawl`);
    return { done: false, outcome: null, enginePatch: { engine_stats: recordEngineOutcome(engineSkill?.engine_stats || {}, 'crawlee', { success: false, recall: cr.recall, credits: 0, at: now }) } };
  };
  ...
  const runCrawleeShadow = async ({ recogniseFromMarkdown = null, recallSentinelPattern = null, maxPages = 1, fcResult }) => {
    try {
      ...
      const stats = recordEngineOutcome(engineSkill?.engine_stats || {}, 'crawlee', { success: cr.lots.length > 0, recall: cr.recall, credits: 0, at: new Date().toISOString() });
