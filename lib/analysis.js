...
// ── computeScrapeDiff — kept inline (30 lines, only consumer is persist-stage) ──
function computeScrapeDiff(oldLots, newLots) {
  ...
  return { lots_added: added.length, lots_removed: removed.length, lots_changed: changed.length, images_gained: imagesGained, images_lost: imagesLost, status_changes: summary, timestamp: new Date().toISOString() };
}
