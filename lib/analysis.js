...
import { resetBrokenExtractors } from './extractors.js';
...
import {
  ...
  probe, scrapeStage, enrichStage, persistStage,
...
// Helper to persist healing state to both in-memory map and DB
...
If you cannot find a catalogue URL, return: {"newUrl": null, "confidence": "none", "reason": "explanation"}`, {
    ...
    // Verify the new URL is reachable AND contains auction lots
    try {
      ...
      // Lot-count gate: try DOM extraction on the verified HTML
      if (_deps.extractWithJSDOM) {
        const testLots = _deps.extractWithJSDOM(verifyHtml, slug, newUrl);
