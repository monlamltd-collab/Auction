      ...
      if (rec?.staticCatalogue && recogniser) {
        try {
          ...
          if (typeof rec.resolveCatalogueUrl === 'function') {
            ...
            targetUrl = resolved.url || resolved;
            resolvedAuctionDate = resolved.auctionDateIso || '';
          }
          ...
          const md = htmlToRecognitionMarkdown(html, targetUrl);
          const map = recogniser(md);
          const rawLots = [...(map?.values?.() || [])]
            .map(l => normaliseScrapedLot(
              resolvedAuctionDate ? { ...l, auction_date: l.auction_date || resolvedAuctionDate } : l,
              { house, catalogueUrl: targetUrl, extractionSource: 'static-recognition' }))
            .filter(Boolean);
          if (rawLots.length > 0) {
