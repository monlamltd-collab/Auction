...
import { isFirecrawlExtractEnabled, extractCatalogueNative, extractPaginatedCatalogue, extractTwoPass } from './pipeline/firecrawl-extract.js';
import {
...
async function autoAnalyseOne(url) {
  ...
  // ── Two-pass extraction for SPAs with low JSON recall ──
  const TWO_PASS_HOUSES = {
    pattinson: {
      maxPages: 84,
  ...
  if (isFirecrawlExtractEnabled() && TWO_PASS_HOUSES[house]) {
    try {
      const tpResult = await extractTwoPass(scrapeUrl, house, TWO_PASS_HOUSES[house]);
      if (tpResult.lots.length > 0) {
        console.log(`AUTO: ${house} — two-pass extract: ${tpResult.lots.length} lots`);
        const enrichDeps = {
          analyseLot,
      ...
      console.log(`AUTO: ${house} — two-pass returned 0 lots, falling back to legacy`);
    } catch (tpErr) {
      console.log(`AUTO: ${house} — two-pass error (${tpErr.message}), falling back to legacy`);
    }
  }
  ...
  if (isFirecrawlExtractEnabled() && rewritten.paginateAs !== 'allsop_api' && !FC_EXTRACT_SKIP.includes(house)) {
    try {
      ...
      const AGENT_HOUSES = ['acuitus', 'foxandsons'];
      if (AGENT_HOUSES.includes(house)) {
        console.log(`AUTO: ${house} — 0 lots from extraction, trying agent mode`);
        try {
          const { extractWithAgent } = await import('./pipeline/firecrawl-extract.js');
          const agentResult = await extractWithAgent(scrapeUrl, house, { timeout: 300000 });
