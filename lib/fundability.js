...
// Works cost as fraction of price, by condition keyword (checked in order, first match wins)
const WORKS_COST_RULES = [
  { keywords: ['derelict'], pct: 0.30, label: 'derelict' },
  { keywords: ['poor'], pct: 0.25, label: 'poor' },
  { keywords: ['needs refurbishment', 'needs work'], pct: 0.15, label: 'refurbishment' },
  { keywords: ['needs modernisation'], pct: 0.12, label: 'modernisation' },
];
const DEFAULT_REFURB_WORKS_PCT = 0.10;
const GDV_REFURB_MULTIPLIER = 1.25;
...
export function _deriveDeal(lot) {
  ...
  const ltvPct = LTV_BY_TYPE[propertyType] ?? 70;
  ...
  if (isRefurb && price > 0) {
    ...
    deal.gdv = Math.round(price * GDV_REFURB_MULTIPLIER);
    deal.loan_term = DEFAULT_LOAN_TERM_MONTHS;
    provenance.works_cost_source = source;
    provenance.gdv_source = `proxy_${GDV_REFURB_MULTIPLIER}x`;
    provenance.loan_term_source = `default_${DEFAULT_LOAN_TERM_MONTHS}m`;
...
export async function getFundabilityBadge(lot) {
  ...
  // Cache on the full deal so derived fields (works_cost, gdv, loan_term, LTV)
  // don't collide across lots that differ in condition or property type
  const cacheKey = JSON.stringify(deal);
...
export async function enrichLotsWithFundability(lots) {
  ...
  for (const lot of lots) {
    ...
    const { deal, provenance } = _deriveDeal(lot);
    const inputsDerived = [
      provenance.gdv_source?.startsWith('proxy_') ? 'gdv' : null,
      provenance.works_cost_source && provenance.works_cost_source !== null ? 'works_cost' : null,
