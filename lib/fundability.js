...
// Target is /apply — the live lead-capture form that reads UTM params into
// the lead record. The old /check path 404s on www.bridgematch.co.uk
// (verified 2026-07-03): every deep link had been landing users on a 404.
export function buildBridgematchUrl(deal) {
  ...
  return `https://www.bridgematch.co.uk/apply?${params}`;
}

// ─── Per-lot attribution (Phase 3, 2026-07-03) ───
// bridgematchUrl is cached per DEAL SHAPE (two lots with the same price/type
// share one cached URL), so per-lot attribution must be appended at use time,
// never baked into the cached URL. utm_campaign carries `lot_<uuid>` because
// BridgeMatch already stores utm_campaign on every lead — attribution lands
// in its lead record with no schema change; lot_ref/click_id are ALSO sent
// explicitly for the dedicated columns on the BridgeMatch side.
// Touchpoints (utm_medium): lot_card | lot_page | digest_email.
export function withLotAttribution(url, { lotRef, medium, clickId } = {}) {
  if (!url) return url;
  try {
    const u = new URL(url);
    if (lotRef) {
      u.searchParams.set('utm_campaign', `lot_${lotRef}`);
      u.searchParams.set('lot_ref', lotRef);
    }
...
export async function getFundabilityBadge(lot) {
  ...
  try {
    const response = await fetch(url, {
      method: 'POST',
      // X-Source tags the call as machine traffic so BridgeMatch can skip
      // logging it as a user-driven filter search in activity_log.
      headers: {
        'Content-Type': 'application/json',
        'X-Source': 'auctionbrain',
      },
