...
import { initSubAgents, resetSubAgentBudget, auditDataQuality, auditCalendarStaleness, auditStatusDrift, initStatusDrift, gatherDataQualityMetrics } from './sub-agents.js';
import { checkEndedLotRatio, checkCalendarDateSanity } from './quality-gate.js';
...
export function initManager(deps) {
  ...
  if (deps.fetchLotPage) initStatusDrift({ fetchLotPage: deps.fetchLotPage });
}
...
async function buildManagerDashboard() {
  ...
  // Enrichment coverage + ended-lot alerts from cached_analyses
  let enrichmentCoverage = {};
  let endedLotAlerts = {};
  let circuitBreakers = {};
  try {
    if (_deps.getCircuitBreakers) circuitBreakers = _deps.getCircuitBreakers();
  } catch { /* non-fatal */ }
  try {
    if (_supabase) {
      const { data: cached } = await _supabase.from('cached_analyses').select('house, lots');
      const byHouse = {};
      ...
      for (const [house, lots] of Object.entries(byHouse)) {
        const n = lots.length;
        if (!n) continue;
        const epc = Math.round(lots.filter(l => l.epcRating).length / n * 100);
        const flood = Math.round(lots.filter(l => l.floodZone).length / n * 100);
        const lr = Math.round(lots.filter(l => l.streetAvg != null).length / n * 100);
        if (epc < 30 || flood < 30 || lr < 30) {
          poorEnrichment[house] = { epc, flood, lr };
        }
      }
      if (Object.keys(poorEnrichment).length > 0) enrichmentCoverage = poorEnrichment;

      // Ended-lot ratio scan — flag houses with >80% terminal lots
      for (const [house, lots] of Object.entries(byHouse)) {
        const endedCheck = checkEndedLotRatio(house, lots);
        if (endedCheck.flagged) {
          endedLotAlerts[house] = { ratio: endedCheck.ratio, ended: endedCheck.endedCount, total: endedCheck.total };
        }
      }
    }
  } catch { /* non-fatal */ }
  ...
  return JSON.stringify({
    ...
    enrichment_gaps: Object.keys(enrichmentCoverage).length > 0 ? enrichmentCoverage : 'all_adequate',
    data_quality: Object.keys(poorQuality).length > 0 ? poorQuality : 'all_adequate',
    ended_lot_alerts: Object.keys(endedLotAlerts).length > 0 ? endedLotAlerts : 'none',
...
- {"type": "audit_status_drift", "house": "slug", "reason": "why"} — sample 3-5 lot detail pages and compare source status vs our status. Use when ended_lot_alerts or status_drift_suspects show a house. Costs ~3-5 Firecrawl credits.
...
The circuit_breakers section shows EPC/flood/landRegistry API statuses (closed=healthy, open=paused due to failures, half-open=testing). If any breaker is open, skip_houses should include houses that rely heavily on that enrichment, and observations should note the stuck breaker.

The enrichment_gaps section shows per-house EPC/flood/LR coverage below 30%. "all_adequate" means all houses have acceptable enrichment. When houses appear here, note them in observations but do not dispatch extra actions — enrichment reruns automatically when breakers close.

The ended_lot_alerts section shows houses where >80% of lots have terminal status (sold/unsold/withdrawn/stc/ended). "none" means all houses are healthy. When houses appear here, dispatch audit_status_drift to verify whether source pages confirm the ended status — if confirmed, the catalogue is stale and the house should be skipped or its URL healed.
...
export async function runManagerCycle() {
  ...
  try {
    ...
    if (_config.useAI && _callAI) {
      try {
        ...
        for (const action of (aiResult.corrective_actions || [])) {
          ...
          } else if (action.type === 'repair_extractor' && action.house) {
            try {
              const repairResult = await _executeRepairExtractor(action.house);
              report.actions_taken.push({
                type: 'ai_repair_extractor', house: action.house,
                result: repairResult?.success ? 'success' : 'failed',
                detail: repairResult?.detail || repairResult?.reason || action.reason,
              });
            } catch (e) {
              report.actions_taken.push({ type: 'ai_repair_extractor', house: action.house, result: 'error', detail: e.message });
            }
          ...
          } else if (action.type === 'audit_status_drift' && action.house && _supabase) {
            try {
              const { data: cached } = await _supabase
                .from('cached_analyses')
                .select('lots')
                ...
                .single();
              if (cached?.lots) {
                const driftResult = await auditStatusDrift(action.house, cached.lots);
                report.actions_taken.push({
                  type: 'ai_audit_status_drift', house: action.house,
                  result: driftResult?.mismatches > 0 ? 'drift_detected' : 'clean',
                  detail: driftResult?.skipped ? driftResult.reason : `${driftResult.mismatches}/${driftResult.checked} mismatches`,
                });
              } else {
                report.actions_taken.push({ type: 'ai_audit_status_drift', house: action.house, result: 'skipped', detail: 'No cached lots found' });
              }
            } catch (e) {
              report.actions_taken.push({ type: 'ai_audit_status_drift', house: action.house, result: 'error', detail: e.message });
            }
          }
        }
      } catch (aiErr) {
    ...
    // ── 4.6. ENDED-LOT RATIO + CALENDAR DATE SANITY (rule-based, no AI cost) ──
    if (_supabase) {
      try {
        const { data: allCached } = await _supabase
          .from('cached_analyses')
          .select('house, lots')
          .gt('expires_at', new Date().toISOString());

        // Also fetch calendar to know which houses are always_on
        const { data: calRows } = await _supabase
          .from('auction_calendar')
          .select('house_slug, status');
        const alwaysOnSlugs = new Set((calRows || []).filter(r => r.status === 'always_on').map(r => r.house_slug));

        const endedFlags = [];
        const dateFlags = [];

        for (const entry of (allCached || [])) {
          if (!entry.house || !entry.lots || entry.lots.length === 0) continue;

          // Ended-lot ratio check
          const endedResult = checkEndedLotRatio(entry.house, entry.lots);
          if (endedResult.flagged) {
            endedFlags.push({ house: entry.house, ratio: endedResult.ratio, ended: endedResult.endedCount, total: endedResult.total });
          }

          // Calendar date sanity check
          const dateResult = checkCalendarDateSanity(entry.house, entry.lots, {
            isAlwaysOn: alwaysOnSlugs.has(entry.house),
          });
          if (dateResult.flagged) {
            dateFlags.push({ house: entry.house, flags: dateResult.flags });
          }
        }

        if (endedFlags.length > 0) {
          report.actions_taken.push({
            type: 'ended_lot_ratio_check',
            house: null,
            result: 'flagged',
            detail: `${endedFlags.length} house(s) with >80% ended lots: ${endedFlags.map(f => `${f.house} (${f.ratio * 100}%)`).join(', ')}`,
          });
        }
        if (dateFlags.length > 0) {
          report.actions_taken.push({
            type: 'calendar_date_sanity_check',
            house: null,
            result: 'flagged',
            detail: `${dateFlags.length} house(s) with date anomalies: ${dateFlags.map(f => `${f.house}: ${f.flags[0]}`).join('; ')}`,
          });
        }
      } catch (e) {
        // Non-fatal
...
async function _executeRepairExtractor(slug) {
  ...
  // Attempt to obtain sample HTML for the extractor repair
  let sampleHtml = '';

  // Strategy 1: Try to fetch stored raw_html from cached_analyses in Supabase
  if (_supabase && !sampleHtml) {
    try {
      const { data: cached } = await _supabase
        .from('cached_analyses')
        .select('raw_html')
        ...
        .single();
      if (cached?.raw_html) {
        sampleHtml = cached.raw_html;
        console.log(`MANAGER: _executeRepairExtractor(${slug}) — using stored raw_html from cached_analyses`);
      }
    } catch { /* non-fatal — column may not exist or no rows */ }
  }

  // Strategy 2: Fetch fresh HTML from the house's catalogue URL via Firecrawl/scraper
  if (!sampleHtml && _deps.houseRoots && _deps.houseRoots[slug]) {
    const catalogueUrl = _deps.houseRoots[slug];
    try {
      if (_deps.scrapeRenderedPage) {
        const scraped = await _deps.scrapeRenderedPage(catalogueUrl, slug);
        if (scraped?.html) {
          sampleHtml = scraped.html;
          console.log(`MANAGER: _executeRepairExtractor(${slug}) — fetched fresh HTML via scrapeRenderedPage`);
        }
      } else if (_deps.fetchPage) {
        sampleHtml = await _deps.fetchPage(catalogueUrl);
        console.log(`MANAGER: _executeRepairExtractor(${slug}) — fetched fresh HTML via fetchPage`);
      }
    } catch (e) {
      console.warn(`MANAGER: _executeRepairExtractor(${slug}) — failed to fetch fresh HTML: ${e.message}`);
    }
  }

  // If no HTML could be obtained, bail out with a clear reason rather than silently failing
  if (!sampleHtml) {
    console.warn(`MANAGER: _executeRepairExtractor(${slug}) — no sample HTML available, skipping repair`);
    return { success: false, reason: 'no_sample_html' };
  }
