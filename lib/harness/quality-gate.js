...
import { executeHealing } from '../pipeline/harness-bridge.js';
...
export function checkEndedLotRatio(slug, lots, { threshold = 0.8 } = {}) {
  ...
  if (flagged) {
    ...
    // Auto-heal: if every scraped lot has terminal status, the catalogue URL
    // is almost certainly stale (e.g. ?auction=N where N is the past auction).
    // healBrokenHouse has its own exponential-backoff cooldown (1h → 24h →
    // 48h → 96h → 7d, see lib/pipeline/healing.js:106), so triggering on
    // every cron cycle is safe — repeats inside cooldown short-circuit at
    // the `HEAL: Skipping ... — DB cooldown until ...` log.
    //
    // Fire-and-forget — the scrape pipeline shouldn't block on the heal RPC.
    // Errors are non-fatal; healing.js already records its own pipeline_alerts
    // on success/failure of the URL discovery step.
    //
    // Lesson learned 2026-05-11: Maggs & Allen's calendar URL stayed at
    // ?auction=2 (past 23 April catalogue) for ~17 days while this exact
    // alert fired on every cron — nothing acted on it. Wiring up auto-heal
    // closes that gap for every house.
    executeHealing(slug).catch(err => {
      console.warn(`HEAL: ${slug} auto-heal from ended_lot_ratio failed: ${err.message}`);
    });
  }
