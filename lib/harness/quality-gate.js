...
export function evaluateGate(slug, batch, regression, currentCache) {
  ...
  // Rule 0a: Minimum lot count — must have at least 3 lots
  if (currentLots < 3) {
    const reason = `Below minimum lot count: ${currentLots} lot(s) (minimum: 3)`;
    _fireGateAlert(slug, 'reject', reason);
    return { pass: false, decision: 'reject', reason, lotCount: currentLots, minimum: 3 };
  }

  // Rule 0b: Core field coverage — address and price must each be ≥ 60%
  ...
  if (addressCoverage < 60 || priceCoverage < 60) {
    const reason = `Core field coverage too low: address=${addressCoverage}%, price=${priceCoverage}% (minimum: 60% each)`;
    _fireGateAlert(slug, 'reject', reason);
    return { pass: false, decision: 'reject', reason, fieldCoverage: { address: addressCoverage, price: priceCoverage }, minimum: 60 };
  }
...
const ENDED_BULLET_RE = /\bAuction\s*Ended\b|\bSOLD\b|\bUNSOLD\b|\bWITHDRAWN\b|\bSTC\b|\bSALE.?AGREED\b|\bNO.?SALE\b|\bPASSED\b/i;
