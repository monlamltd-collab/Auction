...
export function evaluateGate(slug, batch, regression, currentCache) {
  ...
  // Rule 2: Batch quality below 0.3 → reject
  if (batchQuality < 0.3 && currentLots > 0) {
    const reason = `Batch quality too low: ${batchQuality.toFixed(2)} (threshold: 0.30)`;
    _fireGateAlert(slug, 'reject', reason);
    return { decision: 'reject', reason };
  }

  // Rule 3: Batch quality 0.3-0.5 → cache with warning
  if (batchQuality < 0.5 && batchQuality >= 0.3) {
