...
/**
 * Heuristic: does this URL look like a real property photo, not junk
 * (logos, icons, EPC charts, floorplans, placeholders, 1x1 spacers)?
 *
 * NOT a structural URL validator. Distinct from the shared
 * `isValidImageUrl` in `lib/scraper/validation.js` (also exported via
 * `lib/scraper.js` and `public/img-validator.js` on its unify branch),
 * which checks `https://` + image extension OR CDN allowlist. That
 * answers "is this a plausible image URL?"; this answers "is this URL
 * worth keeping in the lot record?" — junk-pattern reject only.
 *
 * The two are complementary, not redundant: a junky URL can pass the
 * structural check (e.g. `https://example.com/logo.png` — has a `.png`
 * extension, fails this filter), and a non-CDN URL can be a real photo.
 * Quality-scoring uses this filter; persistence/render paths use the
...
export function validateLot(lot) {
  ...
  if (lot.imageUrl && !isLikelyRealPhoto(lot.imageUrl)) normalized.imageUrl = '';

  // Calculate quality score
  const gaps = [];
  let quality = 0;

  if (normalized.imageUrl && isLikelyRealPhoto(normalized.imageUrl)) {
...
export function validateBatch(lots, house, baseline = {}) {
  ...
  for (const field of Object.keys(FIELD_WEIGHTS)) {
    const count = normalizedLots.filter(l => {
      if (field === 'imageUrl') return l.imageUrl && isLikelyRealPhoto(l.imageUrl);
