...
export async function initHouseHealth(supabase) {
  ...
  try {
    const { data: skills } = await _supabase
      .from('house_skills')
      .select('slug, status, last_lot_count, average_lot_count, image_coverage, health_score, circuit_state, consecutive_failures, last_success_at, rolling_lot_counts, rolling_image_coverage');
