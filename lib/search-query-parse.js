...
// Region name → UK postcode-area prefixes. Module-level + exported so the
// smart-search route can fold the UI region dropdown (fLocation) into the SAME
// scoping the query-text region path uses — one source of truth for
// region→postcode (the frontend keeps its own copy for client-side browse).
export const REGION_POSTCODES = {
...
export function parseSmartSearchQuery(query) {
  ...
  // ── Region names → postcode prefix filters (module-level map above) ──
  const regionPostcodes = REGION_POSTCODES;
