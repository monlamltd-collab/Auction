...
async function selectStaleCandidates({ limit, force }) {
  // Strategy:
  //   1. Pool = postcodes on any ACTIVE lot (available/stc/unsold), not just
  //      upcoming-auction. Upcoming lots come first (high value); the no-date /
  //      past-date backlog is included too, because that's where most NEVER-
  //      scraped postcodes live (they fell outside the old upcoming-only pool).
  ...
  const ACTIVE = ['available', 'stc', 'unsold'];
  const [upcoming, backlog] = await Promise.all([
    supabase.from('lots').select('postcode')
      .not('postcode', 'is', null).in('status', ACTIVE)
      .gte('auction_date', today)
      .order('auction_date', { ascending: true }).limit(2000),
    supabase.from('lots').select('postcode')
      .not('postcode', 'is', null).in('status', ACTIVE)
