...
export async function scrapeOpenRent(postcode) {
  ...
  // Crawlee render gating — OpenRent's search page returns a JS shell on a
  // plain GET (see header comment), so it needs a rendered DOM. Crawlee is the
  // non-FC render tier; if it's unavailable, skip gracefully (no Puppeteer
  // fallback for rentals — low priority, not worth the Railway memory).
  if (!hasCrawlee()) return { listings: [], skipped: 'crawlee_unavailable' };
  ...
  try {
    ...
    return { listings: [], skipped: `crawlee_error:${err.message.slice(0, 80)}` };
  }
