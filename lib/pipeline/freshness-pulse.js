...
function pulseConcurrency() {
  ...
  // Default 2 — below the shared render ceiling (renderConcurrency() in
  // lib/config.js, default 5), leaving render slots free for on-demand user
  // analyses and the daily pass.
...
export function selectPulseCandidates({
  ...
  // One auction per slug. An `always_on` row is the house's EVERGREEN catalogue
  // — the URL that always holds the current lots — so it OUTRANKS any dated row;
  // only between two rows of the same kind does the soonest date win.
  //
  // Sorting by date alone parks always_on's 2099-12-31 sentinel last, which is
  // how suttonkersh stayed dark: the pulse kept picking a dated 0-lot hub page
  // over the gallery carrying the live lots, and each 0-lot run re-opened the
  // circuit breaker — so the house could never recover even after the 24h
  // half-open trial. Same defect as the daily pass's lookahead, fixed in
  // selectAuctionsPerHouse (lib/analysis.js); both selectors must agree or the
  // pulse silently undoes the daily pass's choice every hour.
  const isEvergreen = (a) => a && a.status === 'always_on';
  const bySlug = new Map();
  for (const a of auctions) {
    const slug = detectHouse(a.url);
    if (!slug) continue;
    const prev = bySlug.get(slug);
    if (!prev) { bySlug.set(slug, a); continue; }
    if (isEvergreen(a) !== isEvergreen(prev)) {
      if (isEvergreen(a)) bySlug.set(slug, a);
      continue;
    }
    if (String(a.date || '9999') < String(prev.date || '9999')) bySlug.set(slug, a);
  }
