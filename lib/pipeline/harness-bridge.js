  ...
  switch (event.event) {
    ...
    case 'scrape_all_tiers_failed':
      ...
      if (_deps.healBrokenHouse) {
        ...
        await _deps.fireAlert?.('scrape_failure', {
          house: event.house,
          severity: 'error',
          message: `All scrape tiers failed for ${event.house}: ${event.lastError}`,
          metadata: { stage: 'scrape', url: event.url },
        });
      }
      break;
    ...
    case 'scrape_thin_content':
      ...
      await _deps.fireAlert?.('scrape_thin_content', {
        house: event.house,
        severity: 'warning',
        message: `Thin content (${event.htmlLength} chars) from ${event.method} for ${event.house}`,
        metadata: { stage: 'scrape', method: event.method, htmlLength: event.htmlLength },
      });
      break;
    ...
    case 'extract_dom_insufficient':
      ...
      if (event.lotCount > 0 && event.lotCount < event.threshold) {
        await _deps.fireAlert?.('extractor_partial_match', {
          house: event.house,
          severity: 'warning',
    ...
    case 'extract_dom_error':
      ...
      await _deps.fireAlert?.('extractor_error', {
        house: event.house,
        severity: 'error',
        message: `DOM extractor error for ${event.house}: ${event.error}`,
        metadata: { stage: 'extract', error: event.error, selector: event.selector },
      });
      break;

    case 'extract_zero_lots':
      // No lots from any strategy
      await _deps.fireAlert?.('extract_zero_lots', {
        house: event.house,
        severity: 'error',
...
export async function executeHealing(house) {
  ...
  switch (recommendation) {
    ...
    case 'heal_and_alert': {
      ...
      await _deps.fireAlert?.('auto_healed', {
        house,
        severity: 'info',
    ...
    case 'suggest':
      await _deps.fireAlert?.('healing_suggested', {
        house,
        severity: 'warning',
