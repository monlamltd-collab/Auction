...
async function routeEventToHarness(event) {
  switch (event.event) {
    ...
    case 'scrape_all_tiers_failed':
      // All tiers failed → likely URL broken. Fire alert + trigger confidence-gated healing.
      ...
      // Execute confidence-gated healing (confidence will be 0.9 → auto_heal)
      try {
        await executeHealing(event.house);
      } catch (healErr) {
        console.warn(`HARNESS-BRIDGE: Healing failed for ${event.house}:`, healErr.message);
      }
      break;
    ...
    // ── Extract failures ──
    // Firecrawl-only catalogue path; the legacy DOM step was retired
    // 2026-05-08, so DOM-specific events (extract_dom_insufficient,
    // extract_dom_error) no longer fire.
    case 'extract_zero_lots':
      await _deps.fireAlert?.({
        type: 'extract_zero_lots',
        house: event.house,
        severity: 'error',
        message: `Zero lots from Firecrawl + Gemini for ${event.house} (AI: ${event.aiLotCount})`,
        meta: {
          stage: 'extract',
...
export function assessHealingConfidence(house) {
  ...
  // Zero lots but scrape succeeded → Firecrawl JSON extract returned nothing
  // and Gemini fallback also failed. Could be calendar empty, anti-bot edge,
  // or page-shape change. URL healing might find a page with different
  // structure → medium confidence.
