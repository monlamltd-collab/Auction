...
export async function runPhantomLotSweep(supabase, deps = {}) {
  ...
  const phantomById = new Map(phantoms.map(p => [p.id, p]));
  const CHUNK = 100;
  let updated = 0;
  const successfulIds = [];
  ...
  // ── Dual-write: emit lot_status_changed for every successfully flipped phantom ──
  // One event per phantom, batched into a single insert. Non-fatal: lot_events
  // is observability infrastructure, never blocks the sweep.
  if (successfulIds.length > 0) {
    try {
      const events = successfulIds
        .map(id => {
          const phantom = phantomById.get(id);
          if (!phantom) return null;
          return buildLotEvent({
            lotId: id,
            eventType: LOT_EVENT_TYPES.STATUS_CHANGED,
            oldValue: { status: phantom.status ?? null },
            newValue: { status: 'extraction_failure' },
            source: {
              scrape_id: null,
              scraper_version: 'phantom-lot-sweep',
              house: phantom.house,
              writer: 'phantom-lot-sweep.markFailure',
            },
          });
        })
        .filter(Boolean);
      if (events.length > 0) await insertLotEvents(events);
    } catch (e) {
      deps.log?.warn?.('phantom-sweep: lot_events emission failed', { err: e.message });
    }
  }
