...
import { detectDealSignals } from './deal-signals.js';

const W2N = { one:1, two:2, three:3, four:4, five:5, six:6, seven:7, eight:8, nine:9, ten:10, eleven:11, twelve:12 };

// Lot shapes whose total guide price has no per-unit relationship —
// scoring per-unit yield or comparing to single-property street comps
// produces nonsense (yield ~ 0%, below_market ~ -5000% on a £9.5M
// 600-flat block scored against single-flat comps). Skip those signals
// and stamp the manifest so the gate stays closed across later passes.
const BLOCK_LOT_PROP_TYPES = new Set(['block_sale', 'portfolio', 'commercial_block']);

/** True when the lot is a multi-unit / portfolio sale where per-unit signals don't apply. */
function isBlockLot(lot) {
  if (lot && BLOCK_LOT_PROP_TYPES.has(lot.propType)) return true;
  // Allsop sometimes flags units explicitly on the API record. Anything > 1
  // is a portfolio signal even if propType wasn't set from the prefix.
...
function analyseLot(raw) {
  ...
  // Detection text: bullets + source-site narrative + address. description was
  // added to this blob 2026-07-13 — catalogue bullets are thin crumbs for most
  // houses while the narrative carries the HMO/income/planning sentences.
  const t = ((raw.bullets || []).join(' ') + ' '
    + (raw.description ? String(raw.description).slice(0, 4000) : '') + ' '
  ...
  const L = { ...raw, score: 0, opps: [], risks: [], dealType: 'Standard', propType: '', beds: null,
    ...
    dealSignals: [], statedIncomePa: null, incomeKind: null,
    _enrichment: raw._enrichment || createManifest() };

  // Block / portfolio detection — captured BEFORE propType is overwritten
  // by inference. Upstream (e.g. Allsop's RI/RP/CI prefix in
  // lib/scraper/allsop.js) sets these propTypes; preserving them here keeps
  // the gating decision consistent with how the lot was extracted, and the
  // upstream `units` field (when > 1) is an extra portfolio signal.
  const upstreamBlockClass = BLOCK_LOT_PROP_TYPES.has(raw.propType) ? raw.propType : null;
  const upstreamUnitsSignal = typeof raw.units === 'number' && raw.units > 1;
  ...
  // Restore upstream block-class propType (e.g. Allsop RI/RP/CI). Inference
  // above sees "Flats/Houses" in the bullets and overwrites to 'flat' — but
  // a 600-flat block at £9.5M is not a flat for scoring purposes. The
  // upstream classification is more authoritative when present.
  if (upstreamBlockClass) L.propType = upstreamBlockClass;
  ...
  // Multi-label deal signals (HMO / investment-valuation / income / short-lease
  // / mixed-use / etc.) — computed after beds/propType/titleSplit so the
  // detectors can gate on them. deal_type below stays single-label for
  // back-compat; dealSignals is the richer, filterable view.
  const ds = detectDealSignals({
    text: t,
    beds: L.beds,
    propType: L.propType,
    tenure: L.tenure,
    leaseLength: (typeof raw.leaseLength === 'number') ? raw.leaseLength : null,
    titleSplit: L.titleSplit,
  });
  L.dealSignals = ds.signals;
  L.statedIncomePa = ds.statedIncomePa;
  L.incomeKind = ds.incomeKind;
  ...
  // Block / portfolio / commercial-investment lots: per-unit yield against
  // a whole-block guide is meaningless (£12k pa rent for one flat / £9.5M
  // total guide ≈ 0.13%). Stamp both scoring slots closed so this and the
  // enrichment-side below-market scorer skip the lot. propType-based gate
  // matches Allsop RI/RP/CI; units > 1 picks up explicit portfolio counts.
  const blockLot = BLOCK_LOT_PROP_TYPES.has(L.propType) || upstreamUnitsSignal;
  if (blockLot) {
    recordYieldSkipped(L._enrichment, 'skipped_block_lot');
    recordBelowMarketSkipped(L._enrichment, 'skipped_block_lot');
  } else {
    const rm = t.match(/(?:let\s+at|rent\s+of|income\s+of|producing)\s+£?([\d,]+)\s*(?:p\.?a|per\s*annum)/);
    // Fall back to the deal-signals income extractor (wider verb/period
    // coverage — pcm, pw, "passing rent of") but only for PASSING rent;
    // appraised "potential" figures must not masquerade as achieved yield.
    const annualRent = rm ? parseInt(rm[1].replace(/,/g, ''))
      : ((ds.statedIncomePa && ds.incomeKind === 'passing') ? ds.statedIncomePa : null);
    if (annualRent && L.price) {
      const rent = annualRent; const gy = (rent / L.price) * 100;
  ...
  // HMO tag: no score of its own (the extension/HMO keyword already scores
  // +1.5 above); the chip is what investors filter on.
  if (ds.signals.includes('hmo')) L.opps.push('HMO');
  // Investment-valuation candidate — scale/fabric/income suggests a lender's
  // valuer may apply a commercial (yield-based) valuation vs bricks & mortar:
  // the refinance-arbitrage case.
  if (ds.signals.includes('investment-valuation')) { s += 1; sb.push({ signal: 'Investment valuation candidate', pts: 1 }); L.opps.push('Investment valuation candidate'); }
  if (ds.statedIncomePa && ds.incomeKind === 'passing') L.opps.push(`Producing £${ds.statedIncomePa.toLocaleString()} pa`);
  ...
  if (/subsidence|structural movement|underpinn/.test(t)) { s -= 1.5; sb.push({ signal: 'Subsidence', pts: -1.5 }); L.risks.push('Subsidence'); }
  // Cladding: "cladding" alone matches benign descriptions ("cedar cladding"),
  // so require EWS1 or a defect/remediation context.
  if (/\bews1\b|unsafe cladding|combustible cladding|cladding (?:issue|defect|remediation|works)/.test(t)) { s -= 1; sb.push({ signal: 'Cladding/EWS1', pts: -1 }); L.risks.push('Cladding/EWS1'); }
  ...
  // HMO outranks Refurb: a tenanted 6-bed HMO needing modernisation is bought
  // as an HMO first — condition still shows via the condition field + opps.
  else if (ds.signals.includes('hmo')) L.dealType = 'HMO';
