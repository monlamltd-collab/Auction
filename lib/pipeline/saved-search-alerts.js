...
// ═══════════════════════════════════════════════════════════════
// SAVED-SEARCH EMAIL ALERTS (free core loop; Pro buys depth)
// ═══════════════════════════════════════════════════════════════
//
// For every saved_search where notify_email = true, find lots seen since
// last_notified_at that match the saved filter set, and send the user a
// digest email with deep links.
//
// FREE SINCE 2026-07-04 (Phase 4): the cycle used to hard-skip non-Pro
// owners, so free users' saved searches silently never emailed — the
// product's core return loop was paywalled off. Every tier now gets the
// daily alert; Pro's edge is DEPTH (full match list per email vs the free
// tier's top 5 + view-all link).
//
...
// Free tier sees the top 5 matches per email (plus a view-all link);
// Pro sees up to 10 (the pre-free-tier rendering cap). Depth, not access,
// is the premium edge — the alert loop itself is free.
const FREE_EMAIL_LOT_CAP = 5;
const PRO_EMAIL_LOT_CAP = 10;

export function renderAlertEmail({ searchName, matches, tier = 'free' }) {
  const n = matches.length;
  const cap = tier === 'premium' ? PRO_EMAIL_LOT_CAP : FREE_EMAIL_LOT_CAP;
  const subject = `${n} new lot${n === 1 ? '' : 's'} match your saved search "${searchName}"`;

  const cards = matches.slice(0, cap).map(lot => {
  ...
  const more = n > cap
    ? `<p style="font-family:Arial,sans-serif;font-size:14px;color:#6b7c8d;margin:0 0 16px">…and ${n - cap} more matches. <a href="${SITE}" style="color:#c0392b">See all on Auction Brain →</a>${tier === 'premium' ? '' : ` <span style="color:#8a847a">Pro members get every match in this email.</span>`}</p>`
    : '';
...
export async function runSavedSearchAlertsCycle(supabase, deps = {}) {
  ...
  for (const s of searches || []) {
    const owner = s.users || {};
    // All tiers get the alert (free since 2026-07-04); tier only sets the
    // per-email depth in renderAlertEmail.
    ...
    const { subject, html } = renderAlertEmail({ searchName: s.name, matches, tier: owner.tier });
