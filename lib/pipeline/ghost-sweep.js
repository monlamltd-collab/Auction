...
//
// Semantics mirror the persist-lots prune: status -> 'withdrawn' stamped with
// removed_reason + removed_at, plus lot_vanished + lot_status_changed events.
//
// The stamp lives INSIDE enrichment_manifest — `lots` has no removed_reason /
// removed_at columns, and naming them at the top level is exactly the bug that
// made every flip fail in production ("Could not find the 'removed_at' column
// of 'lots' in the schema cache", 2026-07-21) while the sweep reported the
// failures and retried forever. Provenance has one home: the manifest (the
// prune writes it there; docs/CRAWLEE-TRIAL-RUNBOOK.md queries it there).
// tests/test-ghost-sweep.js asserts every patch key against LOT_COLUMNS so a
// phantom column can't ship again.
//
// flipLots MERGES the manifest patch (jsonb ||) — see retire_lots in
// migrations/2026-07-22-retire-lots-rpc.sql. It must never replace the
// manifest: sweep candidates average ~9 enrichment keys including paid OS
// Places lookups, and ghost lots often come back.
//
// The per-house safety gate mirrors the prune's regression gate: if a house has
 ...
 *   flipLots: (ids: string[], patch: object) => Promise<number>,  // returns rows updated; throws (or returns 0) on failure.
 *     patch keys are real `lots` columns only; patch.enrichment_manifest is MERGED
 *     into the existing manifest, never replaced (see retire_lots RPC).
...
export async function runGhostSweep(deps, opts = {}) {
  if (process.env.GHOST_SWEEP_DISABLED === 'true') {
    return { skipped: 'disabled', ghosts: 0, pastDated: 0, held: [], flipFailures: 0 };
  }
  ...
  if (opts.dryRun) {
    return { skipped: 'dry_run', ghosts: ghostFlips.length, pastDated: pastDatedFlips.length, held, flipFailures: 0 };
  }

  const now = new Date(nowMs).toISOString();
  let ghosts = 0, pastDated = 0, flipFailures = 0;
  for (const [flips, reason, counterAdd] of [
    ...
    // Every key here MUST be a real `lots` column (LOT_COLUMNS) — the retirement
    // provenance goes inside enrichment_manifest, not alongside it.
    const patch = {
      status: 'withdrawn',
      enrichment_manifest: { removed_reason: reason, removed_at: now },
    };
    // Batch by id, mirroring the prune's batching.
    for (let i = 0; i < flips.length; i += 100) {
      const batch = flips.slice(i, i + 100);
      let updated = 0, flipError = null;
      try {
        updated = await deps.flipLots(batch.map(r => r.id), patch);
      } catch (e) {
        flipError = e;
      }
      if (flipError || !(updated > 0)) {
        // The flip did NOT land — emitting events here would write withdrawals
        // to lot_events (the source of truth) that never happened in `lots`.
        // Skip the batch; the lots stay 'available' and retry next sweep.
        flipFailures += batch.length;
        const why = flipError ? flipError.message : 'flipLots reported 0 rows updated';
        console.error(`GHOST-SWEEP: flip batch FAILED (${reason}, ${batch.length} lots): ${why}`);
        try {
          await deps.fireAlert({
            type: 'ghost_sweep_flip_failed',
            severity: 'error',
            house: null,
            message: `Ghost sweep flip batch FAILED (${reason}): ${batch.length} lots left 'available' — ${why}. No events emitted for the batch; it retries on the next sweep.`,
            meta: { reason, lots: batch.length, error: why },
          });
        } catch { /* alerting must never break the sweep */ }
        continue;
      }
  ...
  console.log(`GHOST-SWEEP: retired ${ghosts} ghosts + ${pastDated} past-dated lots (${held.length} houses held on partial-scrape gate${flipFailures ? `, ${flipFailures} flips FAILED` : ''})`);
  return { skipped: null, ghosts, pastDated, held, flipFailures };
}
