 ...
 */
export function recallGateAlert({ house, recall, lots = 0, sentinelLots = 0, reason = '', engine = 'crawlee', recognised = 0, extra = {} }) {
  const v = recallGateVerdict({ recall, lots, sentinelLots });
  const pct = recall == null ? 'n/a' : Math.round(recall * 100) + '%';
  return {
    type: v.isGap ? 'recall_below_100' : 'recall_diagnostic',
    severity: v.severity,
    house,
    message: v.isGap
      ...
      : `${engine} recall ${pct}: ${lots}/${sentinelLots}${reason ? ` (${reason})` : ''}`,
    meta: { engine, recall, lots, sentinelLots, missing: v.missing, recognised, reason, gate: '100pct', ...extra },
  };
}
