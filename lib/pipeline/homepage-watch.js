...
import { auditHomepageSelfHosted } from '../scraper/homepage-audit.js';
import {
  AH_PLATFORM_SLUGS,
  AH_FUTURE_DATES_URL,
  fetchAhFutureDates,
...
export async function auditHouseHomepage(slug, configuredUrl, opts = {}) {
  ...
  try {
    audit = await auditHomepageSelfHosted(homepage, { prev: opts.prev || null });
  } catch (err) {
...
async function persistResult(supabase, slug, configuredUrl, result, prev) {
  ...
  const row = {
    ...
    last_content_hash: audit?.contentHash || prev?.last_content_hash || null,   // NEW
  ...
  switch (d.verdict) {
    ...
    case VERDICTS.NOT_AN_AUCTION_HOUSE: {
      ...
      return {
        message: lines.join('\n'),
        buttons: [[
          { label: '🏚 Retire house', callback_data: `retire:${d.alertId}` },
        ], [
          { label: '⏸ Snooze 7d', callback_data: `snooze:${d.alertId}` },
          { label: '✗ Dismiss', callback_data: `dismiss:${d.alertId}` },
        ]],
