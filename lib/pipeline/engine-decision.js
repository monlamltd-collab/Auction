...
export function resolveEngineForHouse({
  ...
  const installed = _hasCrawlee();
  // hasMarkdownRecogniser no longer pins to Firecrawl (the Crawlee path is
  // recogniser-aware, Phase 3) — kept in the signature for call-site clarity
  // and future telemetry, but not forwarded to the router.
  void hasMarkdownRecogniser;
  ...
  return chooseEngine({
    ...
    crawleeAvailable: installed && isCrawleeEnabled(house),  // proactive: allowlist-gated
    crawleeInstalled: installed,                             // failover: any house when FC exhausted
    crawleeIsDefault: process.env.CRAWLEE_DEFAULT === 'true', // main-engine switch
