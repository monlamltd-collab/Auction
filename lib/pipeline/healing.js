...
// ── Junk URL filter for _webSearchForCatalogue candidates ──
// Firecrawl /v1/search returns the top web results for a query. Social,
// video, encyclopedic, and PDF surfaces are never the catalogue page we're
// looking for, but FIRE-1 will happily visit them at ~23 credits each. The
// regex is applied to the candidate URL list before any FIRE-1 call.
 ...
 * @param {string|undefined} deps.FIRECRAWL_API_KEY - only still used by the FC /v1/search fallback (Strategy E); core heal no longer needs it
...
export async function healBrokenHouse(slug, oldUrl, deps) {
  // FC key no longer required for the core heal (HTTP/Crawlee + Gemini). It's
  // only needed by the optional /v1/search fallback (Strategy E), which already
  // guards on deps.FIRECRAWL_API_KEY internally. Gating the whole heal on it
  // would keep self-healing dead on machines without an FC key.
...
async function _webSearchForCatalogue(slug, oldUrl, homepageUrl, deps) {
  ...
  // Site-scoped queries only. A previous un-scoped fallback query
  // (`"${houseName}" property auction current lots`) was returning random
  // Facebook posts, blog spam, and PDF catalogues which then cost ~23 credits
  // each when handed to FIRE-1. The site:-scoped queries already cover the
  // case where the catalogue URL has changed within the same domain; if those
  // miss, FIRE-1 chasing third-party indexes is more likely to commit a wrong
  // URL than to find the real one.
  const queries = [
  ...
  for (const query of queries) {
    try {
      ...
      // Firecrawl bills the search call regardless of result count; record
      // it locally so BUDGET-FC reflects the real spend (audit 5.16).
      getBudget()?.recordFcSearchRequest('healing');
      ...
      // Collect candidate URLs from search results. Reject junk domains
      // (social, video, PDFs, etc.) before they cost FIRE-1 credits.
      const candidateUrls = results
        .map(r => r.url)
        .filter(u => typeof u === 'string' && /^https?:\/\//i.test(u))
        .filter(u => !isJunkSearchUrl(u))
        .slice(0, 5);

      if (candidateUrls.length === 0) {
        console.log(`HEAL: All ${results.length} search results for "${query}" filtered as junk (social/video/PDF/etc.)`);
        continue;
      }

      console.log(`HEAL: Got ${candidateUrls.length} candidate URLs for "${query}" — handing to FIRE-1`);

      if (_fire1RecentlyCalled(candidateUrls)) {
        console.log(`HEAL: FIRE-1 search-ranker skipped for ${slug} — same candidate set called within dedup window`);
        continue;
      }
      _fire1MarkCalled(candidateUrls);
...
Return your finding via the structured schema. If none of the candidates is a current catalogue page, return newUrl=null and confidence='none'.`;
      ...
      try {
        const agentData = await aiExtract(candidateUrls, prompt, NEW_URL_SCHEMA, { timeout: AGENT_TIMEOUT_MS });
