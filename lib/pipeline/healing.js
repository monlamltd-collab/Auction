...
const JUNK_SEARCH_URL_RE = /(?:^|\/\/|\.)(?:facebook\.com|fb\.com|twitter\.com|x\.com|youtube\.com|youtu\.be|linkedin\.com|instagram\.com|tiktok\.com|reddit\.com|pinterest\.|wikipedia\.org|paperturn-view\.com|issuu\.com|scribd\.com|slideshare\.net)\b|\.pdf(?:\?|#|$)/i;
export function isJunkSearchUrl(url) {
  if (!url || typeof url !== 'string') return true;
  return JUNK_SEARCH_URL_RE.test(url);
}

// ── Catalogue candidate verification (healCandidateVerdict) ──
// A healed OR discovered URL is a valid catalogue ONLY if it is alive, is not
// itself a single-lot page, and actually advertises lots. This is the guard that
// stops the healer/discovery replacing a correct catalogue URL with a news
// article, a buyer's guide, a bare homepage, or a single lot page — the exact
// shapes that corrupted the fleet in 2026-07 (the ">500 chars = alive" gate was
// its only check). Pure + unit-tested in tests/test-heal-verify.js.

// Minimum distinct lot links a real catalogue listing must advertise.
const MIN_ADVERTISED_LOTS = 4;

// URL shapes that identify a SINGLE lot-detail page (never a catalogue listing),
// even when the page shows a few "related lots". Mirrors discovery.js
// LOT_URL_PATTERNS so both paths reject the same single-lot shapes.
const SINGLE_LOT_URL_RE = /(\/lot\/details?\/|\/lot\/\d+(?:[/#?]|$)|\/property\/details?\/|\/properties\/\d+(?:[/#?]|$)|\/properties\/lot\/|lot[_-]?id=|property[_-]?id=)/i;

// Generic lot-link shapes, counted UNION the house's recall sentinel so a house
// whose sentinel is imprecise/absent (or a relocated catalogue on new lot URLs)
// still has its lots counted. Capture group = the lot id.
const GENERIC_LOT_LINK_RE = /(?:\/lot(?:\/details|\/redirect)?\/|\/properties\/lot\/|property_details\.asp\?id=|[?&](?:lot|property|prop)?[_-]?id=)([0-9a-f][0-9a-f-]{1,})/gi;

// Count the distinct lot ids a page advertises, via the house sentinel + the
// generic shapes (deduped by id). Best-effort and pure; never throws.
export function countAdvertisedLots(html, slug) {
  if (!html || typeof html !== 'string') return 0;
  const ids = new Set();
  // 1. House recall sentinel (authoritative when present).
  try {
    const sentinel = resolveRecallSentinel(slug);
    if (sentinel && sentinel.source) {
      const re = new RegExp(sentinel.source, 'g');
      let m;
  ...
  } catch { /* sentinel resolution is best-effort — fall back to generic shapes */ }
  // 2. Generic lot-link shapes (union with the sentinel; dedup by id).
  {
    const re = new RegExp(GENERIC_LOT_LINK_RE.source, 'gi');
    let m;
    while ((m = re.exec(html)) !== null) {
      if (m[1]) ids.add(m[1]);
      if (m.index === re.lastIndex) re.lastIndex++;
...
/**
 * Decide whether a candidate `url` (with its fetched `html`) is a valid catalogue
 * for `slug`. Used by BOTH the self-healer and the discovery pipeline so neither
 * can persist a non-catalogue URL.
 * @returns {{ok: boolean, reason: string, lots: number}}
 */
export function healCandidateVerdict(url, html, slug) {
  if (!html || html.length < 500) return { ok: false, reason: 'thin-content', lots: 0 };
  if (SINGLE_LOT_URL_RE.test(url || '')) return { ok: false, reason: 'single-lot-url', lots: 0 };
  if (isJunkSearchUrl(url)) return { ok: false, reason: 'junk-url', lots: 0 };
  const lots = countAdvertisedLots(html, slug);
  if (lots < MIN_ADVERTISED_LOTS) return { ok: false, reason: 'no-lots-advertised', lots };
  return { ok: true, reason: 'ok', lots };
}
...
export async function healBrokenHouse(slug, oldUrl, deps) {
  ...
  // ── Kill switch (2026-07-24) ─────────────────────────────────────────
  // Auto-heal URL rewrites were thrashing catalogue targets (past-auctions,
  // calendars, single-lot pages, low-confidence sitemap guesses) and adding
  // noise rather than coverage. Default OFF for automated callers
  // (nightly sweep, homepage watch, harness). Admin `/api/admin/heal` and
  // explicit deps.force=true still run. Opt fleet-wide auto-heal only with
  // AUTO_HEAL_ENABLED=true.
  const autoHealEnvOn = String(process.env.AUTO_HEAL_ENABLED || '').toLowerCase() === 'true';
  const forced = !!(deps && deps.force);
  if (!autoHealEnvOn && !forced) {
    console.log(`HEAL: auto-heal disabled for ${slug} (set AUTO_HEAL_ENABLED=true or pass force:true for manual heal)`);
    return null;
  }
...
async function _verifyNewUrl(newUrl, slug, deps) {
  try {
    ...
    // A candidate is only committed if it's a real catalogue: alive, not a
    // single-lot page, and actually advertising lots. This is what stops the
    // healer replacing a correct URL with a news article / buyer's guide /
    // bare homepage / single lot page (the 2026-07 fleet-wide corruption).
    const verdict = healCandidateVerdict(newUrl, verifyHtml, slug);
    if (!verdict.ok) {
      console.log(`HEAL: rejecting candidate ${newUrl} for ${slug} — ${verdict.reason} (advertises ${verdict.lots} lots)`);
      return false;
    }
    console.log(`HEAL: ✓ verified ${newUrl} for ${slug} — advertises ${verdict.lots} lots`);
    return true;
  } catch (verifyErr) {
