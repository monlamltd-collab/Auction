...
export function dechromeGallery(images, imageUrl, bleedUrls = new Set()) {
  ...
  const isBad = u => isChromeUrl(u) || bleedUrls.has(u);
  const cleaned = arr.filter(u => u && !isBad(u));
  const prevThumb = imageUrl ?? null;
  let newThumb = prevThumb;
  if (!newThumb || newThumb.length < 10 || isBad(newThumb)) {
    newThumb = cleaned.length ? cleaned[0] : null;
  }
  const changed = cleaned.length !== arr.length || newThumb !== prevThumb;
  return { images: cleaned, imageUrl: newThumb, changed };
}
