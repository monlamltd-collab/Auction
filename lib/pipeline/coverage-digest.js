...
export async function buildCoverageDigest(supabase) {
  ...
  // Retry the primary query up to 3 times on transient Supabase failures (e.g. 522 timeout).
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (!error) break;
    if (attempt < 3) {
      console.warn(`coverage-digest: Supabase query failed (attempt ${attempt}/3): ${error.message} — retrying in 10s`);
  ...
  const rows = data || [];
  // Use the exact count from PostgREST, not rows.length — the server caps
  // returned rows at db-max-rows (default 1000) regardless of .limit().
