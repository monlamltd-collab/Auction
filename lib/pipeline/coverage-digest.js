...
export async function buildCoverageDigest(supabase) {
  ...
  // Page through the 7-day window in 1000-row chunks. PostgREST silently caps
  // .select() at db-max-rows (default 1000) regardless of .limit(), so a single
  // unpaginated query computes the numerator over only the first 1000 rows
  // while the denominator (exact count) reflects the true total — deflating
  // every percentage by ~5× once we cross 1000 lots/week. .range() pagination
  // keeps the two sides aligned. .order() makes the windowing deterministic.
  const PAGE_SIZE = 1000;
  const rows = [];
  let totalCount = null;

  for (let from = 0; from < RECENT_LOT_LIMIT; from += PAGE_SIZE) {
    const to = Math.min(from + PAGE_SIZE, RECENT_LOT_LIMIT) - 1;
    ...
    if (pageError) {
      return { error: pageError.message, since, totalLots: 0, coverage: {}, deltas: {}, snapshotWritten: false };
    }
  ...
  const coverage = computeCoverage(rows, total);
  ...
  return { totalLots: total, since, coverage, deltas, snapshotWritten, worstHouses };
}


/**
 * Rank houses by missing image+postcode coverage among recent lots.
 * Only includes houses with >= minLots in the window so tiny samples don't dominate.
 */
