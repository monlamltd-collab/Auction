 ...
 *   - a house flagged `dormant` (known between-auctions/defunct) — its 0/all-terminal
 *     result is expected, not a failure
 *
 * @param {{average_lot_count?:number,last_lot_count?:number,last_probe_result?:string,dormant?:boolean}} skill
 * @returns {boolean}
 */
export function isSilentScraperFailure(skill) {
  if (!skill) return false;
  if (skill.dormant === true) return false;
