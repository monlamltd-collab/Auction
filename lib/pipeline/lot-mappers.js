...
//
// ⚠ DEPRECATED — superseded by lib/types/lot.js
// ─────────────────────────────────────────────────────────────────────────
// This module's exports (LOTS_SELECT, createDbRowToLot, dbRowToFrontendLot)
// are being consolidated into the canonical lot contract at lib/types/lot.js.
// New code MUST import from there:
//
//   import { LOTS_SELECT, LOT_COLUMNS, dbRowToLot, lotToDbRow } from '../types/lot.js';
//
// The two legacy mappers `createDbRowToLot` and `dbRowToFrontendLot` are
// replaced by a single consolidated `dbRowToLot(row)` whose output is the
// union of both, so existing consumers see no shape change after migration.
//
// This file is preserved verbatim until Task 3 of the lot-contract
// consolidation migrates every consumer. Do NOT add new imports against
// this module. After all consumers are cut over, delete this file.

// ── Standard lots select columns for DB queries ──
// `images` (JSONB array, populated by lib/pipeline/multi-image-sweep.js)
// powers the per-card carousel in public/app.js — without it the carousel
// branch is a dead code path (lot.images is always undefined). Was held
// out of LOTS_SELECT historically while the gallery feature was being
// designed; included now since the renderer is shipping.
export const LOTS_SELECT = 'house, lot_number, url, catalogue_url, address, postcode, lat, lng, price, price_text, price_status, prop_type, beds, tenure, lease_length, sqft, condition, image_url, images, bullets, units, auction_date, status, sold_price, epc_rating, epc_score, epc_date, epc_floor_area_sqm, epc_floor_area_sqft, epc_works_cost_mid, epc_works_summary, flood_zone, flood_risk, street_avg, street_sales, street_sales_count, below_market, est_monthly_rent, est_annual_rent, est_gross_yield, score, score_breakdown, opps, risks, deal_type, vacant, title_split, search_text, enrichment_manifest, value_estimate, last_seen_at, quality_score, quality_issues';
...
export function createDbRowToLot(deps) {
  return function dbRowToLot(dbRow) {
    return {
      ...
      price: dbRow.price, priceText: dbRow.price_text, priceStatus: dbRow.price_status || null, propType: dbRow.prop_type, beds: dbRow.beds,
      ...
      epcFloorAreaSqm: dbRow.epc_floor_area_sqm ?? null, epcFloorAreaSqft: dbRow.epc_floor_area_sqft ?? null,
      epcWorksCostMid: dbRow.epc_works_cost_mid ?? null,
      epcWorksSummary: Array.isArray(dbRow.epc_works_summary) ? dbRow.epc_works_summary : [],
      ...
      qualityScore: typeof dbRow.quality_score === 'number' ? dbRow.quality_score : null,
      qualityIssues: Array.isArray(dbRow.quality_issues) ? dbRow.quality_issues : [],
      valueEstimate: dbRow.value_estimate || null,
...
export function dbRowToFrontendLot(r) {
  return {
    ...
    address: r.address, postcode: r.postcode, price: r.price, priceText: r.price_text, priceStatus: r.price_status || null,
    propType: r.prop_type, beds: r.beds, tenure: r.tenure, leaseLength: r.lease_length,
    sqft: r.sqft, condition: r.condition, imageUrl: r.image_url,
    ...
    epcFloorAreaSqm: r.epc_floor_area_sqm ?? null, epcFloorAreaSqft: r.epc_floor_area_sqft ?? null,
    epcWorksCostMid: r.epc_works_cost_mid ?? null,
    epcWorksSummary: Array.isArray(r.epc_works_summary) ? r.epc_works_summary : [],
