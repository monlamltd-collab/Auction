...
export async function probe(ctx, deps) {
  ...
  // Fetch the first page to hash. Plain HTTP only: probe() runs in the rare
  // fallback tail (primary Firecrawl extract returned 0 lots or threw), and
  // changeTracking already covers change-detection on the main path — a
  // Firecrawl scrape here would just spend a credit doing the same job.
  let probeHtml;
  const probeSource = 'http';
