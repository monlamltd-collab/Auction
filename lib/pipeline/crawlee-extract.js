...
export async function renderAndExtractWithCrawlee(scrapeUrl, house, opts = {}, deps = {}) {
  const { maxPages, paginateAs, prefetchedPage1, deadlineAt, recallSentinelPattern, recogniseFromMarkdown, onExtract } = opts;
  ...
  if (recallSentinelPattern) {
    const ids = new Set();
    for (const p of pages) {
      const src = (recogniseFromMarkdown && p.markdown)
        ? p.markdown
        : String(p.html || '').replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi, ' ').replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, ' ');
