...
export async function sweepMultiImages() {
  ...
  // Active lots with no/empty images. Postgres-side filter on JSONB-array
  // null/empty keeps the query cheap. Final cooldown gate runs client-side.
  const { data: candidates, error: queryErr } = await supabase
    .from('lots')
    .select('id, house, url, images, image_url, enrichment_manifest')
    .in('status', ['available', 'unsold'])
    .not('url', 'is', null)
    .or(`images.is.null,images.eq.[]`)
    .order('first_seen_at', { ascending: true })
    .limit(SWEEP_BATCH_LIMIT * 4);

  if (queryErr) {
    log.error('multi-image-sweep: candidate query failed', { err: queryErr.message });
    return zeroStats();
  }
  ...
  for (const lot of candidates) {
    if (eligible.length >= SWEEP_BATCH_LIMIT) break;
  ...
  log.info('multi-image-sweep: starting', { eligible: eligible.length });
  const stats = { ...zeroStats(), eligible: eligible.length };
  ...
  for (const lot of eligible) {
    ...
    // DOM detail extractor first (more accurate where one exists), fall
    // back to generic regex for everything else.
    let images = [];
    try {
      const detail = extractLotDetail(fetchResult.html, lot.house, fetchResult.url || lot.url);
      if (detail && Array.isArray(detail.images) && detail.images.length > 0) {
        images = detail.images.slice(0, MAX_IMAGES_PER_LOT);
      }
    } catch {
      // Non-fatal — fall through to generic.
    }
    if (images.length === 0) {
      images = extractImagesFromHtml(fetchResult.html, fetchResult.url || lot.url);
    }
