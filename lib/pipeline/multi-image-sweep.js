...
export async function sweepMultiImages(opts = {}) {
  const startedAt = Date.now();
  const cooldownCutoff = new Date(Date.now() - SWEEP_COOLDOWN_DAYS * 86400000).toISOString();
  const today = new Date(Date.now()).toISOString().slice(0, 10);
  // Optional house scope. A fleet sweep fills Tier-1 (upcoming auctions) first
  // then round-robins the past/undated Tier-2 backlog, so a house whose live
  // inventory carries a PAST auction_date — cliveemson's rolling "current
  // catalogue" of post-auction-available lots — is starved of the backlog
  // budget indefinitely. `{ house }` restricts both tiers to that slug so an
  // operator can force its galleries to fill (mirrors /api/admin/backfill-images).
  const houseScope = opts.house || null;
  // Only sweep lots a user could actually SEE. get_active_lots() hides
  // 'available' lots not seen in 21 days, so sweeping long-unseen inventory
  // wastes the budget on invisible lots AND — within a house — starves freshly
  // scraped lots behind a wall of stale ones (cliveemson's ~158 retired 264/265
  // lots clogged the house-scoped sweep, all url_dead, before its 42 live lots
  // were reached). 45 days is generous vs the 21-day visibility gate so no
  // currently-shown lot is dropped.
  ...
  // Status scope includes 'stc' (sold-subject-to-contract): those lots are still
  // shown to users and their source pages still carry photos, so they're in the
  // 100% gallery-coverage goal. 'sold'/'withdrawn' stay excluded (truly ended).
  let tier1q = supabase
    .from('lots')
  ...
  if (houseScope) tier1q = tier1q.eq('house', houseScope);
  const tier1 = await tier1q
  ...
  if (rawCandidates.length < SWEEP_FETCH_POOL) {
    ...
    if (houseScope) tier2q = tier2q.eq('house', houseScope);
    const tier2 = await tier2q
