...
export function renderDigestEmail({ recipientEmail, unsubscribeToken, matches, weekLabel }) {
  ...
  const cards = matches.map(lot => {
    ...
    // Finance CTA (Phase 3): deep link into BridgeMatch's /apply form,
    // pre-filled with the guide price and attributed per lot
    // (utm_campaign=lot_<uuid> + lot_ref land in the lead record). Only
    // when there's a price to quote against and an id to attribute to.
    const financeBtn = (lot.id && lot.price)
      ? `<a href="${escapeHtml(`https://www.bridgematch.co.uk/apply?purchase_price=${Number(lot.price)}&utm_source=auctionbrain&utm_medium=digest_email&utm_campaign=lot_${lot.id}&lot_ref=${lot.id}`)}" style="display:inline-block;font-family:Arial,sans-serif;font-size:14px;font-weight:600;background:#1a2a3a;color:#fff;padding:8px 16px;border-radius:6px;text-decoration:none;margin-left:8px">Check bridging finance →</a>`
      : '';
    return [
  ...
  const html = [
    ...
    `<div style="margin:28px 0;padding:20px;background:#faf8f4;border-radius:8px;border:1px solid #e4dfd6">`,
    `<p style="font-family:Arial,sans-serif;font-size:15px;color:#1a1714;font-weight:600;margin:0 0 6px">Quick question</p>`,
    `<p style="font-family:Arial,sans-serif;font-size:14px;color:#6b7c8d;margin:0 0 12px;line-height:1.5">We're early-stage with ~75 users. What would make AuctionBrain more useful for you? Hit reply — we read every response.</p>`,
    `<a href="mailto:hello@auctionbrain.co.uk?subject=AuctionBrain+feedback" style="display:inline-block;font-family:Arial,sans-serif;font-size:14px;font-weight:600;background:#1a1714;color:#fff;padding:8px 16px;border-radius:6px;text-decoration:none">Reply with feedback →</a>`,
    `</div>`,
