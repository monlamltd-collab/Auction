...
import { recordOsPlaces } from '../enrichment-manifest.js';
  ...
  for (let i = 0; i < targets.length; i += OS_PLACES_CONCURRENCY) {
    ...
    await Promise.allSettled(batch.map(async lot => {
      try {
        ...
        if (!lot._enrichmentManifest) lot._enrichmentManifest = {};
        recordOsPlaces(lot._enrichmentManifest, {
        ...
        if (result.status === 'ok' || result.status === 'cache_hit') {
          ...
          if (result.lat != null) setField(lot, '_lat', result.lat, 'os-places');
          if (result.lng != null) setField(lot, '_lng', result.lng, 'os-places');
          if (result.classificationCode) setField(lot, '_osClassification', result.classificationCode, 'os-places');
        }
      } catch (err) {
