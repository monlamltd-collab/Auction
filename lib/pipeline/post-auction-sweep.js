...
async function persistOutcome(lot, outcome) {
  ...
  if (flips.includes(outcome.status) && outcome.newStatus && outcome.newStatus !== outcome.oldStatus) {
    ...
    // Keep price_status coherent with the flip — without this, a swept-to-sold
    // lot keeps price_status='guide' until its next catalogue upsert, leaving
    // two data regimes (backfilled rows 'sold', sweep-flipped rows 'guide').
    // Mirrors derivePriceStatus's status keying (lib/quality/lot-quality.js).
    if (outcome.newStatus === 'sold' || outcome.newStatus === 'unsold') update.price_status = 'sold';
    else if (outcome.newStatus === 'withdrawn') update.price_status = 'withdrawn';
  }
  ...
  // ── Dual-write: emit lot_status_changed when the sweep actually flipped status ──
  // Only fires when `update.status` was set above (real flip detected by the
  // source HTML or the url_dead inference). No event for no_change /
  // fetch_failed — they don't mutate state. Non-fatal: lot_events is
  // observability infrastructure, never blocks the sweep.
  if (update.status && update.status !== outcome.oldStatus) {
    try {
      const ev = buildLotEvent({
        lotId: lot.id,
        eventType: LOT_EVENT_TYPES.STATUS_CHANGED,
        oldValue: { status: outcome.oldStatus ?? null },
        newValue: { status: update.status },
        source: {
          scrape_id: null,
          scraper_version: 'post-auction-sweep',
          house: lot.house,
          writer: 'post-auction-sweep.persistOutcome',
        },
      });
      if (ev) await insertLotEvents([ev]);
    } catch (e) {
      log.warn('post-auction-sweep: lot_events emission failed', { id: lot.id, err: e.message });
    }
  }
}
