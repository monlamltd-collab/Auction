...
//       attemptFn(row) -> Promise<'ok' | 'retry' | 'give_up'>.
//       'ok'      → row deleted.
 ...
 * @param {Function} opts.attemptFn — async (row) → 'ok' | 'retry' | 'give_up'
 * @returns {Promise<{ attempted: number, ok: number, retried: number, gaveUp: number }>}
 */
export async function drainRetryQueue(supabase, opts = {}) {
  const { limit = 50, attemptFn } = opts;
  if (!supabase || typeof attemptFn !== 'function') {
    return { attempted: 0, ok: 0, retried: 0, gaveUp: 0 };
  }
  ...
  return { attempted: due.length, ok, retried, gaveUp };
}
...
// runtime mutability.
export const _internals = { MAX_ATTEMPTS, BASE_BACKOFF_MS, MAX_BACKOFF_MS, backoffMs, nextRetryAt };
