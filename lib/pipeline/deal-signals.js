...
// ── Vendor-motivation archetypes ─────────────────────────────────────────────
// Vendor type predicts reserve flexibility: mortgagee/receiver sales must
// sell, executors want speed over price. The "by order of" phrasings are
// legally standardised in UK auction particulars, so plain regex is reliable.
// PROBATE_RE / RECEIVERSHIP_RE mirror the executor/receivership booleans in
// lib/pipeline/scoring.js (which drive score + opps tags) — keep them in step.
const PROBATE_RE = /executor|probate|estate of|personal representative/;
const RECEIVERSHIP_RE = /receiver|receivership|administrator|liquidator|lpa receiver/;
const REPOSSESSION_RE = /repossess|mortgagees?\s+in\s+possession|by\s+order\s+of\s+the\s+mortgagees?|bank\s+sale/;
const PUBLIC_DISPOSAL_RE = /by\s+order\s+of\s+(?:the\s+)?(?:local\s+authority|council\b|housing\s+association|[\w\s]{0,30}(?:borough|city|county|district)\s+council)/;
...
export function detectDealSignals({ text, beds, propType, tenure, leaseLength, titleSplit }) {
  ...
  // Vendor motivation — repossession outranks receivership when both phrasings
  // appear (a mortgagee sale via LPA receivers is one archetype, not two).
  const repossession = REPOSSESSION_RE.test(t);
  if (PROBATE_RE.test(t)) signals.push('probate');
  if (!repossession && RECEIVERSHIP_RE.test(t)) signals.push('receivership');
  if (repossession) signals.push('repossession');
  if (PUBLIC_DISPOSAL_RE.test(t)) signals.push('public-sector-disposal');
