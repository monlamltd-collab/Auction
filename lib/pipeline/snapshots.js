 ...
 * @returns {Promise<{ written: boolean, hash: string|null, status: string|null, id: string|null }>}
 *   `id` is the inserted catalogue_snapshots row id (used by lot_events as
 *   source.scrape_id); null when the insert was skipped or failed.
 */
export async function writeSnapshot(supabase, args) {
  ...
  if (!auctionId) {
    return { written: false, hash: null, status: null, id: null };
  }
  ...
  try {
    // .select('id').single() so the inserted row's id is returned —
    // lot_events uses it as source.scrape_id to link an event back to
    // the exact scrape that produced it.
    ...
    if (error) {
      ...
      return { written: false, hash: contentHash, status, id: null };
    }
    return { written: true, hash: contentHash, status, id: data?.id || null };
  } catch (e) {
