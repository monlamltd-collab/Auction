...
export async function writeSnapshot(supabase, args) {
  const { auctionId, rows, extractedWith = null, scrapedWith = null, statusOverride = null } = args;
  if (!auctionId) {
  ...
  if (!status) {
    // Look up the latest prior snapshot's content_hash for this auction
    // so we can mark `unchanged` cleanly. Cheap (one row, indexed).
    try {
      const { data: prev } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      status = deriveScrapeStatus(contentHash, prev?.content_hash || null);
    } catch {
      status = 'full'; // non-fatal — default to full
    }
  }
