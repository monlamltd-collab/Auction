...
export function recogniseEigOasLotsFromMarkdown(markdown, todayIso) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    const end = i + 1 < ordered.length ? ordered[i + 1][1] : Math.min(markdown.length, start + 4000);
...
// ── William H Brown (Norwich) — hand-built static table, plain HTTP ──
// A 2000s-era XHTML page (no JS, no framework, 17KB): the Norwich branch lists
// ITS OWN lots out of the shared Sequence / Barnard Marcus sale. The house had no
// recogniser and leaned on the AI extractor, so it went dark when the AI quota
// died — the last good run (2026-07-11, 19 lots) aged past get_active_lots' 7-day
// freshness window while later runs emitted a single junk lot (page LOGO as the
// image, the catalogue URL as the detail URL) and, earlier, a fabricated lot built
// from the office address in the page footer.
//
// Card shape (turndown markdown) — photo cells and text cells are separate table
// rows, so the photo is bound by SHARED DETAIL URL, never by position:
//   [![](…/images/auctions/2026/july26/234.jpg)](…/auctions/28-july-2026/707599/)
//   …
//   **[Lot 234](…/auctions/28-july-2026/707599/)**\\
//   20, Purdy Way, NORWICH,\\
//   Norfolk,\\
//   NR11 6DH\\
//   Guide: £120,000
//
// The lot URL carries the sale date (`/auctions/28-july-2026/`), so each lot's
// auction_date is parsed per lot: a catalogue left up after the sale can never
// ship as live (the page has no "ended" badges of its own to rely on).
const WHB_MONTHS = {
  january: '01', february: '02', march: '03', april: '04', may: '05', june: '06',
  july: '07', august: '08', september: '09', october: '10', november: '11', december: '12',
};
// Lot anchor + photo link. Both keep the lot id in the same capture position as
// RECALL_SENTINELS.williamhbrownnorwich so recogniser and sentinel stay in lockstep.
const WHB_LOT_RE = /\*\*\[Lot\s+(\d+[A-Za-z]?)\]\((https?:\/\/(?:www\.)?barnardmarcusauctions\.co\.uk\/auctions\/([a-z0-9-]+)\/(\d{6,})\/?)\)\*\*/gi;
const WHB_IMG_LINK_RE = /\[!\[[^\]]*\]\((https?:\/\/[^\s)]+)\)\]\((https?:\/\/(?:www\.)?barnardmarcusauctions\.co\.uk\/auctions\/[a-z0-9-]+\/\d{6,}\/?)\)/gi;
// A status annotation sits on its own line inside the card. It must feed the
// status parse but never the address (a "SOLD PRIOR" line otherwise lands in it).
const WHB_STATUS_LINE_RE = /^(?:sold|withdrawn|unsold|not\s+sold|under\s+offer|sale\s+agreed|postponed)\b/i;

/**
 * William H Brown (Norwich) recogniser — returns ONLY lots whose sale date is
 * today or later, keyed by the Sequence lot id.
 *
 * @param {string} markdown - recognition markdown (htmlToRecognitionMarkdown)
 * @param {string} [todayIso] - 'YYYY-MM-DD' (injectable for tests; defaults today)
 * @returns {Map<string, object>} keyed by lot id, in the normaliseScrapedLot shape
 */
export function recogniseWilliamHBrownNorwichLotsFromMarkdown(markdown, todayIso) {
  const lots = new Map();
  if (!markdown || typeof markdown !== 'string') return lots;
  const today = todayIso || new Date().toISOString().slice(0, 10);
  // Address lines arrive as turndown `\\` hard-breaks; drop the backslashes and
  // collapse the comma runs the per-line commas would otherwise double up.
  const clean = (s) => String(s || '').replace(/\\+/g, '').replace(/\*\*/g, '')
    .replace(/\s+/g, ' ').replace(/\s*,\s*/g, ', ').replace(/,(?:\s*,)+/g, ',')
    .replace(/^[\s,]+|[\s,]+$/g, '').trim();

  // Photo → lot binding by shared detail URL. The layout interleaves spacer cells
  // and splits photos/text across table rows, so positional binding would smear
  // one lot's photo across its neighbours.
  const keyOf = (u) => String(u || '').replace(/\/+$/, '').toLowerCase();
  const imgByUrl = new Map();
  for (const im of markdown.matchAll(WHB_IMG_LINK_RE)) {
    if (/\/images\/general\//i.test(im[1])) continue; // logo / spacer / rule chrome
    const k = keyOf(im[2]);
    if (!imgByUrl.has(k)) imgByUrl.set(k, im[1]);
  }

  const ms = [...markdown.matchAll(WHB_LOT_RE)];
  for (let i = 0; i < ms.length; i++) {
    const m = ms[i];
    const id = m[4];
    if (lots.has(id)) continue;
    ...
    const dm = m[3].match(/^(\d{1,2})-([a-z]+)-(\d{4})$/i);
    const mo = dm ? WHB_MONTHS[dm[2].toLowerCase()] : null;
    const auction_date = (dm && mo) ? `${dm[3]}-${mo}-${String(dm[1]).padStart(2, '0')}` : '';
    if (auction_date && auction_date < today) continue; // past sale → never live

    const blockEnd = i + 1 < ms.length ? ms[i + 1].index : Math.min(markdown.length, m.index + 800);
    const block = markdown.slice(m.index + m[0].length, blockEnd);

    // Address = the `\\`-broken lines between the anchor and the `Guide:` line.
    // Stop at the first markdown link/image so a card without a guide price can
    // never absorb the next photo row.
    const addrLines = [];
    const cardLines = [];
    let guideLine = '';
    for (const rawLine of block.split('\n')) {
      const t = rawLine.replace(/\\+/g, '').trim();
      if (!t) { if (addrLines.length) break; else continue; }
      if (/^!\[|^\[|\]\(/.test(t)) break;
      cardLines.push(t);
      if (/^Guide\b/i.test(t)) { guideLine = t; break; }
      if (WHB_STATUS_LINE_RE.test(t)) continue; // status marker, not address
      addrLines.push(t);
    }
    const address = clean(addrLines.join(' '));
    if (!address || address.length < 6) continue;

    const pm = guideLine.match(/£\s?([\d,]{3,})/);
    const guide_price = pm ? `£${pm[1]}` : '';

    // Status — the page carries no badges today, but a SOLD / WITHDRAWN
    // annotation must never persist as `available` if the branch adds one.
    const flat = cardLines.join(' ');
    const lot_status = /\bwithdrawn\b/i.test(flat) ? 'withdrawn'
      : /\b(?:sold|under\s+offer|sale\s+agreed)\b/i.test(flat) ? 'sold'
      : /\b(?:unsold|not\s+sold)\b/i.test(flat) ? 'unsold'
      : 'available';
    ...
    lots.set(id, {
      ...
      bedrooms: null,
      tenure: '',
      image_url: imgByUrl.get(keyOf(m[2])) || '',
      detail_url: m[2],
      // The guide-price page carries no prose; the detail pass + narrative sweep
      // fill description/bullets from the Barnard Marcus lot page.
      description: '',
      bullets: [],
      lot_status,
      auction_date,
...
// ── Bagshaws Residential (Sequence / Connells group) — static HTML catalogue ──
// A hand-maintained table-layout page (no CMS, no JS) whose lot cards point at
// the group's shared national sale on barnardmarcusauctions.co.uk:
//   /auctions/{DD-month-YYYY}/{lotId}/
// The house had NO recogniser, so it depended entirely on the AI extractor:
// the last good AI pass (11 Jul 2026) got 22 of 23 and dropped the SOLD-PRIOR
// lot; every pass since produced ~1 junk row (address-less, url = the catalogue
// root), so the 22 real lots aged out of the 7-day freshness window in
// get_active_lots and the house went dark.
//
...
const BAGSHAWS_LOT_ANCHOR_RE = /\*\*\[\s*Lot\s+(\d+[A-Za-z]?)\s*\]\(([^)\s]*)\)\*\*/gi;
...
const BAGSHAWS_THUMB_RE = /\[!\[[^\]]*\]\((https?:\/\/[^\s)]+)\)\]\((https?:\/\/[^\s)]+)\)/g;
...
const BAGSHAWS_LOT_URL_RE = /\/auctions\/(\d{1,2}-[a-z]+-\d{4})\/(\d{3,})\/?(?:[?#]|$)/i;
const BAGSHAWS_STATUS_LINE_RE = /^\**\s*(sold|withdrawn|postponed|unsold|available)\b/i;
const BAGSHAWS_GUIDE_LINE_RE = /^\**\s*guide\b/i;

// '28-july-2026' → '2026-07-28'. '' when the slug isn't a real date.
function bagshawsSlugDate(slug) {
...
/**
 * Parse the Bagshaws Residential catalogue page into raw lots, keyed by the
 * Sequence lot id (the recall sentinel's capture group) so the Crawlee
 * recovery/corroboration path can match them. Falls back to `lot-{n}` when a
 * lot has no resolvable URL, so a lot is never silently dropped.
 *
 * @param {string} markdown
 * @returns {Map<string, object>}
 */
export function recogniseBagshawsLotsFromMarkdown(markdown) {
  const lots = new Map();
  if (!markdown || typeof markdown !== 'string') return lots;
  ...
  for (const m of markdown.matchAll(BAGSHAWS_THUMB_RE)) {
    const [, imgUrl, href] = m;
    const lm = BAGSHAWS_LOT_URL_RE.exec(href);
    if (!lm) continue;
  ...
  const pageAuctionDate = bagshawsSlugDate(pageDateSlug);

  const anchors = [...markdown.matchAll(BAGSHAWS_LOT_ANCHOR_RE)];
  for (let i = 0; i < anchors.length; i++) {
    const a = anchors[i];
    const lotNumber = a[1];
    ...
    const anchorOk = BAGSHAWS_LOT_URL_RE.test(a[2] || '');
    const detailUrl = anchorOk ? a[2] : (thumb && BAGSHAWS_LOT_URL_RE.test(thumb.href) ? thumb.href : '');
    const lm = detailUrl ? BAGSHAWS_LOT_URL_RE.exec(detailUrl) : null;
    ...
    for (const line of lines) {
      if (!guideLine && BAGSHAWS_GUIDE_LINE_RE.test(line)) { guideLine = line; continue; }
      if (!statusLine && BAGSHAWS_STATUS_LINE_RE.test(line)) { statusLine = line; continue; }
      if (!guideLine && !statusLine) addressLines.push(line);
    }
    ...
    const lot_status = /withdrawn|postponed/.test(st) ? 'withdrawn'
      : /\bsold\b/.test(st) ? 'sold'
        : /\bunsold\b/.test(st) ? 'unsold'
          : 'available';

    const slugDate = lm ? bagshawsSlugDate(lm[1]) : '';
    lots.set(id, {
      lot_number: lotNumber,
      address,
      guide_price,
      ...
      detail_url: detailUrl,
      description: '',
      bullets: statusLine ? [clean(statusLine)] : [],
      auction_date: slugDate || pageAuctionDate,
      lot_status,
    });
  }
  return lots;
