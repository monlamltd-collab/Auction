...
// ⚠ DEPRECATED — superseded by `normaliseScrapedLot` in lib/types/lot.js
// ─────────────────────────────────────────────────────────────────────────
// `normaliseLot` emits a transitional camelCase shape (lotNumber, priceStr,
// lotStatus, auctionDate without underscore, house/catalogueUrl without
// underscore) that downstream consumers then re-translate. The canonical
// version in lib/types/lot.js emits the final app-side shape directly,
// collapsing two transformation steps into one. New code MUST use:
//
//   import { normaliseScrapedLot } from '../types/lot.js';
//
// Callers will be migrated in Task 3 of the lot-contract consolidation.
// Until then, this function is kept verbatim — DO NOT change its output
// shape without updating every caller.
//
...
// counter for logging.
//
// Bullets policy: prefer a recogniser-supplied `raw.bullets` array verbatim
// (Maggs's recogniser populates 5–7 per lot; future recognisers should too).
// Fall back to wrapping `raw.description` in a single-element array when only
// the JSON extractor ran (its schema only has `description`, no bullets). All
// downstream consumers — validation.js status/lease checks, persist-lots
// search-text join, scoring.js signal regex — treat bullets as Array<string>
// via `.join(...)`, so multi-element arrays are strictly richer signal.
export function normaliseLot(raw, house, catalogueUrl) {
