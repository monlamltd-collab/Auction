 ...
 *
 * Past-dated lots are returned WITH their real past date rather than dropped:
 * dropping them would read as a recall shortfall against the sentinel (which
 * counts every lot id on the page), and liveness is already enforced one layer
 * down — get_active_lots admits `available` lots only while
 * `auction_date >= current_date - 1`. Reporting the true date is what makes
 * these houses expire at all; their auction_calendar row is a 2099-12-31
 * always_on placeholder that would otherwise keep every lot live forever.
 *
 * @param {string} markdown
 * @returns {Map<string, object>}
 */
export function recogniseSequenceBranchLotsFromMarkdown(markdown) {
  const lots = new Map();
  if (!markdown || typeof markdown !== 'string') return lots;
