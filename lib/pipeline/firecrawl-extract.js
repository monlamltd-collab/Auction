...
// Purplebricks / GOTO Properties is an EIG OAS archive. Its source moved from
// numeric IDs to UUIDs and mixes ~2,700 ended cards with the current catalogue.
// Keep the house-specific export for registry compatibility, but use the shared
// affirmative-live EIG recogniser so ended cards never reach persistence and
// live cards carry their real end date.
export function recognisePurplebricksGotoLotsFromMarkdown(markdown, todayIso) {
  return recogniseEigOasLotsFromMarkdown(markdown, todayIso);
}
...
// Markdown image wrapped in its lot-detail link. Unlike positional card windows,
// this preserves the source's explicit image→identity relationship across EIG
// themes that place galleries before OR after metadata.
const EIG_LINKED_IMG_RE = /\[!\[[^\]]*\]\((https:\/\/cdn\.eigpropertyauctions\.co\.uk\/[^)\n]+)\)\s*\]\(https?:\/\/[^)]*?\/lot\/details\/([A-Za-z0-9][A-Za-z0-9-]{2,})[^)]*\)/g;
...
const EIG_ENDED_RE = /Auction Ended|Committed Offer Accepted|Committed Offers Closed|View Result|Result:[\s#*]*(?:Sold|Unsold|Withdrawn|Postponed|Not\s*Sold)|\b(?:Under Offer|Sale Agreed)\b/i;
...
export function recogniseEigOasLotsFromMarkdown(markdown, todayIso) {
  ...
  // Bind hero photos by the detail-link identity published around each image.
  // First source image wins, matching the gallery's hero order.
  const linkedImageById = new Map();
  for (const m of markdown.matchAll(EIG_LINKED_IMG_RE)) {
    const image = m[1].split('?')[0];
    if (!linkedImageById.has(m[2]) && /\/auction\/\d+\/\d+_web/i.test(image)
      && !/logo|\/oas\/|\.svg/i.test(image)) linkedImageById.set(m[2], image);
  }
  ...
  // EIG themes either label cards "Lot N" or use the status line itself as the
  // heading. Recognising both prevents the following card's date/status from
  // bleeding backwards when the first anchor in a card is its gallery image.
  const headerIdxs = [...markdown.matchAll(
    /^#{1,6}\s*(?:Lot\s+\d+\b|End Time\b|Auction Ended\b|Committed Offer(?:s)?\s+(?:Accepted|Invited|Closed)\b)[^\n]*$/gim,
  )].map(m => m.index);
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    const hasLiveToken = /\b(?:Available|Committed Offers Invited|Buy\s*it\s*Now|Guide\s*Price|Current\s*Bid|Starting\s*Bid|Opening\s*Bid|For\s*Sale|End\s*Time|Auction\s*Ends|Available\s*Until|Bidding\s*Opens?)\b/i.test(flat);
    if (!hasFutureDate && !hasLiveToken && !guide_price) continue;

    // Image — prefer the source's explicit linked image→lot identity. Positional
    // windows remain a fail-open fallback for malformed/minified markdown where
    // turndown loses the outer link, but can never override an exact binding.
    const isPhoto = (u) => /\/auction\/\d+\/\d+_web/i.test(u) && !/logo|\/oas\/|\.svg/i.test(u);
    let image_url = linkedImageById.get(id) || '';
    if (!image_url) for (const im of block.matchAll(EIG_CDN_IMG_RE)) { if (isPhoto(im[1])) { image_url = im[1].split('?')[0]; break; } }
    if (!image_url) for (const im of backWindow.matchAll(EIG_CDN_IMG_RE)) { if (isPhoto(im[1])) { image_url = im[1].split('?')[0]; break; } }
    ...
    const headingDescriptor = [...block.matchAll(/^#{4,6}\s+\*{2}(.+?)\*{2}\s*$/gm)]
      .map(m => clean(m[1]))
      .find(s => s && !/^(?:Guide|Starting|Opening|Current|Minimum|Result)\b/i.test(s)) || '';
    // Paul Fosh's useful card summary is plain text ("Lot 12 - ..."), not a
    // markdown heading. Keep the descriptive suffix but reject status/timing
    // labels such as "Lot 12 - Auction starts" and "Auction Ended".
    const plainLotDescriptor = block.split('\n').map(clean)
      .map(line => /^Lot\s+\d+\s*[-|:]\s*(.+)$/i.exec(line)?.[1] || '')
      .find(s => s && !/\b(?:Auction\s+(?:starts?|ended|ends?)|End\s+Time|Result|Upcoming)\b/i.test(s)) || '';
    const descriptor = headingDescriptor || plainLotDescriptor;
    const listBullets = [...block.matchAll(/^[-*]\s+(.+)$/gm)]
      .map(b => clean(b[1])).filter(b => b && b.length > 2 && !/^\[/.test(b));
    const bullets = [...new Set([descriptor, ...listBullets].filter(Boolean))].slice(0, 8);
    ...
    lots.set(id, {
      ...
      image_url,
      detail_url,
      description: descriptor || bullets[0] || '',
      bullets,
      lot_status: 'available',
      auction_date: auctionDate || '',
    });
