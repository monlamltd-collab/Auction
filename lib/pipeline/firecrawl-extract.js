...
export function recogniseBtgEddisonsLotsFromMarkdown(markdown) {
  ...
  while ((m = BTG_CARD_RE.exec(markdown)) !== null) {
    ...
    const detailUrl = m[2];
    const id = m[3];
    if (!id || lots.has(id) || !address || address.length < 6) continue;
    ...
    // Guide price ("Guide Price: £595,000+") renders inside the card.
    const priceM = card.match(/Guide\s*Price:?\s*£\s*([\d,]+)/i);
    const guide_price = priceM ? `£${priceM[1]}` : '';
