...
export async function extractCatalogueListing(url, house, options = {}) {
  ...
  if (maxPages === 1) {
    ...
    pageResults[0] = await scrapePage(pageUrls[0]);
    if (changeTracking && pageResults[0]?.changeStatus === 'same' && !forceExtract) {
      console.log(`AUTO: ${house} unchanged (changeTracking on page 1) — skipping ${pageUrls.length - 1} more pages`);
      return { skipped: true, reason: 'unchanged', changeStatus: 'same', lots: [] };
    }

    // Page 1 changed (or changeTracking off) — fan out to pages 2..N.
    let nextIdx = 1;
    const worker = async () => {
      while (true) {
        const i = nextIdx++;
        if (i >= pageUrls.length) return;
        pageResults[i] = await scrapePage(pageUrls[i]);
      }
    };
    const workers = Array.from(
      { length: Math.min(maxConcurrency, pageUrls.length - 1) },
      worker,
    );
