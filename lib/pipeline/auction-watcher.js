...
Find the URL of the NEXT UPCOMING auction catalogue (the one bidders would click to see all lots for the next auction event). Return a JSON array of up to 3 upcoming auctions, newest-first. Each entry MUST have:
  - url: absolute URL of the catalogue page
  - date: ISO date (YYYY-MM-DD) of the auction if visible, or null
...
${stripped}`;
  ...
  let aiText = '';
  try {
    aiText = await _callAI(prompt, { tier: 'capable' });
  } catch (e) {
...
export async function watchOne(slug, opts = {}) {
  ...
  // Tier 2 — AI (only if Tier 1 returned nothing with a future date)
  ...
  // Upsert the best (future-dated, earliest) — or the first one if none dated
  const future = entries.filter(e => isFuture(e.date));
  const picked = future[0] || entries[0];
