...
import { discoverAllsopAuctionEntries } from './allsop-auctions-api.js';
import { discoverLoveittsAuctionEntries } from './loveitts-auctions.js';
import {
...
import {
  ...
  markWatcherHandled,
  setLastWatcherSummary,
  setWatcherHandledSlugs,
} from './cycle-coordination.js';
...
function isFuture(isoDate) {
  return isFutureIsoDate(isoDate);
}
...
// still (url, date). Most houses retain the legacy 30-day fallback for undated
// "current lots" links. Configs such as McHugh explicitly disable it because a
// rolling page can expose stale lots before the next dated catalogue is ready.
async function upsertCalendarEntry(slug, entry, opts = {}) {
  ...
  const allowDateFallback = opts.config?.allowDateFallback !== false;
  const res = await upsertUpcomingCatalogue(supabase, {
    ...
    allowDateFallback,
    fallbackDate: allowDateFallback ? fallbackDate : null,
...
export async function fetchUpcomingHorizon(slug, opts = {}) {
  ...
  try {
    const nearReadyDays = opts.nearReadyDays
      ?? Number(process.env.WATCHER_NEAR_READY_DAYS || DEFAULT_NEAR_READY_DAYS);
    const staleNotReadyDays = opts.staleNotReadyDays
      ?? Number(process.env.WATCHER_STALE_NOT_READY_DAYS || DEFAULT_STALE_NOT_READY_DAYS);
    const horizonOpts = { todayIso: today, horizonDays, nearReadyDays, staleNotReadyDays };
    const { data, error } = await supabase
      .from('auction_calendar')
      .select('url, date, catalogue_ready, status, updated_at')
      .eq('house_slug', slug)
      .in('status', ['upcoming', 'always_on']);
    if (error) {
      // Fail open → refresh so we don't silently stop discovering
      return getUpcomingHorizon([], horizonOpts);
    }
    return getUpcomingHorizon(data || [], horizonOpts);
  } catch {
    return getUpcomingHorizon([], {
      todayIso: today,
      horizonDays,
      nearReadyDays: opts.nearReadyDays
        ?? Number(process.env.WATCHER_NEAR_READY_DAYS || DEFAULT_NEAR_READY_DAYS),
      staleNotReadyDays: opts.staleNotReadyDays
        ?? Number(process.env.WATCHER_STALE_NOT_READY_DAYS || DEFAULT_STALE_NOT_READY_DAYS),
    });
  }
}
 ...
 */
function pageMatchesCandidateDate(html, isoDate) {
  if (!html || !/^\d{4}-\d{2}-\d{2}$/.test(isoDate || '')) return false;
  ...
  return new RegExp(`(?:\\b${year}-${mm}-${dd}\\b|\\b${d}(?:st|nd|rd|th)?\\s+${monthName}\\s+${year}\\b|\\b${monthName}\\s+${d}(?:st|nd|rd|th)?(?:,)?\\s+${year}\\b)`, 'i').test(text);
}

export async function filterAiCandidates(slug, entries, fetchHtml = _fetchHtmlNonFc, config = AUCTION_DISCOVERY[slug] || {}) {
  const out = [];
  for (const e of entries) {
    ...
    try {
      ...
      if (config.requireCandidateDateVerification && !pageMatchesCandidateDate(html, e.date)) {
        log.info('auction-watcher AI candidate rejected', { slug, url: e.url, reason: 'candidate-date-not-on-page' });
        continue;
      }
      out.push(e);
    } catch (error) {
      // Readiness is a positive assertion. A failed verification must not turn
      // a pre-publication shell into a schedulable catalogue.
      log.warn('auction-watcher AI candidate verification failed', { slug, url: e.url, error: error.message });
    }
  }
  return out;
}
...
export async function watchOne(slug, opts = {}) {
  ...
  // Allsop: high-confidence auctions JSON (residential + commercial next sales).
  // Homepage has no catalogue linkPattern; property-search is continuous inventory.
  if (entries.length === 0 && slug === 'allsop') {
    try {
      const allsopEntries = await discoverAllsopAuctionEntries({
        todayIso: opts.todayIso || new Date().toISOString().slice(0, 10),
        headers: HEADERS,
      });
      if (allsopEntries.length > 0) {
        entries = allsopEntries;
        tier = 'allsop-auctions-api';
      }
    } catch (e) {
      log.warn('auction-watcher allsop API failed', { error: e.message });
    }
  }

  // Loveitts: multi-sale dates from loveitts.co.uk catalogue select + upcoming page.
  // Not the EIG live-stream embed (no lots; previously seeded 2099 junk).
  if (entries.length === 0 && slug === 'loveitts') {
    try {
      const loveittsEntries = await discoverLoveittsAuctionEntries({
        todayIso: opts.todayIso || new Date().toISOString().slice(0, 10),
        headers: HEADERS,
      });
      if (loveittsEntries.length > 0) {
        entries = loveittsEntries;
        tier = 'loveitts-auctions-html';
      }
    } catch (e) {
      log.warn('auction-watcher loveitts HTML failed', { error: e.message });
    }
  }
  ...
  if (!hasFuture && !opts.skipAi) {
    ...
    if (aiEntries.length > 0) {
      entries = await filterAiCandidates(slug, aiEntries, _fetchHtmlNonFc, config);
      tier = 'ai';
    }
  }
  ...
  for (const picked of picks) {
    const res = await upsertCalendarEntry(slug, picked, { ...opts, config });
  ...
  if (ok && !opts.dryRun) markWatcherHandled(slug);
...
export async function watchAuctionCalendar(opts = {}) {
  ...
  if (!opts.keepCoordination && !opts.dryRun) clearWatcherHandledSlugs();

  // Global lifecycle cleanup before any horizon or scheduling decision. This is
  // deliberately limited to past `upcoming` rows; always_on/MMOA is untouched.
  try {
    ...
    log.warn('auction-watcher global retirePast failed', { error: e.message });
  }

  if (slugs.length === 0) {
    log.info('auction-watcher: no houses configured, nothing to do');
    const empty = { slugs: 0, results: [] };
    setLastWatcherSummary(empty);
    return empty;
  }
  ...
  const handled = results.filter(r => r.upserted).map(r => r.slug).filter(Boolean);
  if (!opts.dryRun) setWatcherHandledSlugs(handled);
  ...
  const summary = {
    ...
    handledSlugs: handled,
    results,
  };
  setLastWatcherSummary(summary);
  return summary;
}
