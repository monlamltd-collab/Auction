...
let _agentExtract = null;
let _fireAlert = null;
let _budget = null;

export function initWatcher({ scrapeWithFirecrawl, agentExtract, fireAlert, budget }) {
  _scrapeWithFirecrawl = scrapeWithFirecrawl;
  _agentExtract = agentExtract || defaultAgentExtract;
...
async function discoverViaProbe(slug, config) {
  ...
  if (config.auctionsIndexPath) {
    ...
    if (idxUrl) {
      ...
      // Escalate to Firecrawl when plain HTTP found nothing new from the index
      // page — EIG white-label (and similar JS-rendered sites) don't expose
      // auction links in static HTML. One Firecrawl credit is cheap insurance.
      if (candidateIds.size === beforeIdx && _scrapeWithFirecrawl && _budget?.canUseFirecrawl()) {
        try {
          const result = await _scrapeWithFirecrawl(idxUrl, { formats: ['rawHtml'] });
...
Return the array of auctions soonest-first via the structured schema.`;
  ...
  let data;
  try {
    data = await _agentExtract(homepage, prompt, DISCOVER_SCHEMA, { timeout: WATCHER_AGENT_TIMEOUT_MS });
  } catch (e) {
