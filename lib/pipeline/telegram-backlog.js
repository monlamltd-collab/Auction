...
import { scoreCatalogueUrlPath } from './catalogue-candidate.js';
...
export function buildBacklogCardForAlert(alert) {
  ...
  if (hasCandidateUrl) {
    const path = scoreCatalogueUrlPath(candidate);
    lines.push(`Candidate: ${escapeHtml(candidate)}`);
    if (path.tags?.length) lines.push(`<i>score ${path.score} · ${escapeHtml(path.tags.join(', '))}${path.reject ? ' · REJECT ' + escapeHtml(path.rejectReason || '') : ''}</i>`);
  }
  ...
  if (slug && ['house_homepage_unreachable', 'house_no_catalogue_found', 'house_no_longer_auction', 'relocation_needed'].includes(alert.event_type)) {
    row2.unshift({ label: '🏚 Retire', callback_data: `retire:${alert.id}` });
  }
