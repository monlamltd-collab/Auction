...
//
//   accept:<alertId>   — verify + apply the candidate URL in alert.meta
...
//   retire:<alertId>   — set house dormant + resolve open alerts for slug
//
...
import { HOUSE_ROOTS, HOUSE_DISPLAY_NAMES, detectAuctionHouse, RETIRED_HOUSES } from '../houses.js';
import { normaliseUrl } from '../utils.js';
import { validateUrl } from '../security.js';
import { clearHealingCooldown, _commitMerger } from './healing.js';
import { assessCatalogueCandidate, scoreCatalogueUrlPath, sameCatalogueUrl } from './catalogue-candidate.js';
...
export async function handleCallbackData(callbackData) {
  ...
  switch (verb) {
    ...
    case 'retire':  return _retire(alert);
...
// ── accept: verify then apply candidate URL ──
async function _accept(alert) {
  const slug = alert.house;
  if (!slug) return { ok: false, summary: 'alert has no house slug' };
  if (RETIRED_HOUSES.has(String(slug).toLowerCase())) {
    return { ok: false, summary: 'house is retired — use dismiss' };
  }

  const meta = alert.meta || {};
  const newUrl = meta.to || meta.candidate_url || meta.new_url || meta.newUrl;
  if (!newUrl || typeof newUrl !== 'string' || !/^https?:\/\//i.test(newUrl.trim())) {
  ...
  // Path precheck (fast) — reject past/pdf/junk before any apply.
  const path = scoreCatalogueUrlPath(newUrl);
  if (path.reject) {
    return { ok: false, summary: `candidate rejected: ${path.rejectReason} (${path.tags.join(',')})` };
  }

  if (sameCatalogueUrl(newUrl, HOUSE_ROOTS[slug] || '')) {
    await _resolve(alert.id);
    return { ok: true, summary: `already on ${newUrl}` };
  }

  // Soft HTML verify when fetch available — never apply path-rejected targets.
  let assessment = assessCatalogueCandidate(newUrl, null, slug);
  try {
    const { fetchPage } = await import('../scraper/http.js');
    const html = await fetchPage(newUrl);
    assessment = assessCatalogueCandidate(newUrl, html || '', slug);
  } catch (e) {
    // Network fail: still allow if path score is strong (>=3) — human tapped Apply.
    if (path.score < 3) {
      return { ok: false, summary: `could not verify candidate (${e.message || 'fetch failed'})` };
    }
    assessment = { ok: true, score: path.score, lots: 0, reason: 'path-only-trusted', tags: path.tags, summary: 'path-only' };
  }
  if (!assessment.ok) {
    return { ok: false, summary: `candidate failed verify: ${assessment.summary}` };
  }

  HOUSE_ROOTS[slug] = newUrl;
  await _writeCalendarUrl(slug, oldUrl, newUrl);
  ...
  try {
    await supabase.from('pipeline_alerts').insert({
      ...
      severity: 'info',
      house: slug,
      message: `Simon accepted candidate URL via Telegram: ${oldUrl || '?'} → ${newUrl} [${assessment.summary}]`,
      meta: { source_alert: alert.id, old_url: oldUrl, new_url: newUrl, assessment },
    });
  } catch { /* silent */ }
  await _resolve(alert.id);
  ...
  return {
    ok: true,
    summary: `applied → ${HOUSE_DISPLAY_NAMES[slug] || slug} (${assessment.summary})`,
    rescrape: { slug, url: newUrl },
  };
}

// ── retire: mark house dormant + resolve all open alerts for slug ──
async function _retire(alert) {
  const slug = alert.house;
  if (!slug) return { ok: false, summary: 'alert has no house slug' };
  const display = HOUSE_DISPLAY_NAMES[slug] || slug;

  // Code-list membership is ideal but process-local RETIRED_HOUSES needs a PR.
  // DB dormant + blocked-style fields stop scrapes immediately.
  if (supabase) {
    try {
      await supabase.from('house_skills').upsert({
        ...
        circuit_state: 'open',
        health_score: 0,
        healing_cooldown_until: new Date(Date.now() + 365 * 86400000).toISOString(),
        last_probe_result: 'retired_via_telegram',
      }, { onConflict: 'slug' });
    } catch (e) {
      return { ok: false, summary: `retire write failed: ${e.message || e}` };
    }
    try {
      await supabase.from('pipeline_alerts')
        .update({
          ...
          meta: { retired_via: 'telegram', source_alert: alert.id },
        })
        .eq('house', slug)
        .eq('resolved', false);
    } catch { /* best-effort */ }
    try {
      await supabase.from('pipeline_alerts').insert({
        event_type: 'house_retired_by_human',
        severity: 'info',
        house: slug,
        message: `Retired ${display} via Telegram (set dormant; add to RETIRED_HOUSES in code when convenient)`,
        meta: { source_alert: alert.id, via: 'telegram' },
        resolved: true,
        resolved_at: new Date().toISOString(),
      });
    } catch { /* silent */ }
  ...
  return { ok: true, summary: `retired ${display} (dormant) — add '${slug}' to RETIRED_HOUSES in code later` };
}
...
async function _applyAsCatalogue(alert, slug, url) {
  ...
  if (path.reject && ['pdf', 'single-lot-url', 'junk-url', 'non-property', 'past-archive'].includes(path.rejectReason)) {
    return { ok: false, summary: `that URL looks wrong (${path.rejectReason}) — try a current catalogue listing URL` };
  }
...
export const _internal = {
  _accept, _snooze, _rerun, _dismiss, _retire, _loadAlert,
