...
// Houses with multiple calendar rows (sdl, etc) get no fallback —
// the join only succeeds via direct URL match.
const CAL_CACHE_TTL_MS = 5 * 60 * 1000;
let _calCache = null;          // { urlMap: Map<normalisedUrl,{date,id}>, houseMap: Map<house_slug, [{date,id}]> }
 ...
 *  2. House-only fallback when the house has EXACTLY ONE calendar row.
 *     Safe attribution: there's nothing else this lot could belong to.
 *     This catches the scraper-side url-mismatch cases (landwood etc)
 *     without needing to track every scraper redirect.
 *
 * Houses with multiple calendar rows (sdl umbrella, scheduled-date houses)
 * get NO fallback — the writer leaves auction_id NULL when the URL
 * doesn't match, since attributing to an arbitrary row would mis-tag.
 *
