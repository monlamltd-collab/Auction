...
import { isPlaceholderUrl } from '../scraper/validation.js';
...
function derivePriceStatus(lot) {
  ...
  // Nil Reserve — a lot offered with NO reserve (sells to the highest bid).
  // It's a real, investor-POSITIVE state, not a missing price: counting it as
  // a coverage gap is the bug behind the "No Guide Price" false alarms
  // (pugh, 2026-06-12). Only when there's no numeric guide — "Guide £X, Nil
  // Reserve" keeps 'guide' (the number wins; the nil-reserve badge is derived
  // separately for display).
  if (!hasPrice && priceText && /\b(?:nil|no|without|zero)\s*reserve|unreserved\b/i.test(priceText)) return 'nil_reserve';

  if (priceText && /poa|on application|guide on application/i.test(priceText) && !hasPrice) return 'poa';
...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    // were quarantined from production on 2026-06-12. Placeholder image URLs
    // are nulled (the lot may be real; the photo is invented).
    const beforePlaceholder = enrichedLots.length;
    enrichedLots = enrichedLots.filter(l => !isPlaceholderUrl(l?.url));
    if (enrichedLots.length < beforePlaceholder) {
      console.warn(`LOTS: ${house}: dropped ${beforePlaceholder - enrichedLots.length} fabricated lot(s) with placeholder-domain URLs`);
    }
    ...
    for (const lot of enrichedLots) {
      ...
      rows.push({
        ...
        // Structured pricing intent. derivePriceStatus was defined but never
        // wired in — so price_status went stale after the one-time April
        // backfill and every lot since carried a NULL/heuristic status. Now
        // populated on every upsert (incl. nil_reserve), which is what stops
        // the quality scanner counting Nil Reserve / POA / TBA as gaps.
        price_status: derivePriceStatus(lot),
