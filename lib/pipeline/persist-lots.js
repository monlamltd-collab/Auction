...
// key doesn't collide). Pure — returns a new array. A repeated URL takes its
// LAST position (keep-last), so the result is ordered by last occurrence, not
// first; immaterial downstream, which consumes only the URL set + counts.
export function dedupeRowsByUrl(rows) {
  const seen = new Set();
  const out = [];
  for (let i = rows.length - 1; i >= 0; i--) {
    const key = rows[i] && rows[i].url;
    if (key) {
...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    for (const lot of enrichedLots) {
      ...
      const lotAuctionDate = bulletDate || lot._auctionDate || catalogueAuctionDate;
