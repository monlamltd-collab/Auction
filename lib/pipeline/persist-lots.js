...
let _calCache = null;          // Map<normalisedUrl, { date, id }>
...
async function getCalendarDateMap() {
  ...
  try {
    ...
    for (const r of data || []) {
      ...
      // we use Map.has() guard. Use it. We carry `id` alongside `date` so
      // upserts can stamp lots.auction_id without a second round-trip.
      if (k && !map.has(k)) map.set(k, { date: r.date, id: r.id });
    }
...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    // Backed by an in-process URL→{date, id} Map with 5-min TTL — see
    // getCalendarDateMap() at the top of this module. auction_id is stamped
    // on the row payload below so lots can be joined to auction_calendar by
    // FK rather than by the fragile (house, catalogue_url) string pair.
    let catalogueAuctionDate = null;
    let catalogueAuctionId = null;
    try {
      const calMap = await getCalendarDateMap();
      const entry = calMap.get(normaliseUrl(catalogueUrl)) || null;
