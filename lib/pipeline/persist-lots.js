  ...
  try {
    ...
    for (const lot of enrichedLots) {
      ...
      rows.push({
        ...
        last_seen_at: now,
        lat: (typeof lot._lat === 'number') ? lot._lat : null,
        lng: (typeof lot._lng === 'number') ? lot._lng : null,
    ...
    for (const row of rows) {
      ...
      if (existing.field_sources && typeof existing.field_sources === 'object') {
        row.field_sources = { ...existing.field_sources, ...(row.field_sources || {}) };
      }
    ...
    try {
      ...
      if (persistedRows && persistedRows.length > 0) {
        ...
        for (const r of persistedRows) {
          ...
          const snapshotHash = createHash('sha1').update(fingerprint).digest('hex').slice(0, 16);
