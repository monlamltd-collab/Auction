...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    // Move 3 Phase 3a: persist a catalogue_snapshots row capturing the
    // canonical URL set for this scrape. Best-effort — failure is logged
    // but never propagates. Skipped when auction_id is unresolved (the
    // url_mismatch cohort from Move 2) since snapshots are auction-keyed.
