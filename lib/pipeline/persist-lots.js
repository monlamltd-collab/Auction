...
import { isPlaceholderUrl, detectFabricatedBatch } from '../scraper/validation.js';
...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    if (keptLots.length < enrichedLots.length) {
      console.warn(`LOTS: ${house}: dropped ${enrichedLots.length - keptLots.length} fabricated lot(s) with placeholder-domain URLs`);
    }
