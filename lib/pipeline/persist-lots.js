...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    try {
      ...
      // price_status is included so the lot_events diff (below) can detect
      // price_status transitions like poa → guide that don't touch `status`.
      const { data: persistedRows } = await supabase
        .from('lots')
        .select('id, url, price, status, bullets, image_url, sold_price, price_text, price_status')
        .eq('house', house)
      ...
      if (persistedRows && persistedRows.length > 0) {
        ...
        // ── Dual-write: emit lot_events for the same persistedRows ──
        // Runs alongside lot_history (above) and lot_status_history (line
        // ~620) during the consumer migration window. Per-field events —
        // lot_status_changed, lot_price_changed, lot_sold_price_set,
        // lot_price_status_changed — let consumers slice without re-deriving
        // diffs across snapshots. lot_first_seen captures the snapshot for
        // brand-new URLs. Failures non-fatal: lot_events is observability,
        ...
        try {
          ...
          console.warn(`LOTS: lot_events emission failed for ${house}: ${eventsErr.message}`);
        }
      }
    } catch (err) {
    ...
    //
    // Two code paths:
    //   1. Snapshot-diff (Move 3 Phase 3b) — when a prevSnapshot exists for
    //      this auction. Authoritative: prev.lot_url_set vs current.lot_url_set
    //      gives an exact vanished set, no heuristics over the live lots table.
    //   2. Legacy (selectPruneCandidates) — fallback when no prevSnapshot is
    //      available (brand-new auction, or first scrape after Phase 3a deploy).
    //      Same behaviour as pre-Move-3.
    //
    // Both produce identical I/O downstream (UPDATE lots SET status='withdrawn').
    // Once snapshots have rolled out for ~a week, the legacy fallback is
    // mostly dead code and can be retired.
    try {
      let candidates, ratio, blockedByRatio, prevCount, scrapedCount, regressionReason;
      if (prevSnapshot && prevSnapshot.lot_count > 0 && Array.isArray(prevSnapshot.lot_url_set)) {
        ...
        blockedByRatio = reg.severe && candidates.length > 0;
        regressionReason = reg.reason;
      } else {
        const scrapedUrls = new Set(rows.map(r => r.url));
        const r = selectPruneCandidates(existingLots || [], scrapedUrls, now);
        candidates = r.candidates;
        ratio = r.ratio;
        blockedByRatio = r.blockedByRatio;
        prevCount = r.prevCount;
        scrapedCount = r.scrapedCount;
        regressionReason = blockedByRatio ? 'legacy_ratio_gate' : null;
      }
      if (blockedByRatio) {
        ...
        await supabase.from('pipeline_alerts').insert({
          ...
          meta: { ratio, prevCount, scrapedCount, candidateCount: candidates.length, catalogue_url: normaliseUrl(catalogueUrl), source: prevSnapshot ? 'snapshot_diff' : 'legacy', reason: regressionReason },
        });
        ...
        if (pruned > 0) {
          ...
          // ── Dual-write: emit lot_events for prune-vanished ──
