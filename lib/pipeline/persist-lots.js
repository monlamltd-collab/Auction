  ...
  try {
    ...
    for (const lot of enrichedLots) {
      ...
      rows.push({
        ...
        // Same self-URL guard as image_url — a floor plan that points back to the
        // detail page is an LLM fallback, not a real plan; drop it so backfill
        // can retry later rather than persist HTML behind an image fetch.
        floor_plan_url: (lot.floorPlanUrl && lot.floorPlanUrl === lot.url) ? null : (lot.floorPlanUrl || null),
