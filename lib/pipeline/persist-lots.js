...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    for (const lot of enrichedLots) {
      ...
      // A lot whose only URL is the catalogue page has no real identity. The
      // enrichment fallback used to stamp the catalogue URL onto every urlless
      // lot; because lots.url is UNIQUE, a whole degraded batch collapsed onto
      // ONE page-identity row that was then served as a fake 'available' lot
      // (2026-07-22). Treat url==catalogue as urlless so it gets a stable
      // synthetic key here and is matched to its real-URL twin by property key
      // below — belt-and-braces with the buildLotUrl fix, catching any other
      // path that resolves a lot URL to the catalogue page.
      if (lotUrl && catalogueUrl && lotUrl === canonicaliseLotUrl(catalogueUrl, house)) {
        lotUrl = null;
      }
      if (!lotUrl) {
      ...
      // Prefer lot-native dates; strip absurd far-future non-sentinel typos before stamp.
      let lotNative = bulletDate || lot._auctionDate || null;
      if (lotNative) {
        const d = String(lotNative).slice(0, 10);
        const todayIso = new Date().toISOString().slice(0, 10);
        // Keep past real dates (sold history); reject nonsense future years > today+2.
        if (/^\d{4}-\d{2}-\d{2}$/.test(d) && d >= todayIso) {
          const maxYear = Number(todayIso.slice(0, 4)) + 2;
          if (Number(d.slice(0, 4)) > maxYear) lotNative = null;
        }
        if (d >= '2098-01-01') lotNative = null;
      }
      const lotAuctionDate = lotNative || calendarDate;
    ...
    // Also build the map when this batch carries any urlless (synthetic-keyed)
    // row: those are the degraded-run lots that would otherwise INSERT a
    // duplicate of a lot already stored under its real detail URL. Matching
    // them by property key lets the upsert UPDATE the real row instead. Cheap
    // guard — the extra full-house read only fires on batches that need it.
    const hasSyntheticRows = rows.some(r => typeof r.url === 'string' && r.url.startsWith('__synthetic__'));
    if (UNSTABLE_LOT_URL_HOUSES.has(house) || hasSyntheticRows) {
    ...
    for (const row of rows) {
      ...
      // Property-key adoption fires for venmore (every row — its URLs are
      // unstable) and, on ANY house, for a urlless (synthetic-keyed) row — the
      // degraded-run lot that must match its real-URL twin rather than INSERT a
      // duplicate. A real-URL row on a normal house never consults the map, so
      // this adds no dedup blast radius to the healthy path.
      const isSynthetic = typeof row.url === 'string' && row.url.startsWith('__synthetic__');
      if (!existing && existingByPropertyKey && (UNSTABLE_LOT_URL_HOUSES.has(house) || isSynthetic)) {
    ...
    try {
      ...
      if (blockedByRatio) {
        ...
        // + -allowed-statuses.sql) rather than a batch .update(). Two reasons:
        //
        //  1. The stamp must MERGE into enrichment_manifest, not replace it. A
        //     PostgREST .update() writes one literal value across the whole
        //     batch, so it clobbered every pruned lot's provenance — OS Places
        //     (paid, £0.01/lookup, trial exhausted), EPC, scoring, fundability —
        //     and pruned lots frequently come back via lot-URL identity churn,
        //     re-paying for all of it. It also broke "silent failures banned",
        //     whose entire record lives in the manifest. The RPC does
        //     `enrichment_manifest || patch` server-side, per row.
        //  2. It returns rows ACTUALLY updated. The old loop credited
        //     `pruned += idBatch.length` for every batch that didn't error,
        //     so the count was the candidate count, not the flip count.
        //
        // p_allowed_statuses is the prune's own IN_PLAY set, NOT the RPC's
        // available-only default: 53% of real prune flips are stc/unsold
        // (measured on lot_events), and passing the default would have left
        // every one of them in play forever.
        const { pruned, flipped, failedLots } = await flipPruneCandidates({
          candidates,
          flipLots: async (idBatch) => {
            const { data, error } = await supabase.rpc('retire_lots', {
              p_ids: idBatch,
              p_manifest_patch: stamp,
              p_allowed_statuses: PRUNE_RETIRE_STATUSES,
            });
            if (error) throw new Error(error.message);
            return typeof data === 'number' ? data : 0;
          },
          onBatchFailure: (why) => {
            console.warn(`LOTS: prune retire_lots batch failed for ${house}: ${why}`);
          },
        });
        if (failedLots > 0) {
          console.warn(`LOTS: ${house}: ${failedLots} prune candidates left in play (flip did not land) — retried next scrape`);
        }
        if (pruned > 0) {
          ...
          // Iterates `flipped`, not `candidates`: a batch whose flip failed
          // leaves its lots in play, and asserting their withdrawal here would
          // put a state change in lot_events that never happened in `lots`.
          try {
            const pruneSource = {
              scrape_id: scrapeId,
              scraper_version: metadata.extractedWith || metadata.scrapedWith || 'unknown',
