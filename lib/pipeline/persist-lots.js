...
async function upsertToLotsTable(enrichedLots, house, catalogueUrl, metadata = {}) {
  ...
  try {
    ...
    // price_status must be derived from the MERGED row, not the pre-merge
    // lot: derivePriceStatus never returns null (floor 'unknown'), so the
    // never-overwrite-with-null guard can't protect it, and a sparse
    // re-scrape (no price on the list page; old price/price_text preserved
    // by the merge above) would write price_status='unknown' next to a
    // preserved price=£X — an inconsistent row the gap scanner then
    // re-flags. (review 2026-06-12 #1)
    for (const row of rows) {
