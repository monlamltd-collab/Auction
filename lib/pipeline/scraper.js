...
import { isFirecrawlCfBypassOnly } from '../scraper/firecrawl.js';
...
export async function scrape(url, house, opts = {}) {
  ...
  // Tier 1: Firecrawl — gated off under FIRECRAWL_CF_BYPASS_ONLY (default on)
  // so it's never reached for non-stealth; Tier 2 (Puppeteer) is primary. The
  // FC dependency is retained (dead) per the keep-FC directive.
  if (!isFirecrawlCfBypassOnly() && _deps.FIRECRAWL_API_KEY && !_deps.isFcCreditExhausted() && !_deps.FIRECRAWL_SKIP.has(house)) {
    try {
