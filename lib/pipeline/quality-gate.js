...
export function qualityGate(lots, house, prevCached, prevLots) {
  ...
  // Some catalogues (AH UK branches in particular) show the buyer's premium
  // prominently on the card while hiding the guide on the detail page; the
  // extractor picks the prominent number. Nulling lets detail-page enrichment
  // (lib/scraper/lot-detail.js price extractor) recover the real guide.
  let feeNulledCount = 0;
  for (const lot of lots) {
    if (typeof lot.price === 'number' && lot.price > 0 && lot.price < 5000
        && (lot.propType === 'house' || lot.propType === 'flat')
        && typeof lot.beds === 'number' && lot.beds >= 1) {
      lot.price = null;
      feeNulledCount++;
    }
  }
