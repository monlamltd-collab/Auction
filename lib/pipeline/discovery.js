...
export async function discoverAndUpdateCalendar(deps) {
  ...
  for (const slug of slugs) {
    ...
    try {
      ...
      const found = await _upsertCatalogues(slug, catalogues);
      discovered += found;
...
// ── Fetch root page: Firecrawl → plain HTTP ──
async function _fetchRootPage(rootUrl, slug, deps) {
  let html;
  if (deps.FIRECRAWL_API_KEY && !deps.isFcCreditExhausted()) {
    try {
      const fcResult = await deps.scrapeWithFirecrawl(rootUrl, { formats: ['rawHtml'] });
      html = fcResult.html || '';
    } catch (fcErr) {
  ...
  if (!html) {
    try {
      ...
      const resp = await fetch(rootUrl, { headers: deps.HEADERS, signal: controller.signal });
...
async function _upsertCatalogues(slug, catalogues) {
  let found = 0;
