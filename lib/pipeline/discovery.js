...
import { upsertUpcomingCatalogue } from './calendar-entries.js';
...
async function _upsertCatalogues(slug, catalogues, deps = {}) {
  ...
  for (const cat of catalogues) {
    ...
    // Shared safe writer (Task 3). Requires a real auction date — no more
    // inventing "today" for undated candidates. Continuity for rolling houses
    // remains calendar-sync always_on, not fake upcoming rows.
    ...
    if (res.ok && res.action === 'upserted') {
      ...
      // house_date / url conflicts are silent skips (already covered)
      if (res.error) {
