...
export async function discoverAndUpdateCalendar(deps = {}) {
  ...
  const vipBudget = Number(process.env.DISCOVERY_VIP_BUDGET || DEFAULT_VIP_BUDGET);
  ...
  for (const slug of Object.keys(HOUSE_ROOTS)) {
    ...
    // Recheck cadence uses discovery-attempt age only — never homepage-watch
    // updated_at (signal freshness ≠ attempt age; fairness invariant).
    const ev = evaluateDiscoveryEligibility({
      ...
      lastDiscoveryAt: lastDiscoveryAttemptAt,
      calendarRows: rows,
      classifyInput: {
        slug,
    ...
    // Preserve attempt age for selectDiscoveryTargets (ev may echo lastDiscoveryAt).
    evaluations.push({ ...ev, slug, lastDiscoveryAt: lastDiscoveryAttemptAt });
  }

  const { selected, darkUsed, recheckUsed, vipExtraUsed, eligibleCount } = selectDiscoveryTargets(evaluations, {
    darkBudget,
    recheckBudget,
    vipBudget,
  });
  ...
  console.log(
    ...
    ` (darkBudget ${darkUsed}/${darkBudget}, recheck ${recheckUsed}/${recheckBudget}, vipExtra ${vipExtraUsed || 0}/${vipBudget})` +
    `${dryRun ? ' [dry-run]' : ''}`,
  );
