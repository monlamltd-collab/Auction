...
export async function sweepNarratives(opts = {}) {
  ...
  // Visibility date-gate mirroring get_active_lots: null date (perpetually
  // live) or ending today onwards. Without it, ascending date order put
  // PAST-DATED stock at the FRONT of the pool — the first prod run (2026-07-23)
  // burned 88% of its budget (812/921 lots) on ended-adjacent stc/unsold rows
  // while the ~13k user-visible lots waited. Narrative budget follows what
  // users can actually see; among those, soonest-ending still goes first.
  const visibilityFloor = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
  // Supabase REST clamps any single request to 1,000 rows (db-max-rows), which
  // silently halved the intended pool (log: pool=1000 with SWEEP_FETCH_POOL=
  // 2000). Page with .range() until the pool target is filled.
  const PAGE = 1000;
  const rawCandidates = [];
  let pageError = null;
  for (let from = 0; from < SWEEP_FETCH_POOL; from += PAGE) {
    let q = supabase
      .from('lots')
      ...
      .or(`auction_date.is.null,auction_date.gte.${visibilityFloor}`)
      .order('auction_date', { ascending: true, nullsFirst: false })
      .range(from, Math.min(from + PAGE, SWEEP_FETCH_POOL) - 1);
    if (houseScope) q = q.eq('house', houseScope);
    const { data, error } = await q;
    if (error) { pageError = error; break; }
    rawCandidates.push(...(data || []));
    if (!data || data.length < PAGE) break; // pool exhausted
  }

  if (pageError && rawCandidates.length === 0) {
    log.error('narrative-sweep: candidate query failed', { err: pageError.message });
    return zeroStats();
  }
