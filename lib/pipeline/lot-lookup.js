 ...
 */
export async function getLotsForCatalogues(supabase, catalogues, { select = '*' } = {}) {
  const withId = [];
  const withoutId = [];
  ...
  if (auctionIds.length > 0) {
    queries.push(supabase.from('lots').select(select).in('auction_id', auctionIds));
  }
  if (urls.length > 0) {
    queries.push(supabase.from('lots').select(select).in('catalogue_url', urls));
  }
