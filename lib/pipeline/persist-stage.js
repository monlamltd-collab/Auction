...
export async function persistStage(ctx, deps) {
  ...
  // ── Dormant auto-clear ──
  // A dormant-flagged house re-activates the moment we extract a LIVE
  // (available/unsold) lot — its auction has reopened. The .eq('dormant', true)
  // filter makes this a no-op for every non-dormant house (the common case),
  // so it's a cheap guarded write, not a per-scrape mutation of every row.
  const hasLiveLot = lots.some(l => l.status === 'available' || l.status === 'unsold');
  if (hasLiveLot) {
    try {
      await supabase.from('house_skills')
        .update({ dormant: false, dormant_since: null })
        .eq('slug', house)
        .eq('dormant', true);
    } catch (e) { console.warn(`DORMANT: auto-clear failed for ${house}: ${e.message}`); }
  }
