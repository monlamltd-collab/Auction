...
export async function persistStage(ctx, deps) {
  ...
  // enrich-stage stashes _osPlacesRetry on lots whose OS Places lookup hit a
  // transient failure or no_match. Lots only get DB ids during upsert, so we
  // re-fetch them here and enqueue. COVERAGE_FIX_PLAN.md fix #4 — without
  // this, transient failures wait for the next hygiene wave (~6h) before any
  // record of the failure exists.
  try {
    const lotsNeedingRetry = lots.filter(l => l._osPlacesRetry && l.url);
    if (lotsNeedingRetry.length > 0) {
      ...
      // ── Parallel enqueue ── retry-queue inserts are independent per-lot;
      // sequential awaits made a 50-failure scrape pay 50 RTTs back-to-back.
      await Promise.allSettled(lotsNeedingRetry.map(lot => {
        const lotId = idByUrl.get(lot.url);
        if (!lotId) return Promise.resolve();
        return enqueueRetry(supabase, {
          lotId,
          field: 'uprn',
