...
import { isSanelyNearFutureDate } from '../utils/auction-date-parse.js';
import { maybeLiftLotDateConsensus } from './lot-date-consensus.js';
...
export async function persistStage(ctx, deps) {
  ...
  // Move 2: dual-read helper for lots; no auctionId resolved here, so legacy
  // (house, catalogue_url) path fires. The added house filter is defensive —
  // catalogue_url is already house-specific in practice.
  const [{ data: prevCached }, { data: prevLotRows }] = await Promise.all([
    ...
    getLotsForCatalogue(supabase, { house, catalogueUrl: normalisedUrl, select: LOTS_SELECT }),
  ]);
  const prevLots = (prevLotRows || []).map(dbRowToLot);
  ...
  // Task 8: light sanity clamp on absurd far-future lot dates before persist.
  // Never invents dates; only strips nonsense so consensus / UI stay clean.
  const todayIso = new Date().toISOString().slice(0, 10);
  for (const lot of lots) {
    for (const key of ['_auctionDate', 'auctionDate', 'auction_date']) {
      const raw = lot[key];
      if (!raw) continue;
      const d = String(raw).slice(0, 10);
      if (/^\d{4}-\d{2}-\d{2}$/.test(d) && d < '2098-01-01' && !isSanelyNearFutureDate(d, { todayIso }) && d >= todayIso) {
        lot[key] = null;
      }
    }
  }
  ...
  // Task 8: majority lot date → calendar (gated by LOT_DATE_CONSENSUS_LIFT_ENABLED).
  try {
    const lift = await maybeLiftLotDateConsensus(supabase, {
      slug: house,
      lots,
      catalogueUrl: url,
    });
    if (lift?.decision?.lift) {
      console.log(
        `LOT-CONSENSUS: ${house} majority=${lift.consensus?.majorityDate} ` +
        `ratio=${(lift.consensus?.ratio || 0).toFixed(2)} ` +
        `action=${lift.action || 'observe'} upserted=${!!lift.upserted}`,
      );
    }
  } catch (e) {
    console.warn(`LOT-CONSENSUS: ${house} failed (non-fatal): ${e.message}`);
  }
