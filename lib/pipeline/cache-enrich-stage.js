...
async function _backfillImageCascade(auction, lots, deps) {
  ...
  if (stillNoImages > 0 && PUPPETEER_IMAGE_HOUSES.has(houseSlug)) {
    ...
    // Pass 5: Puppeteer — retired no-op since 2026-05-08; callers may no
    // longer wire it into deps, so guard before calling.
    const afterFc = lots.filter(l => !l.imageUrl).length;
    if (afterFc > 0 && deps.puppeteer && typeof deps.backfillImagesWithPuppeteer === 'function') {
