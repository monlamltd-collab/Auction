...
export async function runEnrichmentWave(deps, opts = {}) {
    ...
    for (let i = 0; i < pricelessLots.length; i += 5) {
      ...
      await Promise.allSettled(batch.map(async (dbRow) => {
        try {
          ...
          // raw_text dropped in the lean rebuild — no longer captured/persisted.
  ...
  const { data: needsEnrichment } = await supabase
    ...
    .or('enriched_at.is.null,epc_rating.is.null,flood_risk.is.null,comparable_price.is.null,est_gross_yield.is.null')
    .order('last_seen_at', { ascending: false })
    .limit(500);
      ...
      try {
        ...
        for (const lot of lotObjs) {
          if (lot.score == null && (!lot.scoreBreakdown || lot.scoreBreakdown.length === 0)) {
            ...
            lot.dealSignals = [...new Set([...(lot.dealSignals || []), ...(analysed.dealSignals || [])])];
            if (lot.statedIncomePa == null && analysed.statedIncomePa != null) {
              lot.statedIncomePa = analysed.statedIncomePa;
              lot.incomeKind = analysed.incomeKind;
            }
          }
      ...
      await Promise.allSettled(batch.map(async (row) => {
        try {
          ...
          if ((result.status === 'ok' || result.status === 'cache_hit') && result.uprn) {
            ...
            // os_classification dropped in the lean rebuild — no longer persisted.
  ...
  async function drainEpcRow(row) {
    ...
    // epc_date dropped in the lean rebuild.
