...
/** Align with watcher-horizon: nearest sale in this window must be ready. */
...
export function analyseHouseSpine(h, opts) {
  ...
  const nearReadyEnd = opts.nearReadyEnd || (today ? addDays(today, nearReadyDays) : null);
  ...
  const nearestInNearWindow =
    !!nearestUpcoming && !!nearReadyEnd && nearestUpcoming <= nearReadyEnd;
...
export function scoreHousePopulation({ discoveryClass, spine, explainedMissReason = null }) {
  ...
  if (
    ...
    // Savills-class: later ready sales must not hide a not-ready nearest sale.
    if (spine.nearestInNearWindow && spine.nearestReady === false) {
      return {
        credit: 0,
        dark: true,
        reason: 'nearest_not_ready',
        detail: 'trad_nearest_not_ready',
      };
    }
...
export function computeFleetCoverage(houses, options = {}) {
  ...
  const nearReadyDays = options.nearReadyDays ?? DEFAULT_NEAR_READY_DAYS;
...
export function isFleetCoverageAlertsEnabled(env = process.env) {
  // Default ON after owner approval (2026-07-25). Set false to silence Telegram.
  const raw = env.FLEET_COVERAGE_ALERTS_ENABLED;
