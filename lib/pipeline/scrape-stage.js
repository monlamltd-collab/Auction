...
//
// Two branches:
//   1. Allsop JSON API — bespoke private-JSON consumer (lib/scraper/allsop.js).
//      Exception, not the anti-pattern: zero credits, structurally stable.
...
async function _handleZeroLots(house, url) {
  try {
    ...
    if (prevSkill && prevSkill.last_lot_count > 0) {
      // Dormant houses (known between-auctions/defunct) legitimately drop to 0
      // lots when their auction ends — record it as info, not an error, and
      // don't kick self-healing (there's no broken URL to heal). Auto-clears
      // when a live lot reappears (persist-stage).
      if (prevSkill.dormant === true) {
        await supabase.from('pipeline_alerts').insert({
          event_type: 'extractor_regression',
          severity: 'info',
          house,
          message: `${HOUSE_DISPLAY_NAMES[house] || house} returned 0 lots (was ${prevSkill.last_lot_count}) — house flagged dormant; expected between auctions.`
        });
        console.log(`ALERT: ${house} 0 lots but dormant — info only, no heal`);
        return;
      }
      await supabase.from('pipeline_alerts').insert({
        event_type: 'extractor_regression',
