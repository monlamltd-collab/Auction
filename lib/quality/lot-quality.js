...
export function computeLotQuality(lot) {
  ...
  // Price — branched on the structured priceStatus when available, falling
  // back to the priceText regex for old code paths that haven't started
  // populating priceStatus yet. Status-class outcomes (sold / withdrawn)
  // are awarded full marks because the price field is missing for a
  // legitimate reason — they're not coverage failures.
  const priceStatus = (lot.priceStatus || '').toLowerCase();
  ...
  } else if (lot.priceText && /\b(?:nil|no|without|zero)\s*reserve|unreserved\b/i.test(lot.priceText) && !(typeof lot.price === 'number' && lot.price > 0)) {
    // Fallback for lots persisted before priceStatus was populated at runtime.
    score += FIELD_WEIGHTS.price;
    issues.push('nil_reserve');
  } else if (lot.priceText && /poa|on application/i.test(lot.priceText)) {
    // Fallback for lots persisted before priceStatus existed.
    score += Math.floor(FIELD_WEIGHTS.price / 2);
    issues.push('poa_price');
  } else if (lot.priceText && /tba|tbc|to be advised|to be confirmed/i.test(lot.priceText)) {
    score += Math.floor(FIELD_WEIGHTS.price / 2);
    issues.push('tba_price');
  } else {
...
export function computeBatchCoverage(lots) {
  ...
  // a "not a gap" value (poa / tba / sold / withdrawn) are removed from
  // BOTH numerator and denominator. Without this, a house with a 10%
  // POA fraction would falsely show a 10pp drop in price_pct vs a house
  // with no POA listings — and the regression-alert system would page
  // for a bug that doesn't exist. See migrations/2026-04-28-price-status.sql
  // for the column definition.
  const priceEligible = lots.filter(l => !PRICE_STATUS_NOT_A_GAP.has((l.priceStatus || '').toLowerCase()));
