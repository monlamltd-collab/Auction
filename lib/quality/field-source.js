...
/**
 * Like stampSource, but only writes the source when no provenance is already
 * recorded for that field. Used by the catalogue-path "blanket stamp" so it
 * doesn't overwrite a more specific stamp written earlier in the same scrape
 * (e.g. mergeDomai stamping url/imageUrl as 'dom' before the gemini-catalogue
 * pass tries to claim them).
 */
export function stampSourceIfEmpty(lot, field, source) {
  if (!lot || typeof lot !== 'object') return;
  const v = lot[field];
  if (v === null || v === undefined || v === '') return;
  if (!lot._fieldSources) lot._fieldSources = {};
  if (lot._fieldSources[field]) return;
  lot._fieldSources[field] = source;
}
