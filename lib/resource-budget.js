...
export class ResourceBudget {
  ...
  recordFcRequest(tier = 'unknown', weight = 1) {
    ...
    if (cycle !== this._fc.monthStartedAt) {
      ...
      // creditsUsed itself is NOT reset here — it tracks lifetime spend that
      // a separate process (or operator) clears explicitly when needed.
      // Threshold flags above are the per-cycle state.
    }
  ...
  recordFcMapRequest(tier = 'unknown') {
  ...
  _autoReset() {
    ...
    // Plan-cycle threshold-flag rollover (matches the daily pattern above).
    const cycle = this._planCycleKey();
    if (cycle !== this._fc.monthStartedAt) {
      this._fc.monthStartedAt = cycle;
      this._fc.monthlyCapHit = false;
