...
export class ResourceBudget {
  constructor({ firecrawlApiKey, monthlyBudget, dailyBudget, skipHouses, minGapMs } = {}) {
    ...
    this._fc = {
      ...
      // Monthly accounting — used to clear the threshold flags below at month
      // rollover. Format: 'YYYY-MM'.
      monthStartedAt: this._utcMonthKey(),
      monthlyCapHit: false,
      dailyCapHit: false,
  ...
  _isOverDailyCap() {
    return this.dailyBudget > 0 && this._fc.creditsUsedToday >= this.dailyBudget;
  }
  ...
  recordFcRequest(tier = 'unknown', weight = 1) {
    ...
    // Roll monthly counter / clear threshold flags at month boundary.
    const month = this._utcMonthKey();
    if (month !== this._fc.monthStartedAt) {
      this._fc.monthStartedAt = month;
  ...
  recordFcAgentRequest(tier = 'unknown') {
    this.recordFcRequest(tier, this.fire1CreditMult);
  }
  ...
  _autoReset() {
    ...
    // Monthly threshold-flag rollover (matches the daily pattern above).
    const month = this._utcMonthKey();
    if (month !== this._fc.monthStartedAt) {
      this._fc.monthStartedAt = month;
      this._fc.monthlyCapHit = false;
