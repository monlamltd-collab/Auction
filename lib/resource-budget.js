...
export class ResourceBudget {
  constructor({ firecrawlApiKey, monthlyBudget, dailyBudget, skipHouses, minGapMs } = {}) {
    ...
    this._fc = {
      ...
      exhausted: false,
      exhaustedAt: 0,
      // 402 plan-exhaustion: latched until cycle rollover (distinct from the
      // 1h-TTL 429 `exhausted` flag). See recordFcError / _autoReset.
      planExhausted: false,
      fallbackCount: 0,
      errorCount: 0,
      requestCount: 0,
  ...
  whyBlocked() {
    ...
    if (this._fc.planExhausted) return 'plan-exhausted-402';
    if (this._fc.exhausted) return 'rate-limited-429';
  ...
  recordFcRequest(tier = 'unknown', weight = 1, eventMeta = null) {
    ...
    if (cycle !== this._fc.monthStartedAt) {
      ...
      // New cycle = fresh credits: lift the 402 plan-exhaustion latch.
      this._fc.planExhausted = false;
      if (this._fc.exhausted) { this._fc.exhausted = false; this._fc.exhaustedAt = 0; }
  ...
  recordFcError(statusCode, err) {
    ...
    if (statusCode === 402) {
      // 402 = plan out of credits. This is NOT transient (it persists until the
      // billing cycle refreshes), so latch it HARD: planExhausted is never
      // cleared by the 1h TTL, only at cycle rollover. Without this the 1h TTL
      // un-latches every hour and the next house burns another 402 before
      // dropping to fallback — the "exhaustion thrash" (PR #67 review F5).
      this._fc.planExhausted = true;
      this._fc.exhausted = true;
      this._fc.exhaustedAt = Date.now();
      console.log('ResourceBudget: Firecrawl 402 — plan out of credits, latched until cycle refresh');
    } else if (statusCode === 429) {
      // 429 = rate limited. Transient — the 1h TTL is correct here.
      this._fc.exhausted = true;
      this._fc.exhaustedAt = Date.now();
      console.log('ResourceBudget: Firecrawl 429 rate limit — switching to fallback (1h TTL)');
    }
  ...
  _autoReset() {
    ...
    // Firecrawl 429 rate-limit exhaustion: clear after 1 hour. A 402
    // plan-exhaustion (planExhausted) is NOT cleared here — it only lifts at
    // cycle rollover, so a dead plan stays latched instead of thrashing hourly.
    if (this._fc.exhausted && !this._fc.planExhausted && Date.now() - this._fc.exhaustedAt > 3600000) {
      this._fc.exhausted = false;
      this._fc.exhaustedAt = 0;
      console.log('ResourceBudget: Firecrawl 429 exhaustion flag auto-cleared (1h TTL)');
    }
