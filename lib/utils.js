...
export function parseAuctionDateFromBullet(bullet, today) {
  ...
  //    Resolve to the next occurrence on or after `today` so we never assign
  //    a date in the past.
  ...
  if (m3 && /\bAUCTION\b/i.test(t)) {
    ...
    if (mo && day >= 1 && day <= 31) {
      ...
      // If "20 May 2026" with today=2026-09-01, that's in the past → roll to next year
      return candidate >= todayIso ? candidate : `${yr+1}-${String(mo).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
    }
  }
