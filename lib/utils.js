...
export const normaliseUrl = u => (u || '').trim().replace(/#.*$/, '').replace(/\/+$/, '').replace(/^http:\/\//i, 'https://').replace(/^(https:\/\/)www\./i, '$1').toLowerCase();
...
// ── Savills auction path slug → sale dates ───────────────────────────
// Savills encodes the sale date(s) and a rotating auction id in the catalogue
// path: /auctions/28--29-july-2026-227. Both the URL rewriter (lib/houses.js)
// and the catalogue resolver (lib/pipeline/firecrawl-extract.js) need to order
// sales by date, and this module is import-leaf so neither picks up a cycle.
export const SAVILLS_MONTHS = {
  january: 1, february: 2, march: 3, april: 4, may: 5, june: 6,
  july: 7, august: 8, september: 9, october: 10, november: 11, december: 12,
};

/**
 * '28--29-july-2026-227' → { year:2026, auctionId:'227',
 *                            days:[{day:28,month:7},{day:29,month:7}],
 *                            startIso:'2026-07-28', endIso:'2026-07-29' }
 * Handles single dates ('31-march-2026-220') and cross-month ranges
 * ('30-june--1-july-2026-231'). Returns null when the slug isn't a dated sale.
 */
export function parseSavillsAuctionSlug(slug) {
  if (!slug || typeof slug !== 'string') return null;
  const m = /^(.+?)-(\d{4})-(\d+)$/.exec(slug.toLowerCase());
  if (!m) return null;
  ...
  for (const tok of m[1].split('-')) {
    if (!tok) continue;
    if (/^\d{1,2}(?:st|nd|rd|th)?$/.test(tok)) { pending.push(parseInt(tok, 10)); continue; }
    const month = SAVILLS_MONTHS[tok];
    if (!month || pending.length === 0) continue;
    for (const day of pending) days.push({ day, month });
    pending = [];
  }
  if (!days.length) return null;
  const iso = ({ day, month }) => `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
  return { year, auctionId: m[3], days, startIso: iso(days[0]), endIso: iso(days[days.length - 1]) };
}
