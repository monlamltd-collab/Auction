...
export async function rewriteUrl(url, house) {
  ...
  // Bagshaws Residential: one hand-maintained static HTML page carries the whole
  // catalogue. Pin any bagshawsauctions.co.uk URL to the root so a stale calendar
  // row / healed sub-page (Current_Auction.html, Auction-Results.html — the
  // results page lists PAST sales) can never become the scrape target. Plain
  // HTTP; recogniseBagshawsLotsFromMarkdown parses it via the staticCatalogue path.
  if (house === 'bagshaws') {
    return { baseUrl: HOUSE_ROOTS.bagshaws, isApi: false, paginateAs: null, preferPuppeteer: false };
  }
