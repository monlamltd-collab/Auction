...
export const HOUSE_ROOTS = {
  ...
  network:            'https://www.btgeddisonspropertyauctions.com/auctions/multi-lot-timed/may-2026',
  bondwolfe:          'https://www.bondwolfe.com/auctions/properties/',
  barnardmarcus:      'https://www.barnardmarcusauctions.co.uk/auctions/19-may-2026/',
  ...
  knightfrank:        'https://www.knightfrankauctions.com/auction/3834/knight-frank-auctions-2026-05-07/',
  ...
  halls:                  'https://www.hallsgb.com/property-auction/',
  ...
  webbers:                'https://www.webbers.co.uk/online-auctions/',
  ...
  // charlesdarrow — REMOVED: Charles Darrow was acquired by BTG Eddisons.
  // Their lots now flow through the BTG Eddisons catalogue at
  // btgeddisonspropertyauctions.com which is already scraped under the
  // 'sdl' slug. A separate charlesdarrow entry would just duplicate a
  // subset of those lots and trip phantom regression alerts on the dead
  // legacy domain.
  // Eastern England
  ...
  williamhbrownleeds:     'https://www.williamhbrownauctions-leeds.co.uk/Current_Auction.html',
...
export function detectAuctionHouse(url) {
  ...
  if (u.includes('networkauctions') || (u.includes('btgeddisonspropertyauctions') && u.includes('/auctions/'))) return 'network';
  ...
  if (u.includes('johnpyeproperty.co.uk') || u.includes('johnpye.co.uk')) return 'johnpye';
  // ── EIG batch 5 houses ──
  ...
  // acquired by BTG Eddisons; their lots route through the sdl extractor.
  if (u.includes('charlesdarrow.co.uk') || u.includes('charlesdarrowauctions.com')) return 'sdl';
  ...
  // ── William H Brown (Sequence platform, April 2026) ──
  if (u.includes('williamhbrownauctions-leeds')) return 'williamhbrownleeds';
...
export const HOUSE_DISPLAY_NAMES = {
  ...
  williamhbrownleeds: 'William H Brown (Leeds)',
...
const _LEGACY_HOUSE_NAME_ALIASES = {
  'sdlauctions': 'sdl',  // FALLBACK_CALENDAR still uses the old "SDL Auctions" label
};
 ...
 *   canonicaliseHouseSlug('VENMORE')          → 'venmore'
 *   canonicaliseHouseSlug('SDL Auctions')     → 'sdl'   (legacy alias)
...
export const EXTRACTION_PROFILE = {
  ...
  hollismorgan:  { catalogue: 'shell', policy: 'always-deep', overwriteFields: ['imageUrl', 'bullets'], maxPerCycle: 80 },
  fssproperty:   { catalogue: 'shell', policy: 'always-deep', overwriteFields: ['imageUrl', 'bullets'], maxPerCycle: 80 },
...
export const AUCTION_DISCOVERY = {
  ...
  countrywide: {
    homepage: 'https://www.countrywidepropertyauctions.co.uk/',
    linkPattern: /\?auction_date=(\d{4}-\d{2}-\d{2})/g,
    buildUrl: (date) => `https://www.countrywidepropertyauctions.co.uk/properties/?auction_date=${date}`,
  },
  ...
  knightfrank: {
    homepage: 'https://www.knightfrankauctions.com/forthcoming-auctions/',
  },
  ...
  // ── Varying paths (Network Auctions, AH London) ──
  network: {
    homepage: 'https://www.networkauctions.co.uk/',
  },
  auctionhouselondon: {
...
export async function rewriteUrl(url, house) {
  ...
  if (house === 'network') {
    // Network Auctions rebranded to BTG Eddisons — date-specific auction pages
  ...
  if (house === 'barnardmarcus') {
    // Barnard Marcus: date-specific auction pages (e.g. /auctions/19-may-2026/)
    // Homepage links to the next auction — auto-discover if the stored URL is stale
  ...
  if (house === 'hollismorgan') {
    if (u.includes('bid=')) {
      return { baseUrl: url, isApi: false, paginateAs: 'query_page', preferPuppeteer: false };
    }
    try {
  ...
  if (house === 'symondsandsampson') {
    ...
    // Firecrawl fallback for JS-rendered events pages
    if (!discovered && FIRECRAWL_API_KEY && !module_state.fcCreditExhausted) {
      try {
        const fcResult = await scrapeWithFirecrawl(eventsUrl, { formats: ['rawHtml'], waitFor: 5000 });
        discovered = discoverFromHtml(fcResult.html);
  ...
  // Halls (Shrewsbury): server returns HTTP 401 to all clients (browser, curl, Firecrawl) —
  // confirmed 2026-04-25. The page sits behind an auth wall regardless of UA / referer.
  // Mark blocked rather than burning credits indefinitely. Revisit if the auth wall is lifted.
  if (house === 'halls') {
    console.log(`${house}: skipped — HTTP 401 auth wall blocks all automated requests`);
