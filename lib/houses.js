...
export const HOUSE_ROOTS = {
  ...
  // Charles Darrow migrated their live auction catalogue to a dedicated
  // sub-brand (charlesdarrowauctions.com) — the old /auctions/ marketing
  // page on the main site no longer lists lots. Site is StackProtect /
  // reCAPTCHA-protected so Firecrawl rendering is required (plain HTTP +
  // Puppeteer will both bounce off the challenge page).
  charlesdarrow:          'https://charlesdarrowauctions.com/',
  // Eastern England
...
export const AUCTION_DISCOVERY = {
  ...
  // ── Charles Darrow ──
  // 2026-04-24: catalogue moved off main domain onto charlesdarrowauctions.com
  // (StackProtect/reCAPTCHA-protected sub-brand). Homepage points at the new
  // auctions domain so the healing harness can find it; main site is just
  // marketing. If the auctions sub-brand ever rotates URL/path, retry against
  // www.charlesdarrow.co.uk to pick up the new outbound link.
  charlesdarrow: {
    homepage: 'https://charlesdarrowauctions.com/',
  },
};
...
export async function rewriteUrl(url, house) {
  ...
  if (house === 'symondsandsampson') {
    ...
    if (!discoveredUrl && FIRECRAWL_API_KEY && !module_state.fcCreditExhausted) {
      try {
