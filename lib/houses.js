...
export const HOUSE_ROOTS = {
  ...
  markjenkinson:          'https://www.markjenkinson.co.uk/property-search',
  ...
  humberts:               'https://www.humberts.com/search/?department=residential-sales&type=auction',
  // Wales
...
export async function rewriteUrl(url, house) {
  ...
  if (house === 'network') {
    ...
    // BTG's live-stream catalogue is paginated via ?page=N (~9 lots per page,
    // up to ~20 pages on a typical event). Without paginateAs the JSON extract
    // only sees page 1; with 'query_page' we get the full catalogue.
