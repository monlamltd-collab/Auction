...
import { parseSavillsAuctionSlug } from './utils.js';
import { scrapeWithCrawlee, hasCrawlee } from './scraper/crawlee.js';
import { UTH_CATALOGUE_URL } from './scraper/underthehammer.js';
import { PATTINSON_CATALOGUE_URL } from './scraper/pattinson.js';
...
export const HOUSE_ROOTS = {
  ...
  // Branded host is the live canonical endpoint; the old eigonlineauctions host
  // now redirects here. canonicaliseLotUrl pins both aliases to this root.
  paulfosh:               'https://auction.paulfosh.com/search?view=List&pagesize=200&page=1',
  ...
  // Loveitts is Auction House Coventry & Warwickshire on loveitts.co.uk.
  // EIG live-stream embed has NO lot cards — do not use it as HOUSE_ROOTS.
  loveitts:           'https://www.loveitts.co.uk/auctions/',
  ...
  tcpa:               'https://www.townandcountrypropertyauctions.co.uk/search?pagesize=500',
  ...
  // auctionhouseeastmidlands retired 2026-04-24, auctionhousewestmidlands retired
  // 2026-07-05 (Simon-approved, decision queue) — both branch paths 404; the branches
  // merged into auctionhouse.co.uk/midlands (slug auctionhousemidlands below).
  // Slugs + display names kept for historical lots.
  ...
  // Fisher German runs its auctions on Bamboo; the .co.uk page is marketing only
  // (dark 55d). Lots live at fishergerman.bambooauctions.com (20 live 2026-07-10).
  fisherGerman:           'https://fishergerman.bambooauctions.com/',
  ...
  // foxgrant /auctions/ returns an empty 204; catalogue moved (verified 2026-07-05)
  foxgrant:               'https://www.foxgrant.com/properties/sales/tag-auction',
  ...
  // Hawkesford: /auctions/ is a marketing landing page. Real (Bamboo) catalogue below;
  // currently near-empty — auctions are ~annual, so 0 live is a genuine zero.
  hawkesford:             'https://hawkesford.bambooauctions.com/',
  // Howkins & Harrison: /auctions/ is a WordPress marketing page with no lots.
  // Real catalogue is the Bamboo subdomain (20 lots 2026-07-10).
  howkinsandharrison:     'https://howkinsandharrison.bambooauctions.com/',
  ...
  // wrightmarshall — REMOVED 2026-07-04: estate agent with only occasional
  // auction lots (owner-confirmed), sold via the iamSold platform. Its URL was
  // on iamsold.co.uk, which detectAuctionHouse maps to the `iamsold` slug, so
  // this slug never probed under its own name anyway — coverage continues via
  // the iamsold platform scrape. See docs/houses/wrightmarshall.md.
  ...
  purplebricksgoto:       'https://purplebricks.gotoproperties.co.uk/search?pagesize=5000',
  ...
  // Site ROOT, not /Current_Auction.html: the root serves the identical catalogue
  // (verified 19/19 on 2026-07-22) and matches its two Sequence siblings. rewriteUrl
  // pins this house to HOUSE_ROOTS, so pointing at a single hand-maintained FILE
  // would mean a rename takes the house to 0 with healing unable to recover — every
  // healed URL is discarded by the pin. A root cannot 404 that way.
  williamhbrownnorwich:   'https://www.williamhbrownauctions-norwich.co.uk/',
  ...
  // sequence — REMOVED 2026-07-05 (Simon-approved): Connells-group portal only; all
  // catalogues live on its constituent houses (barnardmarcus, foxandsons, bagshaws,
  // williamhbrownnorwich; William H Brown Leeds pending onboarding).
...
export const RETIRED_HOUSES = new Set([
  ...
  'humberts',
  // wrightmarshall retired 2026-07-04 — estate agent, occasional lots flow via
  // the iamsold platform slug (docs/houses/wrightmarshall.md)
  'wrightmarshall',
  ...
  'lextons',
  // ── Simon-approved retirements (2026-07-05, house-removal decision queue) ──
  // West + East Midlands AH branches merged into auctionhouse.co.uk/midlands
  // (auctionhousemidlands — already in HOUSE_ROOTS; NB its capture has never landed
  // a lot and auctionhousebirmingham is blocked, so the AH-UK Midlands region needs
  // a scrape fix — see memory/approvals/house-removals.md work items).
  'auctionhousewestmidlands',
  'auctionhouseeastmidlands',
  // sequence is a Connells-group portal: every catalogue link points at its
  // constituent houses (barnardmarcus, foxandsons, bagshaws, williamhbrown*).
  'sequence',
  // ── Duplicate brand-fronts (retired 2026-07-22 after the full dark-house triage) ──
  // These are NOT dark houses — their lots are already captured in full under
  // another slug, so scraping them DUPLICATES properties in the product.
  //
  // pugh / markjenkinson / sdl are Eddisons-group brands fronting the SAME
  // auctions as btgeddisons, which holds the SUPERSET (77 lots @2026-07-27,
  // 252 @07-29, 18 @08-24 vs the brands' 74/247/14). Proven by cross-checking a
  // lot: btgeddisons' "1-4 Salisbury Street, Widnes WA8 6PJ" appears verbatim in
  // markjenkinson's 27-Jul catalogue. pugh's configured URL is the whole-site
  // archive (6,338 properties, no auction dates), so its rows all carry the
  // 2099-12-31 sentinel and 58 of them were showing as "live" despite being
  // past/unsold stock.
  'pugh',
  'markjenkinson',
  'sdl',
  // auctionhousenational is the superset roll-up of the ~25 auctionhouse.co.uk
  // franchise sites. They all share ONE lot-URL namespace
  // (online.auctionhouse.co.uk/lot/redirect/{id}); lots.url is globally UNIQUE and
  // persist-lots upserts onConflict:'url' with `house` in IDENTITY_FIELDS, so
  // whichever sibling scrapes last STEALS the row and national flaps 0-49. Its
  // 328 "available" rows were duplicates of lots already live under the regional
  // slugs. Retiring the roll-up removes the duplication; no lots are lost.
...
export function detectAuctionHouse(url) {
  ...
  // Same catch-all trap: these three run their auctions on Bamboo subdomains while
  // their HOUSE_ROOTS used to point at the marketing site. Without an explicit clause
  // ahead of the catch-all their lots would all be attributed to `hunters`.
  if (u.includes('howkinsandharrison.bambooauctions')) return 'howkinsandharrison';
  if (u.includes('fishergerman.bambooauctions')) return 'fisherGerman';
  if (u.includes('hawkesford.bambooauctions')) return 'hawkesford';
  ...
  // wrightmarshall detection removed 2026-07-04 — estate agent; lots route via iamsold
...
export const HOUSE_DISPLAY_NAMES = {
  ...
  // wrightmarshall retired — estate agent, lots via iamsold platform
...
//
//   policy:    'never-deep'  — never fetch the lot detail page on a re-scrape,
//                              however gappy the lot looks. The ONE exception is
//                              a first-contact lot (`_isFirstContact`), which
//                              still gets the kitchen-sink pass — that's a
//                              separate invariant owned by enrich-stage.js and
//                              costs one fetch per lot URL, ever.
...
export const EXTRACTION_PROFILE = {
  ...
  // Under The Hammer's /api/properties returns address+postcode, guide, beds,
  // baths, tenure, type, occupancy, EPC and the full photo gallery per lot —
  // nothing left for a detail fetch to add, and its detail pages are SPA
  // shells that would cost a browser render each.
  underthehammer: { catalogue: 'rich', policy: 'never-deep' },
};
...
export const AUCTION_DISCOVERY = {
  ...
  // Rolling event page exists before publication. Do not infer readiness from
  // stale lot links or synthesize a date: the candidate date must appear on the
  // fetched catalogue page before the watcher may persist it.
  mchughandco: {
    homepage: 'https://mchughandco.com/current-auction',
    requireCandidateDateVerification: true,
  ...
  allsop: {
    ...
    // Next-sale dates from /api/auctions/current-and-past (see allsop-auctions-api.js).
    // rewriteUrl maps catalogue rows to property-search JSON for lot scrape.
  },

  // Loveitts — dated multi-sale franchise on loveitts.co.uk (not EIG live-stream).
  // Dates from /auctions/ select options + /auctions/upcoming-auctions (see loveitts-auctions.js).
  loveitts: {
    homepage: 'https://www.loveitts.co.uk/auctions/',
    allowDateFallback: false,
  },
...
export function getDiscoveryConfig(slug) {
  // Explicit map only. Runtime family expansion (AH/EIG) lives in
  // lib/pipeline/platform-discovery.js via auction-watcher.resolveWatcherConfig.
...
// ═══════════════════════════════════════════════════════════════
// Lot-URL canonicalisation (2026-07-03 duplicate-lots incident)
// ═══════════════════════════════════════════════════════════════
// lots.url is the table's UNIQUE conflict key, so every URL variant a house
// emits for the same lot mints a duplicate row the frontend shows twice.
// Verified variants in prod: hollismorgan appends list-navigation params
// (?page=1&bid=11&showstc=on…) to /property-details/ hrefs depending on how
// the render reached the lot; venmore serves www and bare-host hrefs
// interchangeably. Canonicalise ONCE at persist so the conflict key is
// stable across renders.

// Houses whose lot-detail path uniquely identifies the lot, with any
// querystring being list-navigation state — strip the whole query.
const LOT_URL_STRIP_QUERY_HOUSES = new Set(['hollismorgan']);

// Tracking params are never lot identity on any house.
const LOT_URL_TRACKING_PARAM_RE = /^(?:utm_\w+|fbclid|gclid|msclkid)$/i;

// Verified host migrations where the lot-detail path/ID stayed stable.
// Restrict aliases per house so the normal foreign-domain defence remains.
const LOT_URL_HOST_ALIASES = {
  paulfosh: new Set(['auction.paulfosh.com', 'paulfosh.eigonlineauctions.com']),
};

// Houses whose lot URLs are UNSTABLE — the site emits a different URL for
// the same lot on different renders, so no URL rewrite can produce a stable
// key. Verified: venmore's ?property_reference= is positional ('4'/'6'),
// an L-ref, an AUC-ref, or the URL-encoded address for the SAME property
// depending on render. persist-lots.js falls back to property-identity
// (postcode|first address segment — mirrors the lots.property_key generated
// column) matching for these houses instead of inserting a new row.
export const UNSTABLE_LOT_URL_HOUSES = new Set(['venmore']);

export function canonicaliseLotUrl(url, house) {
  if (!url || !/^https?:\/\//i.test(url)) return url; // synthetic keys pass through
  try {
    const parsed = new URL(url);
    // Host: pin to the house's canonical www-policy host when configured,
    // else align a www/bare mismatch with the HOUSE_ROOTS host.
    const rootHost = HOUSE_ROOTS[house] ? new URL(HOUSE_ROOTS[house]).hostname : null;
    const aliases = LOT_URL_HOST_ALIASES[house];
    const pinned = WWW_CANONICAL_HOSTS[house];
    if (rootHost && aliases?.has(parsed.hostname)) {
      parsed.host = rootHost;
    } else if (pinned && parsed.hostname.endsWith(pinned.replace(/^www\./, ''))) {
      parsed.host = pinned;
    } else if (rootHost) {
      if (parsed.hostname !== rootHost
        && parsed.hostname.replace(/^www\./, '') === rootHost.replace(/^www\./, '')) {
        parsed.host = rootHost;
      }
    }
    if (LOT_URL_STRIP_QUERY_HOUSES.has(house)) {
      parsed.search = '';
    } else {
      const drop = [];
      parsed.searchParams.forEach((_, k) => { if (LOT_URL_TRACKING_PARAM_RE.test(k)) drop.push(k); });
...
export async function rewriteUrl(url, house) {
  ...
  if (house === 'paulfosh') {
    // Existing auction_calendar rows can retain the retired EIG hostname.
    // Pin every Paul Fosh scrape to the branded, wide list view so stale rows
    // cannot bypass HOUSE_ROOTS or omit a newly appended final page/card.
    return { baseUrl: HOUSE_ROOTS.paulfosh, isApi: false, paginateAs: null };
  }

  if (house === 'tcpa') {
    // The national EIG search defaults to 50 cards, and regional/stale
    // calendar URLs select an even narrower surface. Pin every TCPA scrape to
    // the complete national list (verified 2026-07-26: 354 live UUID lots).
    return { baseUrl: HOUSE_ROOTS.tcpa, isApi: false, paginateAs: null };
  }

  if (house === 'savills') {
    // Savills: auctions.savills.co.uk/auctions/{d[--d]-month-year-{auctionId}} —
    // fully server-rendered, page size is a PATH segment. Forcing
    // /page-1/quantity-500 puts the WHOLE catalogue in one static fetch, so the
    // paginated render path is unnecessary (paginateAs stays null — appending
    // /page-N after /quantity-500 would produce a nonsense URL).
    //
    // The cron path re-resolves every upcoming sale from the calendar
    // (resolveSavillsCatalogueUrl); this rewrite is what keeps the on-demand
    // /analyse path off the 10-lots-per-page default.
    const SAVILLS_WIDE = (base) => `${base.replace(/\/+$/, '')}/page-1/quantity-500`;
    const today = new Date().toISOString().slice(0, 10);
    const dated = /^(https?:\/\/(?:www\.)?auctions\.savills\.co\.uk\/auctions\/([a-z0-9-]+-\d+))(?:\/|$)/i.exec(url);
    // A pinned dated URL is honoured ONLY while its sale is still running. The
    // auction id rotates every sale, so a stale auction_calendar row otherwise
    // pins us to a FINISHED catalogue — whose ended lots must never be scraped
    // as current. A past sale falls through to calendar discovery below.
    if (dated && parseSavillsAuctionSlug(dated[2])?.endIso >= today) {
      return { baseUrl: SAVILLS_WIDE(dated[1]), isApi: false, paginateAs: null, preferPuppeteer: true };
    }
    // Calendar URL (e.g. /upcoming-auctions) — drill to the SOONEST upcoming
    // sale. Ordering by parsed date (not document order) matters: the calendar
    // is not guaranteed sorted and may retain a just-finished sale.
    if (u.includes('auctions.savills.co.uk')) {
      try {
        const resp = await fetch('https://auctions.savills.co.uk/upcoming-auctions', { headers: HEADERS, redirect: 'follow' });
        if (resp.ok) {
          const html = await resp.text();
          const dates = [];
          const seen = new Set();
          for (const m of html.matchAll(/href="(https?:\/\/(?:www\.)?auctions\.savills\.co\.uk\/auctions\/([a-z0-9-]+))"/gi)) {
            const slug = m[2].toLowerCase();
            if (seen.has(slug)) continue;
            const info = parseSavillsAuctionSlug(slug);
            if (!info || info.endIso < today) continue;
            seen.add(slug);
            dates.push({ iso: info.endIso, url: `https://auctions.savills.co.uk/auctions/${slug}` });
          }
          dates.sort((a, b) => a.iso.localeCompare(b.iso));
          if (dates.length > 0) {
            console.log(`Savills: auto-discovered catalogue ${dates[0].url} (${dates[0].iso}) from ${url}`);
            return { baseUrl: SAVILLS_WIDE(dates[0].url), isApi: false, paginateAs: null, preferPuppeteer: true };
          }
        }
  ...
  if (house === 'loveitts') {
    // Pin every Loveitts scrape to the live franchise catalogue. Stale calendar
    // rows still point at EIG live-stream embeds or /upcoming-auctions/{id}
    // shells that are not the lot grid.
    return { baseUrl: HOUSE_ROOTS.loveitts, isApi: false, paginateAs: null };
  }
  ...
  if (house === 'underthehammer') {
    // Next.js SPA whose catalogue page ships an empty shell — the AI extractor
    // saw ~9 of 161 lots and died with the AI quota. The site hydrates from a
    // plain, server-fetchable JSON endpoint, so force EVERY underthehammer URL
    // (stale calendar rows, the /for-auction/properties landing page, a lot
    // page) at the API. Consumed by lib/scraper/underthehammer.js via
    // scrape-stage's `underthehammer_api` branch. `status=upcoming` narrows
    // 285 records → 161 live; the scraper re-checks status + auction end date
    // client-side so an ended lot can never persist as available.
    return {
      baseUrl: UTH_CATALOGUE_URL,
      isApi: true,
      paginateAs: 'underthehammer_api',
    };
  }
  ...
  if (house === 'pattinson') {
    // Next.js + Payload CMS behind Cloudflare, ~1,780 lots across 90 FIXED pages
    // of 20 — the render path capped at MAX_PUPPETEER_PAGES=15 could only ever
    // ship ~17%. The bespoke scraper (lib/scraper/pattinson.js) renders page 1
    // once to clear Cloudflare, then walks the site's own /api/property/list-search
    // endpoint from inside that session. Force EVERY pattinson URL (stale
    // calendar rows, a lot page, the homepage) at the canonical catalogue so the
    // in-page paginator's host gate and the endpoint's page walk always line up.
    return {
      baseUrl: PATTINSON_CATALOGUE_URL,
      isApi: false,
      paginateAs: 'pattinson_api',
    };
  }
  ...
  // Sutton Kersh: static HTML gallery. `?perPage=all` returns the WHOLE current
  // auction (97 lots, 211KB) in ONE fetch — the default 16-per-page + start=N walk
  // only ever landed page 1, and the house had no recogniser so it leaned on the AI
  // extractor and went dark whenever the AI quota died. Force perPage=all, drop
  // pagination, and let recogniseSuttonKershLotsFromMarkdown parse it statically.
  if (house === 'suttonkersh') {
    let baseUrl = url;
    try {
      const u = new URL(url);
      // Repair query-param CASE before anything else. Calendar URLs are stored
      // through TWO lowercasing layers — the `normalise_calendar_url` DB trigger
      // (`NEW.url := lower(...)`) and `normaliseUrl` in lib/utils.js — so
      // `auctionPeriod` comes back as `auctionperiod`. Sutton Kersh's param is
      // CASE-SENSITIVE, and combined with perPage=all the lowercased form returns
      // the entire ARCHIVE instead of the current sale. Verified live 2026-07-22:
      //   auctionPeriod=current&perPage=all ->     86,259 bytes,     22 lots
      //   auctionperiod=current&perPage=all -> 15,370,123 bytes, 11,476 lots
      // That 15MB fetch times out into a 0-lot run, which is what kept re-opening
      // the circuit breaker and left the house dark with a healthy extractor.
      // Repair here, not in the DB: the trigger re-lowercases every write, so a
      // corrected auction_calendar row cannot survive.
      for (const [k, v] of [...u.searchParams]) {
        if (k !== 'auctionPeriod' && k.toLowerCase() === 'auctionperiod') {
          u.searchParams.delete(k);
          u.searchParams.set('auctionPeriod', v);
        }
      }
      u.searchParams.set('perPage', 'all');
      u.searchParams.delete('start');
      baseUrl = u.toString();
    } catch { /* keep url as-is if unparseable */ }
    return { baseUrl, isApi: false, paginateAs: null, preferPuppeteer: false };
  }
  // Sequence / Connells branches (Bagshaws, Fox & Sons, William H Brown Norwich):
  // one hand-maintained static HTML page carries each branch's whole catalogue.
  // Pin every URL on these hosts to the house's own HOUSE_ROOTS entry so a stale
  // calendar row or a healed sub-page can never become the scrape target — all
  // three sites serve an Auction-Results.html that lists PAST sales, and healing
  // onto it would silently swap the live catalogue for an ended one.
  // Verified 2026-07-22: each pinned page carries 100% of its house's lots
  // (bagshaws 23, foxandsons 21, williamhbrownnorwich 19). Plain HTTP;
  // recogniseSequenceBranchLotsFromMarkdown parses it via the staticCatalogue path.
  if (house === 'bagshaws' || house === 'foxandsons' || house === 'williamhbrownnorwich') {
    return { baseUrl: HOUSE_ROOTS[house], isApi: false, paginateAs: null, preferPuppeteer: false };
  }
  ...
  // Retired 2026-07-05 (Simon-approved): dead AH branches merged into /midlands;
  // sequence is a portal with no own catalogue. blocked:true so a stale calendar
  // row or direct URL can never re-scrape them.
  if (house === 'auctionhouseeastmidlands' || house === 'auctionhousewestmidlands' || house === 'sequence') {
    console.log(`${house}: skipped — slug retired (branch/portal consolidated, 2026-07-05)`);
  ...
  // auctionhousebirmingham + starpropertyonline were hard-blocked as "persistently
  // returns 0 lots". Both blocks were STALE (unblocked 2026-07-10): the 0 lots came
  // from a failing browser render / a quota-dead AI extractor, NOT an empty catalogue.
  // Birmingham's static HTML yields 94 lots at 100% recall through the (now
  // staticCatalogue) AuctionHouse platform recogniser; starpropertyonline is an EIG
  // OAS house that resolves through the EIG current-auction recogniser and returns a
  // correct genuine zero while it is between auctions. A permanent block guarantees a
  // house can never recover — exactly the "house left in limbo" the 100% commandment
  // forbids. If either regresses, fix the cause; do not re-block.
  ...
  // GOTO Properties / Purplebricks: EIG OAS, server-rendered. The old ?page=N
  // walk under-captured (1,124 of 2,867) and then broke to 0 (circuit open,
  // mid-June). ?pagesize=5000 returns the WHOLE catalogue (~2,867 lots) in ONE
  // static fetch WITH inline addresses/prices — force it for ANY purplebricksgoto
  // URL and let recognisePurplebricksGotoLotsFromMarkdown parse it via the
  // staticCatalogue HTTP path (a browser render re-hydrates and breaks capture).
  if (house === 'purplebricksgoto') {
    return { baseUrl: 'https://purplebricks.gotoproperties.co.uk/search?pagesize=5000', isApi: false, paginateAs: null, preferPuppeteer: false };
  }
