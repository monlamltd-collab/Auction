...
export const HOUSE_ROOTS = {
  ...
  // johnpye — REMOVED: not a property auction site (commercial/industrial asset disposal)
...
export function detectAuctionHouse(url) {
  ...
  // johnpye detection removed — not a property auction site
  // ── EIG batch 5 houses ──
...
export const HOUSE_DISPLAY_NAMES = {
  ...
  strakers: 'Strakers',
  // johnpye retired — not a property auction site
  // ── Batch 7 (March 2026) ──
...
export async function rewriteUrl(url, house) {
  ...
  if (house === 'sdl') {
    // sdl = the BTG Eddisons / SDL-network catalogue (HOUSE_ROOTS.sdl points here).
    ...
    // ANY sdl row and let recogniseBtgEddisonsLotsFromMarkdown parse it (single
    // page, no ?page=N walk). Server HTML, so no Puppeteer needed. 2026-06-14.
    return {
