...
export const AUCTION_DISCOVERY = {
  ...
  // The "current" auction always lives at the same auction=1 / bid=N URL —
  // the ID doesn't increment per event, the page content swaps over once
  // the previous auction completes. The watcher's job for these houses is
  // to (a) confirm the URL still resolves and (b) read the next auction
  // date from the page header so the calendar stays accurate. The
  // auctionsIndexPath fallback gives the watcher a cleaner page to read
  // upcoming dates from when the homepage doesn't carry them.
  maggsandallen: {
    homepage: 'https://www.maggsandallen.co.uk/',
    auctionsIndexPath: '/auctions/',
    platform: 'eig-whitelabel',
    linkPattern: /\/search-auction\/\?auction=(\d+)/g,
    buildUrl: (id) => `https://www.maggsandallen.co.uk/search-auction/?auction=${id}&orderby=lot_no&n=0&showsold=on&showstc=on`,
  },
  hollismorgan: {
    ...
    buildUrl: (id) => `https://www.hollismorgan.co.uk/search-auction/?bid=${id}&showstc=on&orderby=lot_no+asc`,
  },
  buttersjohnbee: {
