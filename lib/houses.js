...
export const RETIRED_HOUSES = new Set([
  ...
  'hammertime',
  // ── Merged into BTG Eddisons (sdl) (confirmed 2026-05-20) ──
  'network',
]);
...
export function detectAuctionHouse(url) {
  ...
  if (u.includes('networkauctions')) return 'sdl';
...
export async function rewriteUrl(url, house) {
  ...
  // johnpye: not a property auction site — commercial/industrial asset disposal.
  // Scraping produces non-property lots that pollute the directory.
  if (house === 'johnpye') {
    console.log(`${house}: skipped — not a property auction site`);
    return { baseUrl: url, isApi: false, paginateAs: null, preferPuppeteer: false, blocked: true };
  }

  // humberts: general estate agency, not an auction house. Catalogue has
  // almost no auction properties — pollutes the directory with non-auction stock.
  if (house === 'humberts') {
    console.log(`${house}: skipped — estate agent, not an auction house`);
    return { baseUrl: url, isApi: false, paginateAs: null, preferPuppeteer: false, blocked: true };
  }
