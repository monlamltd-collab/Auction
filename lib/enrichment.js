 ...
 *   1. Same postcode, same beds, ≥3 listings → median × unit count
 *   2. Same postcode, any beds (excludes room-shares), ≥3 listings → median
 *   3. Static estimateMonthlyRent (always returns something)
 *
 * Returns: { rent: number, source: 'comps_typed' | 'comps_postcode' | 'static',
 *            sample: number, postcode?: string }
 *
 * COVERAGE_FIX_PLAN.md rollout #7 — turns the SpareRoom + OnTheMarket
 * scrape into a real signal consumed by est_gross_yield.
 */
export async function estimateMonthlyRentSmart({ address, beds, units, postcode }) {
  const unitCount = (units && units >= 2) ? units : 1;
  ...
  try {
    // Tier 1: same postcode, same beds (whole-property listings only —
    // room-shares are per-room rents, useful for HMO yield but
    // misleading for whole-property comps).
    if (typeof beds === 'number' && beds >= 0 && beds <= 10) {
      ...
      if (typed && typed.length >= 3) {
        const med = median(typed.map(r => r.rent_pcm));
        return { rent: Math.round(med * unitCount), source: 'comps_typed', sample: typed.length, postcode };
      }
    }

    // Tier 2: same postcode, any beds (still excluding room-shares).
    const { data: any } = await supabase
      ...
      .gt('rent_pcm', 0);
    if (any && any.length >= 3) {
      const med = median(any.map(r => r.rent_pcm));
      return { rent: Math.round(med * unitCount), source: 'comps_postcode', sample: any.length, postcode };
    }
  } catch (err) {
