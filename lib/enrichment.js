...
// Canonical casing for postcode_sales.property_type. The retired SPARQL
// endpoint emitted lowercase slugs ('terraced', 'flat-maisonette') which
// still re-enter via cached lr_data fan-outs, splitting the vocabulary
// ('Terraced' vs 'terraced'). Normalise at the write site only — in-memory
// sales keep their original propertyType for the type-matched comps regex.
const PROPERTY_TYPE_CANON = {
  'terraced': 'Terraced',
...
export function canonPropertyType(type) {
  if (!type) return null;
  return PROPERTY_TYPE_CANON[String(type).toLowerCase()] || type;
}
...
export function buildLotUrl(lot, house, sourceUrl) {
  ...
  switch (house) {
    ...
    case 'btgeddisons':
      // BTG Eddisons (formerly 'sdl' slug): property pages are /properties/{id}/for-auction-{slug}
      if (lot.url && lot.url.startsWith('http')) return lot.url;
    ...
    case 'bondwolfe':
      ...
      break;
    case 'network':
      // Network Auctions: folded into btgeddisons 2026-06-21 (slug 'network' retired).
      // Kept here so any legacy-URL lots (networkauctions.co.uk) still build correctly.
      if (lot.url && lot.url.startsWith('/')) {
    ...
    case 'paulfosh':
      if (lot.url && lot.url.startsWith('/')) {
        return `https://auction.paulfosh.com${lot.url}`;
      }
      break;
    case 'cottons':
  ...
  // No lot-specific URL: return empty, NOT the catalogue URL. Stamping the
  // catalogue URL here (the old `|| sourceUrl`) gave every urlless lot the same
  // page identity — persist then stored it as lots.url, so a degraded run's
  // lots squatted the UNIQUE(url) slot and were served as fake "available"
  // page-lots (2026-07-22; the AI extractor path, which never reaches the
  // isNonLotUrl guard, is the main producer of urlless lots). An empty URL lets
  // persist mint a STABLE per-lot synthetic key and match the lot to its
  // real-URL twin by property key — see persist-lots.js.
  return lot.url || '';
}
...
// Migrated 2026-06-13: the old epc.opendatacommunities.org Basic-auth API was
// RETIRED 30 May 2026. Now uses the "Get energy performance of buildings data"
// service (https://api.get-energy-performance-data.communities.gov.uk) with a
// Bearer token in EPC_API_TOKEN. The search response carries the rating BAND +
// address + UPRN but NOT the numeric score / floor area / lmk-key (those need a
// per-certificate fetch — future enhancement); matchEPCToLot tolerates the
// missing score. Legacy EPC_API_EMAIL/EPC_API_KEY are dead (old host 301s).
const EPC_API_TOKEN = process.env.EPC_API_TOKEN || '';
const EPC_API_BASE = 'https://api.get-energy-performance-data.communities.gov.uk/api';
if (!EPC_API_TOKEN) {
  console.warn('WARNING: EPC_API_TOKEN not set — EPC enrichment will be skipped');
}

let _lastEPCCallTime = 0;

/**
 * Map a record from the new EPC search API (camelCase, band-only) into the
 * field names matchEPCToLot reads. Pure + exported for tests.
 * @param {object} rec one element of the new API's `data` array
 * @returns {object|null}
 */
export function normaliseEpcSearchRecord(rec) {
  if (!rec) return null;
  return {
    address1: rec.addressLine1 || '',
    address2: rec.addressLine2 || '',
    address3: [rec.addressLine3, rec.addressLine4].filter(Boolean).join(' '),
    currentEnergyRating: rec.currentEnergyEfficiencyBand || '',
    lodgementDate: rec.registrationDate || '',
    uprn: rec.uprn || null,
    lmkKey: rec.certificateNumber || null,
  };
}

/**
 * Fetch EPC records for a postcode from the MHCLG "Get energy performance of
 * buildings data" API (Bearer auth).
...
export async function fetchEPCByPostcode(postcode) {
  if (!EPC_API_TOKEN) return { status: 'skipped_no_creds', records: null };
  ...
  try {
    ...
    const url = `${EPC_API_BASE}/domestic/search?postcode=${encoded}&page_size=5000`;
    ...
    // The new API returns 404 with body {"data":{"error":"No certificates..."}}
    // when a postcode has no domestic EPCs — that's an EMPTY result, not a
    // failure, so it must NOT trip the breaker.
    if (response.status === 404) {
      epcBreaker.recordSuccess();
      return { status: 'api_empty_for_postcode', records: [] };
    }
    ...
    epcBreaker.recordSuccess();
    if (!Array.isArray(rawRows)) return { status: 'api_error', records: null };
    const records = rawRows.map(normaliseEpcSearchRecord).filter(Boolean);
    return { status: records.length > 0 ? 'ok' : 'api_empty_for_postcode', records };
  } catch (e) {
 ...
 */
/**
 * Parse the new API's full-certificate response (GET /api/certificate) into the
 * numeric fields the band-only search can't supply (score, floor area, beds).
 * Pure + exported for tests.
 * @param {object} data the `data` object from the certificate response
 */
export function parseEpcCertificate(data) {
  if (!data || typeof data !== 'object') {
    return { epcScore: null, epcFloorAreaSqm: null, epcFloorAreaSqft: null, epcBeds: null, band: null };
  }
  const score = parseInt(data.energy_rating_current, 10);
  const floorAreaSqm = parseFloat(data.total_floor_area);
  const beds = parseInt(data.heated_room_count, 10);
  const validSqm = floorAreaSqm > 0 && floorAreaSqm < 10000;
  return {
    epcScore: (score >= 1 && score <= 100) ? score : null,
    epcFloorAreaSqm: validSqm ? floorAreaSqm : null,
    epcFloorAreaSqft: validSqm ? Math.round(floorAreaSqm * 10.7639) : null,
    epcBeds: (beds >= 1 && beds <= 20) ? beds : null,
    band: (data.current_energy_efficiency_band || '').toUpperCase() || null,
  };
}

/**
 * Fetch a single full EPC certificate by number from the new API. The band-only
 * search (fetchEPCByPostcode) gives the rating but no numeric score/floor-area;
 * this fills them for a matched lot. Shares the rate limiter + breaker.
 * @returns {Promise<{ status: string, epcScore?, epcFloorAreaSqm?, epcFloorAreaSqft?, epcBeds?, band? }>}
 */
export async function fetchEPCCertificate(certificateNumber) {
  if (!EPC_API_TOKEN) return { status: 'skipped_no_creds' };
  if (!certificateNumber) return { status: 'skipped_no_lmk_key' };
  ...
  try {
    const url = `${EPC_API_BASE}/certificate?certificate_number=${encodeURIComponent(certificateNumber)}`;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 10000);
    const response = await fetch(url, {
      headers: { 'Authorization': `Bearer ${EPC_API_TOKEN}`, 'Accept': 'application/json' },
      signal: controller.signal,
    });
    clearTimeout(timeout);

    if (response.status === 404) { epcBreaker.recordSuccess(); return { status: 'not_found' }; }
    if (!response.ok) { epcBreaker.recordFailure(); return { status: 'api_error' }; }

    const json = await response.json();
    epcBreaker.recordSuccess();
    return { status: 'ok', ...parseEpcCertificate(json.data || json) };
  } catch (e) {
    if (e.name === 'AbortError') { epcBreaker.recordFailure(); return { status: 'timeout' }; }
    epcBreaker.recordFailure();
...
export async function fetchEPCRecommendations(lmkKey) {
  // Recommendations are NOT yet migrated to the new EPC API. The old
  // epc.opendatacommunities.org recommendations endpoint was retired with the
  // rest of that service (30 May 2026); the new service exposes the data via
  // GET /api/certificate?certificate_number={cn} (a different shape — a
  // follow-up to wire up + parse for score / floor-area / recommendations).
  //
  // CRITICAL (2026-06-13 migration incident): this must NOT reference the
  // removed EPC_API_EMAIL/EPC_API_KEY (ReferenceError) and must NOT call the
  // dead old endpoint — doing so threw on every matched lot and tripped the
  // SHARED epcBreaker, which then short-circuited the band lookup
  // (fetchEPCByPostcode) and held EPC at circuit_open fleet-wide. The rating
  // BAND from fetchEPCByPostcode is the primary EPC field; recommendations
  // (improvement-cost estimates) are a secondary nice-to-have we skip for now.
...
export function matchEPCToLot(epcRecords, lotAddress) {
  ...
  for (const rec of epcRecords) {
    ...
    const rating = (rec['current-energy-rating'] || rec.currentEnergyRating || rec.currentEnergyEfficiencyBand || '').toUpperCase();
    const epcScoreRaw = parseInt(rec['current-energy-efficiency'] || rec.currentEnergyEfficiency || '0', 10);
    const hasScore = epcScoreRaw >= 1 && epcScoreRaw <= 100;
    const date = rec['lodgement-date'] || rec.lodgementDate || rec.registrationDate || '';

    if (!/^[A-G]$/.test(rating)) continue;
    // A score PRESENT but out of range (>100) is junk → skip. But the new EPC
    // search API returns NO score (band only), so an absent/zero score is NOT a
    // reason to skip — band-only matches keep epcScore=null. (The old full-
    // record API always supplied a score 1-100.)
    if (epcScoreRaw > 100) continue;
    ...
    if (!bestMatch || score > bestStreetScore || (score === bestStreetScore && date > bestDate)) {
      bestMatch = {
        epcRating: rating,
        epcScore: hasScore ? epcScoreRaw : null,
        epcDate: date,
...
export async function enrichLots(lots, house, sourceUrl, onProgress) {
  ...
  for (const lot of lots) {
      ...
      try {
        const rows = sales
          ...
          .map(s => ({
            ...
            sold_date: s.date,
            property_type: canonPropertyType(s.propertyType),
    ...
    if (relevantSales.length > 0) {
      ...
      if (isBlockLot(lot)) {
        ...
        if (lot.price && avg > 0) {
          ...
          // Extreme discounts vs street comps (≥70% below, ratio ≤0.30) are
          // almost always land/parking/token guides — keep the belowMarket
          // number for transparency but do NOT award score points or "bargain"
          // opportunity tags that pollute Top picks / best-BMV hunts.
          const extremeBargain = discount >= 70;
          if (compReliable && discount > 20 && eligible && !extremeBargain) {
            lot.score += 2;
          ...
          } else if (compReliable && discount > 10 && eligible && !extremeBargain) {
            lot.score += 1;
            lot.scoreBreakdown = lot.scoreBreakdown || [];
            lot.scoreBreakdown.push({ signal: `${lot.belowMarket}% below market`, pts: 1 });
            lot.opps.push(`${lot.belowMarket}% below market`);
            recordBelowMarketScoring(lot._enrichment);
          } else if (extremeBargain && eligible) {
            // Close the double-score gate without awarding points.
            recordBelowMarketScoring(lot._enrichment);
            if (!lot.risks) lot.risks = [];
            if (!lot.risks.some(r => /not comparable|token guide|far below local/i.test(r))) {
              lot.risks.push('Guide far below local sold prices — may be land, parking or a nominal figure');
            }
          } else if (discount < -10) {
    ...
    if (lot.price && lot.price > 0 && lot.estAnnualRent) {
      ...
      // Flag unrealistic yields — typically caused by very low / token guide
      // prices (e.g. £1.5k guide on a £250k street, or a parking space rented
      // as if it were a whole house). Also flag when discount vs street comps
      // is extreme (≥70% below) so scoring and UI can suppress heroic % returns.
      const streetRef = lot.streetAvg || lot.comparablePrice || null;
      const extremeVsStreet = streetRef >= 50000 && lot.price / streetRef <= 0.30;
      if (lot.estGrossYield > 30 || extremeVsStreet) {
        lot._yieldEstimateWarning = true;
        if (!lot.risks) lot.risks = [];
        if (!lot.risks.some(r => /yield.*unrealistic|verify.*rent|guide.*not comparable|token guide/i.test(r))) {
          if (extremeVsStreet && lot.estGrossYield <= 30) {
            lot.risks.push('Guide far below local sold prices — treat % returns as not comparable');
          } else {
        ...
        for (const lot of lotsForPc) {
          ...
          if (['skipped_no_creds', 'circuit_open', 'api_error', 'timeout', 'skipped_no_postcode'].includes(epcApiStatus)) {
            ...
            if (epcMatch) {
              ...
              lot.epcScore = epcMatch.epcScore;        // null from the band-only search
              ...
              // Harvest UPRN free from the address-matched EPC certificate. OS
              // Places is the canonical UPRN source but it's paid (Royal Mail
              // PAF, excluded from the £1k/mo Premium free allowance) and our
              // trial is exhausted — the EPC record already carries the
              // property's UPRN at no extra cost. Fill it only when OS Places
              // hasn't (OS still wins when it runs). Verified 2026-06-14: the
              // EPC search API returns a numeric `uprn` per certificate.
              if (!lot.uprn && epcMatch.uprn && /^\d{6,}$/.test(String(epcMatch.uprn))) {
                lot.uprn = String(epcMatch.uprn);
                lot._uprnSource = 'epc';
              }

              // ── Full-certificate fetch — the new search API is band-only, so
              // fetch the matched certificate to fill the numeric score + floor
              // area (these feed value_estimate + £/sqft scoring). One extra
              // call per matched lot under the 6000/5min limit; shares the
              // breaker + rate limiter. Replaces the retired recommendations
              // call (that endpoint died with the old service).
              if (epcMatch.epcLmkKey && (lot.epcScore == null || lot.epcFloorAreaSqm == null)) {
                try {
                  const cert = await fetchEPCCertificate(epcMatch.epcLmkKey);
                  if (cert.status === 'ok') {
                    if (lot.epcScore == null) lot.epcScore = cert.epcScore;
                    if (lot.epcFloorAreaSqm == null) {
                      lot.epcFloorAreaSqm = cert.epcFloorAreaSqm;
                      lot.epcFloorAreaSqft = cert.epcFloorAreaSqft;
                    }
                    if (!lot.beds && cert.epcBeds) { lot.beds = cert.epcBeds; lot._bedsSource = 'epc'; }
                  }
                } catch (certErr) {
                  console.warn(`EPC certificate fetch failed for ${epcMatch.epcLmkKey}: ${certErr.message}`);
                }
              }

              // Fill sqft from the EPC floor area if the extractor didn't capture it
              if (!lot.sqft && lot.epcFloorAreaSqft) {
                lot.sqft = lot.epcFloorAreaSqft;
                lot._sqftSource = 'epc';
              }
              ...
              recordEpc(lot._enrichment, {
                ...
                floorAreaSqm: lot.epcFloorAreaSqm,
                floorAreaSqft: lot.epcFloorAreaSqft,
                addressCompleteness: 'full',
                matchConfidence: epcMatch._matchConfidence,
              });
              // Recommendations (improvement costs) are not yet migrated to the
              // new API — record the honest skip for observability.
              recordEpcRecommendations(lot._enrichment, { status: 'skipped_no_creds', lmk_key: epcMatch.epcLmkKey || null });
            } else {
