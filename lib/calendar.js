...
import { RESOLVER_ROOT_SLUGS } from './scraper/house-recognisers.js';
...
export const FALLBACK_CALENDAR = [
    ...
    // (Network Auctions rows removed 2026-06-21: slug 'network' retired, folded into btgeddisons)
    // ── BTG EDDISONS (slug: btgeddisons; formerly 'sdl') ──
    // BTG Eddisons runs rolling timed auctions — all current lots on /properties/
    {
      house: 'BTG Eddisons', houseSlug: 'btgeddisons', logo: '⚡',
    ...
    // (SDL Auctions rows removed 2026-06-21: SDL Auctions is a separate house,
    //  onboarded under slug 'sdlauctions' in plan 4. Do not re-add here.)
    // ── BOND WOLFE ──
    {
    ...
    // ── PAUL FOSH ──
    {
      ...
      url: 'https://auction.paulfosh.com/search',
    ...
    // ── LOVEITTS ──
    // Dates rotate; seed nearest known sale only. Watcher (loveitts-auctions.js)
    // owns multi-sale horizon. Catalogue is /auctions/ not EIG live-stream.
    {
      house: 'Loveitts', houseSlug: 'loveitts', logo: '❤️',
      date: '2026-07-28', title: '28 July 2026', lots: null,
    ...
    // Town & Country Property Auctions
    {
      ...
      url: 'https://www.townandcountrypropertyauctions.co.uk/search?pagesize=500',
      location: 'Online (Timed)', type: 'Residential & Commercial', status: 'upcoming',
      catalogueReady: true,
    },
    ...
    { house: 'Purplebricks (GOTO Properties)', houseSlug: 'purplebricksgoto', logo: '🔨', date: '2099-12-31', title: 'Current Catalogue', lots: null, url: 'https://purplebricks.gotoproperties.co.uk/search?pagesize=5000', location: 'Online', type: 'Residential & Commercial', status: 'always_on', catalogueReady: true },
    ...
    // (scargillmann row removed 2026-06-21: house retired — was mis-wired to sdlauctions.co.uk)
    ...
    // wrightmarshall removed 2026-07-04 — retired (estate agent; lots via the iamsold platform slug)
...
export function addResolverBackstops(rows = [], opts = {}) {
  const resolverSlugs = opts.resolverSlugs || RESOLVER_ROOT_SLUGS;
  const houseRoots = opts.houseRoots || HOUSE_ROOTS;
  const displayNames = opts.displayNames || HOUSE_DISPLAY_NAMES;
  const out = [...rows];
  const scheduledSlugs = new Set(rows.map(r => String(r?.houseSlug || '').toLowerCase()).filter(Boolean));
  const scheduledUrls = new Set(rows.map(r => normaliseUrl(r?.url)).filter(Boolean));
  for (const slug of resolverSlugs) {
    const root = houseRoots[slug];
    const house = displayNames[slug] || slug;
    const normalisedRoot = normaliseUrl(root);
    if (!root || scheduledSlugs.has(String(slug).toLowerCase()) || scheduledUrls.has(normalisedRoot)) continue;
    out.push({
      house,
      houseSlug: slug,
      url: root,
      date: null,
      catalogueReady: false,
      status: 'resolver_backstop',
      resolverBackstop: true,
    });
  }
  return out;
}
...
export async function getCalendarAuctions() {
  ...
  try {
    const { data, error } = await supabase
      .from('auction_calendar')
      .select('house, house_slug, url, date, catalogue_ready, status')
    ...
    if (!error && data && data.length > 0) {
      ...
      const scheduled = deduped.map(row => ({
        house: row.house,
        houseSlug: row.house_slug,
        url: row.url,
        date: row.date,
      ...
      return addResolverBackstops(scheduled);
    }
  ...
  return addResolverBackstops(fallbackFiltered
    .map(a => ({
      house: a.house,
      houseSlug: a.houseSlug,
      url: a.url,
      date: a.date,
