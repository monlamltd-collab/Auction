...
export const FLOOD_STATUSES = Object.freeze([
  ...
  'circuit_open',
  'api_error',
  'timeout',       // checkFloodRisk AbortError — was MISSING → recordFlood threw
                   // "unknown status" and aborted the EPC/Flood enrichment wave's
                   // batch (2026-06-16; same class as os_places cache_hit_no_match).
  'no_coords',     // lot had no lat/lng to query
]);
...
export const LR_STATUSES = Object.freeze([
  ...
  'circuit_open',
  'api_error',
  'timeout',           // request exceeded budget — parity with the other API recorders
]);
...
export const OS_PLACES_STATUSES = Object.freeze([
  ...
  'skipped_no_uprn',       // reverse-lookup path had no UPRN to query (os-places.js:492)
...
export const FLOOR_PLAN_STATUSES = Object.freeze([
  'captured',               // floor plan(s) found in detail-page markup and persisted to floor_plans[]
  'none_found',             // page parsed but no floor plan present (genuine absence — many lots have none)
  'url_dead',               // detail page 404/410
  'fetch_failed',           // network/timeout/anti-bot
]);

export const NARRATIVE_STATUSES = Object.freeze([
  'description_added',      // detail-page narrative extracted, lot.description now populated
  'no_description_found',   // page fetched/cached but no substantive narrative survived cleaning
  'url_dead',               // detail page 404/410
  'fetch_failed',           // network/timeout/anti-bot
]);
...
export const EPC_RECOMMENDATIONS_STATUSES = Object.freeze([
  ...
  'skipped_no_creds',      // EPC_API_TOKEN missing (mirrors fetchEPCByPostcode). Was
                           // MISSING here → recordEpcRecommendations threw "unknown
                           // status" and aborted the entire EPC/Flood enrichment wave
                           // (the 500-lots-missing-EPC/flood hygiene pass), 2026-06-16.
  'skipped_no_postcode',   // no postcode to query (mirrors the EPC/flood fetch chain)
  'api_error',             // non-2xx
  'timeout',
  'circuit_open',
]);
...
export function createManifest() {
  return {
    ...
    // Data-hygiene notes: transformations applied at the persist boundary to
    // keep a malformed source value from failing the write (e.g. a fractional
    // `beds` coerced to an integer). Records the per-lot audit trail the raw
    // console log can't give — "empty" vs "value was altered" stay distinct.
    data_hygiene: [],
  };
}
...
export function recordFloorPlanSweep(manifest, entry) {
  if (!entry || !entry.status) throw new Error('recordFloorPlanSweep: status is required');
  if (!FLOOR_PLAN_STATUSES.includes(entry.status)) {
    throw new Error(`recordFloorPlanSweep: unknown status "${entry.status}" — expected one of ${FLOOR_PLAN_STATUSES.join(', ')}`);
  }
  manifest.floor_plan_sweep = { ...entry, recorded_at: new Date().toISOString() };
  return manifest;
}

export function recordNarrativeSweep(manifest, entry) {
  if (!entry || !entry.status) throw new Error('recordNarrativeSweep: status is required');
  if (!NARRATIVE_STATUSES.includes(entry.status)) {
    throw new Error(`recordNarrativeSweep: unknown status "${entry.status}" — expected one of ${NARRATIVE_STATUSES.join(', ')}`);
  }
  manifest.narrative_sweep = { ...entry, recorded_at: new Date().toISOString() };
  return manifest;
}
...
/**
 * Note a value the persist boundary had to transform to keep the write from
 * failing — currently integer coercion of a fractional/unparseable source value
 * (`lotToDbRow.setInt`). Freeform by design (no status vocab): a rare hygiene
 * event, not a per-source lookup. Tolerates a missing manifest (no-op).
 */
export function recordFieldCoercion(manifest, { field, from, to, kind = 'int_coercion' } = {}) {
  if (!manifest || !field) return manifest;
  if (!Array.isArray(manifest.data_hygiene)) manifest.data_hygiene = [];
  manifest.data_hygiene.push({ kind, field, from, to });
  return manifest;
}
