...
export function extractWithJSDOM(html, house, baseUrl, firecrawlImages) {
  ...
  // banner/hero/catalogue image (or staff headshot), not a per-lot photo. Strip
  // it from all lots. URLs are normalised (query strings stripped, protocol/host
  // lower-cased) before comparison so cache-busting params like ?v=123 don't
  // defeat the check — this is what made Charles Darrow's auctioneer headshot
  // sneak through on every lot.
  const _normaliseImgUrl = (u) => {
    if (!u) return '';
    try {
  ...
  if (lots.length >= 3) {
    const imgCounts = {};
    const urlByNormalised = {}; // normalised → set of original URLs so we can null them all
    for (const lot of lots) {
      if (!lot.imageUrl) continue;
      const norm = _normaliseImgUrl(lot.imageUrl);
      imgCounts[norm] = (imgCounts[norm] || 0) + 1;
      (urlByNormalised[norm] = urlByNormalised[norm] || new Set()).add(lot.imageUrl);
    }
    for (const [norm, count] of Object.entries(imgCounts)) {
      if (count > lots.length * 0.5) {
        console.log(`[IMG] ${house}: stripped duplicate image appearing on ${count}/${lots.length} lots: ${norm.substring(0, 80)}`);
        const toStrip = urlByNormalised[norm];
        for (const lot of lots) {
          if (lot.imageUrl && toStrip.has(lot.imageUrl)) lot.imageUrl = null;
        }
      }
    }
  }
