      ...
      for (const card of cards) {
        ...
        if (!status) {
          ...
          if (/\\bSold\\s*(?:@|for)\\b/i.test(text) || /\\bSALE\\s*AGREED\\b/i.test(text)
            ...
            // Heuristic — only mark as sold if the price block itself talks
            ...
            if (/SOLD|SALE\\s*AGREED|WITHDRAWN/.test(priceText)) {
              status = priceText.match(/SOLD|SALE\\s*AGREED|WITHDRAWN/)[0];
            }
          }
        }
        if (/^SOLD\\b|^SALE\\s*AGREED|^WITHDRAWN/.test(status)) continue;

        // ── Lot number ──
        // Hollis shows "Lot TBC" for upcoming, Maggs shows "LOT 1". Take the
        // first integer we see in a lot-y element.
