...
**John Pye** is the reference for *template-rebuild drift that fails loudly-looking-quiet*: the site moved to an Avada/Fusion post-card grid where each card opens with an **empty anchor** and carries its title on the *next* line. The old recogniser split markdown on `\n- {CAPS}` and read each block's first line, so it returned 2 fabricated "lots" (the page `<title>` and the words "Auction Location") rather than 0 — real recall was 0 and the frontend was being fed by the quota-dead AI fallback, which leaked an Under-Offer lot as available and a lettings card as a sale. `recogniseJohnPyeLotsFromMarkdown` now delimits cards by **runs of same-slug lot links**, reads status from the **whole card** (some lots carry "UNDER OFFER" only on the button label), takes the address from the title segment carrying the postcode, and parses `Auction Ends | Thursday 30th Jul | 11:00am` for the timed lots. Registered `staticCatalogue` — one plain-HTTP fetch, no AI. Verified 17/17 = 100% recall (9 available / 8 sold, 0 leaks).
