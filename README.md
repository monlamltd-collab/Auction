...
In `HOUSE_RECOGNISERS` — currently John Pye, McHugh & Co, Mark Jenkinson, Maggs & Allen, Hollis Morgan, Nesbits, Bond Wolfe, Property Solvers, Auction House London, BTG Eddisons, Charles Darrow, SDL Auctions, Clive Emson, Sutton Kersh, Future Property Auctions, Edward Mellor and Savills, plus the Auction House UK platform recogniser shared across the ~33 `auctionhouse.co.uk/{region}` franchise sites. Reads the same markdown (Firecrawl's, or the turndown bridge's when Crawlee renders) to recover lots the JSON extractor missed. Some houses (e.g. Nesbits) list lots as image-only links with no inline text — the recogniser harvests the lot URLs and seeds the address from the URL slug, leaving the first-contact deep-fetch to fill price/date/beds. Recogniser URL regexes are host-tolerant (`(?:www\.)?`) because Crawlee renders on whichever host the calendar URL uses. On the Crawlee path the recogniser also corroborates the AI extractor: for lot ids both saw, the recogniser's deterministically-parsed status (and hero image, when the extractor has none) wins — AI status inference smears SOLD/STC across overlay-heavy pages. Some houses (e.g. Bond Wolfe) append their catalogue via a WordPress "Load more" button behind Cloudflare with a JS-injected ajax nonce, so a plain-HTTP API consumer is impossible — the Crawlee render clicks "Load more" to exhaustion (`CLICK_TO_LOAD_SELECTORS` in `crawlee.js`) before the recogniser parses the rendered cards.

The **Bamboo Auctions** platform (`{house}.bambooauctions.com` — hunters, lsk, stags, carterjonas, fisherGerman, webbers, allwalesauction, hawkesford, howkinsandharrison, rendells, 247propertyauctions) shares one recogniser (`recogniseBambooLotsFromMarkdown`) resolved via `resolvePlatformRecogniser`. It's Next.js SSR, so a plain HTTP fetch returns every card (`staticCatalogue`) — none of the 11 houses had a recogniser, so all depended on the AI extractor and several went dark. The recogniser also parses the **SOLD** badge deterministically: most Bamboo cards are sold-prior (howkinsandharrison 18 of 20), and the AI extractor smears them as `available`. Two subtleties it handles: the `###` heading is often a short *title* with the full postal address on the next line, and lot slugs may contain a **dot** (an id-shaped anchor regex silently drops those cards). Verified 100% recall on all 11 live catalogues.

**Future Property Auctions** is the reference for the *paginated static catalogue*: a classic ASP SSR site whose rolling timed-online catalogue (749 entries) is addressed by `?offset=N` in steps of 21. `recogniseFutureAuctionsLotsFromMarkdown` parses cards whose price/lot/photo sit *before* the type-heading anchor and whose address/auction-date sit *after* it, and upgrades its `http://` photos to `https` (the frontend is HTTPS-only). A generic `staticPaginate: { param, step }` walker in `lib/analysis.js` drives it — kept separate from the EIG live-boundary walker. Verified 749/749 = 100% recall.

**Sutton Kersh** is the reference for the *AI-independence* case: it had no recogniser and leaned entirely on the AI extractor, so it went dark whenever the AI quota died. `rewriteUrl` now forces `?perPage=all` (the whole current auction, 97 lots, in ONE server-rendered fetch) and `recogniseSuttonKershLotsFromMarkdown` parses it deterministically — including per-card SOLD-prior / withdrawn status, so a sold lot never persists as `available`. Verified 97/97 = 100% recall against the live page.

**Savills** is the reference for the *concurrent-sales calendar drill*, and for the reminder that **a dead recall sentinel hides a total blackout**. It sat at 0 live for two compounding reasons: no recogniser (so it depended on the quota-dead AI extractor) *and* a sentinel whose `(?=$|[/?#])` lookahead matched no lot URL in either markdown's `](…)` or HTML's `href="…"`, so recall computed 0/0 and the harness never fired. `resolveSavillsCatalogueUrl` reads the `/upcoming-auctions` **calendar** — never a pinned catalogue row, since the dated auction id rotates every sale — and returns **one page target per upcoming sale**, sized from the calendar's own "N properties for sale" count (`/page-{n}/quantity-100`, a PATH segment). Unlike Edward Mellor, "soonest sale only" would be a partial: Savills runs several sales concurrently and the later ones already carry live lots. `recogniseSavillsLotsFromMarkdown` parses the SSR cards deterministically, treating each card's own `[Full details]` link as the terminator so the interleaved **section-divider pseudo-lots** ("Lot 0", empty address, `Guide Price TBA`) can't bleed their fields into the next real lot, and reading SOLD/WITHDRAWN-prior status only from the zone between `Your Bid` and the address anchor so bullet prose ("sold off on long lease") never flips an available lot. This is what `resolveCatalogueUrl` returning a **list** exists for (`staticCatalogue` block in `lib/analysis.js`); single-target houses are unaffected. Verified 306/306 = 100% across all four upcoming sales. Dossier: [`docs/houses/savills.md`](docs/houses/savills.md).

The **EIG "Online Auction System" (OAS)** cluster (~25 houses on `{sub}.eigonlineauctions.com/search`, custom domains, and `eigpropertyauctions.co.uk/live-stream` embeds) shares one recogniser (`recogniseEigOasLotsFromMarkdown`) driven by a **paginated static-catalogue** path (`staticCatalogue` + `paginateStatic` in `lib/analysis.js`). Their `/search` page returns the *full archive* behind a **signed token** — raw `?Order=`/`?auctionId=`/`?status=` params are ignored — so the recogniser is the current-auction guard: it keeps only affirmatively-live lots (dropping any card with an ended marker, a past date, or no positive live signal) and stamps each lot's real future auction date, while the pagination walks `?pagesize&page` in native order and **stops at the first page with zero live lots** (the live↔ended boundary), fetching only the live head rather than the multi-thousand-lot archive. On zero live lots it returns a genuine zero and never falls through to the AI extractor (which would re-ingest the ended archive as available). Registered per verified house via `resolvePlatformRecogniser` behind an allowlist expanded as each house is verified.
...
**7. Pattinson in-page paginator exception**
`lib/scraper/pattinson.js` — `pattinson.co.uk` compounds two blockers: Cloudflare 403s any non-browser request (GET on the catalogue *and* POST on its JSON endpoint), **and** the catalogue is ~1,780 lots across 90 **fixed** 20-lot pages, far past `MAX_PUPPETEER_PAGES=15`. That capped the house at ~17%. The fix renders page 1 **once** through Crawlee — clearing Cloudflare and leaving a warm session — then walks the site's own `POST /api/property/list-search` endpoint from **inside** that page context, because `cf_clearance` is HttpOnly and fingerprint-bound and cannot be lifted into a node fetch. Implemented as the host-gated `IN_PAGE_PAGINATORS` hook in `crawlee.js` (house-agnostic `collectPagedJson` mechanics + per-host endpoint config, mirroring `CLICK_TO_LOAD_SELECTORS`); the records ride back on `scrapeWithCrawlee`'s `inPageData`. Compact JSON (~66KB/page) — never concatenated HTML, which would be ~117MB and OOM. Dispatched on `paginateAs:'pattinson_api'`. **The catalogue is not all live**: 59 of 1,783 records had already ended, so two independent gates (`isSold !== true` **and** a deadline that is absent or strictly in the future, compared as full timestamps) keep ended lots out; a missing deadline is a live in-room lot, not an ended one. Verified 1,724/1,724 live lots, peak RSS 205MB. Dossier: [`docs/houses/pattinson.md`](docs/houses/pattinson.md).
...
```bash
npm test           # runs all 125 test files
```

Key test files: `test-scoring.js`, `test-finance-metrics.js` (deterministic investor metrics in `public/finance.js` — SDLT port parity vs bridgematch-lite, net yield, ROCE cash-on-cash, BRRR recycled %, max-bid inversion; nulls when rent estimate missing, never fabricated), `test-harness.js`, `test-enrichment.js`, `test-healing-agent.js`, `test-recall.js`, `test-recall-gate.js` (THE 100% COMMANDMENT — a scrape below sentinel parity fires a coverage-bug alert), `test-extraction-chunking.js` (dense-page chunking so extraction never truncates at the output cap), `test-fundability.js`, `test-manifest.js`, `test-fabrication-guard.js`, `test-search-parse.js` (AI-search response salvage), `test-smart-query-parse.js` (AI-search query parsing: price/beds/location extraction), `test-search-deep-read.js` (AI-search stage-2 deep read: full-narrative payload budgets, verdict index mapping, semantic-query gate), `test-charlesdarrow-recogniser.js`, `test-sdlauctions-recogniser.js`, `test-cliveemson-recogniser.js`, `test-savills-recogniser.js` (calendar drill, divider-bleed and sold/withdrawn anti-leak, sentinel counts in both markdown and HTML), `test-foxandsons-recogniser.js` / `test-bagshaws-recogniser.js` / `test-williamhbrownnorwich-recogniser.js` (the three Sequence/Connells branches sharing ONE recogniser — anti-fabrication: page nav, footer office address and page chrome must never become lots), `test-calendar-selection.js` (an `always_on` evergreen catalogue must never be starved out of the scrape queue by dated junk rows — the bug that took suttonkersh dark), `test-suttonkersh-recogniser.js` (also pins the `rewriteUrl` query-param case repair — lowercased `auctionperiod` fetched the 15MB archive, not the 22-lot sale), `test-freshness-pulse.js` (the pulse prefers an `always_on` evergreen row over a dated hub, mirroring the daily pass), `test-ghost-sweep.js` (retirement needs BOTH scope and completeness evidence; flip-patch keys are asserted against `LOT_COLUMNS`).
