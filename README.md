...
**William H Brown (Norwich)** is the reference for the *freshness-rot* case: the house scraped "successfully" the whole time, but with no recogniser it depended on the AI extractor, so once the quota died its correct rows stopped being re-stamped and aged past `get_active_lots`' 7-day `last_seen_at` window — 0 live while `lots` still held them. Degraded AI runs then fabricated (a lot built from the office address in the page footer; another with the page logo as its photo). The catalogue is a hand-built static XHTML table, so `recogniseWilliamHBrownNorwichLotsFromMarkdown` (`staticCatalogue`) parses it with zero AI: photos bind by **shared detail URL** rather than position (photo cells and text cells are separate table rows), and the live boundary comes from the auction date embedded in the lot URL slug (`/auctions/28-july-2026/`) because the page carries no ended badges — which also replaces the `2099` `always_on` calendar placeholder with the real date. Verified 19/19 = 100% recall against the live page, counted **after** `normaliseScrapedLot`.
...
Key test files: `test-scoring.js`, `test-harness.js`, `test-enrichment.js`, `test-healing-agent.js`, `test-recall.js`, `test-recall-gate.js` (THE 100% COMMANDMENT — a scrape below sentinel parity fires a coverage-bug alert), `test-extraction-chunking.js` (dense-page chunking so extraction never truncates at the output cap), `test-fundability.js`, `test-manifest.js`, `test-fabrication-guard.js`, `test-search-parse.js` (AI-search response salvage), `test-smart-query-parse.js` (AI-search query parsing: price/beds/location extraction), `test-charlesdarrow-recogniser.js`, `test-sdlauctions-recogniser.js`, `test-cliveemson-recogniser.js`, `test-williamhbrownnorwich-recogniser.js` (recall counted *after* `normaliseScrapedLot`, plus the live-boundary and photo-binding contracts).
