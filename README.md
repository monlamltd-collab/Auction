...
**Bagshaws Residential** is the same AI-independence case on a *hand-built* page — no CMS, no JS, a table layout whose lot cards link out to the Sequence/Connells group's shared national sale on `barnardmarcusauctions.co.uk/auctions/{DD-month-YYYY}/{id}/`. `recogniseBagshawsLotsFromMarkdown` parses one plain-HTTP fetch (`staticCatalogue`) and pins three things the AI kept getting wrong: a `**Sold Prior**` lot emits `lot_status='sold'`, the sale date is parsed from the lot-URL slug (so the house expires correctly instead of inheriting its `2099-12-31` `always_on` calendar row), and a lot whose text anchor is a broken `href="link"` is recovered from its thumbnail block (the image basename is the lot number). Verified 23/23 = 100% recall against the live page. See `docs/houses/bagshaws.md`.
